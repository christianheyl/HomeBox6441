...
static int ifx_gpio_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
#else
...
#if 1
static int proc_read_old_board(char *buf, char **start, off_t offset, int count, int *eof, void *data);
#endif
...
/* Terry 20150317, need to hack for new/old boards. */
#if 1
    create_proc_read_entry("old_board",
						   0,
						   g_gpio_dir,
						   proc_read_old_board,
						   NULL);
...
/* Terry 20150317, need to hack for new/old boards. */
#if 1
	remove_proc_entry("old_board", g_gpio_dir);
#endif
...
/* Terry 20150317, need to hack for new/old boards. */
/* This function is for userspace query */
#if 1
static int proc_read_old_board(char *buf, char **start, off_t offset, int count, int *eof, void *data)
{
    int len = 0;
...
/* Terry 20150317, need to hack for new/old boards. */
#if 1
static struct ifx_gpio_ioctl_pin_config *modify_board_gpio_pin_map(struct ifx_gpio_ioctl_pin_config *in_cfg, int old_gpio, int new_gpio)
{
	struct ifx_gpio_ioctl_pin_config *p = in_cfg;
...
#if 1
	if (is_old_board()) {
		/* Hack g_board_gpio_pin_map */
		struct ifx_gpio_ioctl_pin_config *ptr_gpio_pin_cfg = &g_board_gpio_pin_map[0];
		
		printk("[ifxmips_gpio] Old board detected. Need to modify GPIO table.\n");
		/* IFX_GPIO_PIN_ID(0, 3) => IFX_GPIO_PIN_ID(0, 11) */
		ptr_gpio_pin_cfg = modify_board_gpio_pin_map(ptr_gpio_pin_cfg, IFX_GPIO_PIN_ID(0, 3), IFX_GPIO_PIN_ID(0, 11));
		
		/* IFX_GPIO_PIN_ID(0, 14) => IFX_GPIO_PIN_ID(3, 10) */
		ptr_gpio_pin_cfg = modify_board_gpio_pin_map(ptr_gpio_pin_cfg, IFX_GPIO_PIN_ID(0, 14), IFX_GPIO_PIN_ID(3, 10));

		/* IFX_GPIO_PIN_ID(0, 11) => IFX_GPIO_PIN_ID(0, 3) */
		ptr_gpio_pin_cfg = modify_board_gpio_pin_map(ptr_gpio_pin_cfg, IFX_GPIO_PIN_ID(0, 11), IFX_GPIO_PIN_ID(0, 3));
		
		/* IFX_GPIO_PIN_ID(3, 10) => IFX_GPIO_PIN_ID(0, 14) */
		ptr_gpio_pin_cfg = modify_board_gpio_pin_map(ptr_gpio_pin_cfg, IFX_GPIO_PIN_ID(3, 10), IFX_GPIO_PIN_ID(0, 14));
	}
#endif
