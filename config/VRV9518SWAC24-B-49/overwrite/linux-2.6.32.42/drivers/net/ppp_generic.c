...
#ifdef CONFIG_FFP
		{
			struct ppp_keepalive_info keepalive;

			if(ffp_get_ppp_keepalive_info(ppp->dev, &keepalive))
			{
				idle.xmit_idle = (jiffies - MAX(ppp->last_xmit, keepalive.jiffies_last_xmit)) / HZ;
				idle.recv_idle = (jiffies - MAX(ppp->last_recv, keepalive.jiffies_last_recv)) / HZ;
			}
			else
			{
...
#ifdef CONFIG_FFP
	ffp_clean_ppp_keepalive_info(ppp->dev);
#endif
