...
/* Terry 20141209, disable prom_printf for security issue */
unsigned long g_disable_prom_printf = 1;
...
static char * chip_arg[3] =
{
    ...
    "root=/dev/ram rw ip=172.20.80.222:172.20.80.1::::eth0:off console=ttyS0,38400 ethaddr=00:01:02:03:04:05 phym=32M mem=32M panic=1 nowait=1 shell",
   #else
    "root=/dev/ram rw ip=172.20.80.222:172.20.80.1::::eth0:off console=ttyS0,9600 ethaddr=00:01:02:03:04:05 phym=32M mem=32M panic=1 nowait=1",
   #endif
...
{
    change_c0_status(ST0_IM, ST0_IM);
}
#endif

/* Terry 20150317, need to hack for new/old boards. */
#if 1
int is_old_board(void)
{
	static int old_board_ret = -1;
	volatile unsigned int reg_val;
	
	if (old_board_ret == -1) {
		*((volatile unsigned int *)0xbe100b18) &= ~(0x01);		/* GPIO0 DIR -> IN */
		*((volatile unsigned int *)0xbe100b1c) &= ~(0x01);		/* GPIO0 ALT0 -> 0 */
		*((volatile unsigned int *)0xbe100b20) &= ~(0x01);		/* GPIO0 ALT1 -> 0 */
		*((volatile unsigned int *)0xbe100b30) |=  (0x01);		/* GPIO0 PUDEN -> 1:Enable */
		*((volatile unsigned int *)0xbe100b2c) &= ~(0x01);		/* GPIO0 PUDSEL -> 0:Down */
		*((volatile unsigned int *)0xbe100b14) &= ~(0x01);		/* GPIO0 IN write 0 for detection */
		reg_val = *((volatile unsigned int *)0xbe100b14);
		
		if (reg_val & 0x01)
			old_board_ret = 0;
		else
			old_board_ret = 1;
		prom_printf("[%s] 0x%08x 0x%08x\n", __FUNCTION__, 0xbe100b18, *((volatile unsigned int *)0xbe100b18));
		prom_printf("[%s] 0x%08x 0x%08x\n", __FUNCTION__, 0xbe100b1c, *((volatile unsigned int *)0xbe100b1c));
		prom_printf("[%s] 0x%08x 0x%08x\n", __FUNCTION__, 0xbe100b20, *((volatile unsigned int *)0xbe100b20));
		prom_printf("[%s] 0x%08x 0x%08x\n", __FUNCTION__, 0xbe100b30, *((volatile unsigned int *)0xbe100b30));
		prom_printf("[%s] 0x%08x 0x%08x\n", __FUNCTION__, 0xbe100b2c, *((volatile unsigned int *)0xbe100b2c));
		prom_printf("[%s] 0x%08x 0x%08x\n", __FUNCTION__, 0xbe100b14, *((volatile unsigned int *)0xbe100b14));
		prom_printf("[%s] old_board_ret %d\n", __FUNCTION__, old_board_ret);
	}

	return old_board_ret;
}
EXPORT_SYMBOL(is_old_board);
#endif
...
void __init prom_init(void)
{
	...
	/* Terry 20141209, disable prom_printf for security issue */
	do {
		char **l_argv = (char **)KSEG1ADDR((unsigned long)argv);
		for (i = 0; i < fw_arg0; i++) {
			if (strstr((char *)KSEG1ADDR(l_argv[i]), "console=none")) {
				g_disable_prom_printf = 1;
				break;
			} else if (strstr((char *)KSEG1ADDR(l_argv[i]), "console=ttyS0")) {
				g_disable_prom_printf = 0;
				break;
			}
		}
	} while (0);
...
#if 1
	if (is_old_board()) {
		prom_printf("[%s] Old board detected.", __FUNCTION__);
	}
#endif

	
#ifdef CONFIG_HN1
    prom_printf("Lantiq G.Hn Networking Transceiver " BOARD_SYSTEM_TYPE "\n");
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,32) && defined(CONFIG_MIPS_MT_SMP)
    vsmp_smp_ops.init_secondary = vsmp_init_secondary;
