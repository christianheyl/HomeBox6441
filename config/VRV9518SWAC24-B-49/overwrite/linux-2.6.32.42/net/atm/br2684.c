...
/* arcadyan:
We found that sometimes, when adsl just sync up, packets flow-in, before nas interface is ready.
This makes br2684 panic.
So, we put a check here as a workaround.
*/
	brvcc = BR2684_VCC(atmvcc);
	if (brvcc==NULL)
	{
		printk("\nbr2684_push:brvcc=NULL");
		goto free_skb;
	}
...
#if 1 /* Terry 20140904, fix exception problem */
		struct net_device *dev = skb->dev;
