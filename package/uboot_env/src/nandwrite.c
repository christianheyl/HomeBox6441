...
//	int readcnt = 0;
	...
	if (pad && writeoob) {
        ...
        if(strcmp(mtd_dev,"") == 0) {
        	fprintf(stderr,"srcAddr = %lx destAddr = %lx, partition is NOT FOUND\n",srcAddr, destAddr);
		exit (EXIT_FAILURE);
        }
        fprintf(stderr,"mtd_dev = %s srcAddr = %lx destAddr = %lx, partition FOUND\n",mtd_dev, srcAddr, destAddr);
	mtd_device = mtd_dev;
#endif
...
#else
	ifd = 4; //Caution: Used 4 to minimize the changes in the subsequent code
#endif
	...
	while (imglen && (mtdoffset < meminfo.size)) {
		...
		while (blockstart != (mtdoffset & (~meminfo.erasesize + 1))) {
			...
			fprintf(stderr, "Erasing from %08lx-%08lx\n",
					(long)erase.start, (long)erase.start+erase.length-1);
			if (ioctl(fd, MEMERASE, &erase) != 0) {
				fprintf(stderr, "Erasing failed from %08lx-%08lx\n",
					(long)erase.start, (long)erase.start+erase.length-1);
				goto closeall;
			}
#endif
			if (!quiet)
				fprintf (stderr, "Writing data to block %d at offset 0x%x\n",
...
#if defined(BUILD_FROM_IFX_UTILITIES)
				//fprintf(stderr, "index = %u, readlen = %u\n", index, readlen);
				memcpy(writebuf + tinycnt, (char *) (srcAddr + index), readlen - tinycnt);
				index += (readlen - tinycnt);
				if (index >= (srcLen-1))
					cnt = 0;
				else
					cnt = (readlen - tinycnt);
		...
		if (writeoob) {
			...
			while(tinycnt < readlen) {
#if defined(BUILD_FROM_IFX_UTILITIES)
				memcpy(oobreadbuf + tinycnt, (char *)(srcAddr + index), meminfo.oobsize - tinycnt);
				index += (meminfo.oobsize - tinycnt);
				if (index >= (srcLen - 1))
					cnt = 0;
				else
					cnt = (meminfo.oobsize - tinycnt);
#else
		...
		if (write(fd, writebuf, meminfo.writesize) != meminfo.writesize) {
			int rewind_blocks;
			off_t rewind_bytes;
			erase_info_t erase;
		...
		//fprintf(stderr,"successfully wrote %u bytes remaining length = %u\n", meminfo.writesize, imglen);
	}
...
restoreoob:
	if (oobinfochanged == 1) {
		if (ioctl (fd, MEMSETOOBSEL, &old_oobinfo) != 0) {
			perror ("MEMSETOOBSEL");
			close (fd);
			fprintf(stderr,"restoreoob closed mtd oobinfochanged\n");
			exit (EXIT_FAILURE);
		}
	}
