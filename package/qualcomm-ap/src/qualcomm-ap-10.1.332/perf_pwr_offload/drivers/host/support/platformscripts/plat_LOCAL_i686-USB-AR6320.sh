#!/bin/sh -x
#
# Platform-Dependent load script for x86 systems (USB)
# 
#
...
case $1 in
    ...
    /sbin/insmod $IMAGEPATH/$AR6K_MODULE_NAME.ko $AR6K_MODULE_ARGS htc_credit_flow=0 hif_usb_disable_rxdata2=1
    if [ $? -ne 0 ]; then
