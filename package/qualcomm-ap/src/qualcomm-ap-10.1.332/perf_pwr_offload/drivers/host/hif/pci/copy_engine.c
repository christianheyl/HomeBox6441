...
/*
 * Updating the CE# 4 indices
 */
void
CE_tx_desc_cmpl(struct CE_state *ce_state, uint32_t num_tx_cmpls)
{
    struct CE_ring_state *src_ring = ce_state->src_ring;
    uint32_t nentries_mask = src_ring->nentries_mask;
     ...
     */
    src_ring->sw_index += (num_tx_cmpls << 1);
    src_ring->sw_index &= nentries_mask;
...
CE_per_engine_service_fast(struct ath_hif_pci_softc *sc, uint32_t ce_id)
{
    ...
    nbuf_cmpl_arr = htt_pdev->nbuf_cmpl_arr;
    ...
    for (;;) {
        ...
        if (adf_os_unlikely(nbuf_cmpl_idx) == MSG_FLUSH_NUM) {
            ...
            htt_t2h_msg_handler_fast(htt_pdev, nbuf_cmpl_arr, MSG_FLUSH_NUM, &num_tx_cmpls);
            ...
            CE_tx_desc_cmpl(CE_tx_cmpl_state, num_tx_cmpls);
            num_tx_cmpls = 0;
            nbuf_cmpl_idx = 0;
        }
    ...
    if (adf_os_likely(nbuf_cmpl_idx)) {
        ...
        htt_t2h_msg_handler_fast(htt_pdev, nbuf_cmpl_arr, nbuf_cmpl_idx, &num_tx_cmpls);
...
#if QCA_OL_11AC_FAST_PATH
    /*
     * Fast path specific optimizations
     */
    /* The magic no. 28 is derived as follows:
     * 14 (Ethernet header) +
     *  4 (VLAN header) +
     *  8 (SNAP/LLC header, if at all) +
     *  2 (2 bytes from IP header for TOS)
     * --
     * 28 bytes (total)
     */
    if (CE_state->id == CE_HTT_TX_CE) {
        CE_state->download_len = 28;
    }
#endif
