...
/* ARCADYAN, #ARC_RC_SD2#, 2013/4/24, Dino, Select alternative datapath for wireless */
#define PPA_HOOK_USE_SPECIFIC_DATAPATH    1
...
#ifdef CONFIG_IFX_PPA
#if PPA_HOOK_USE_SPECIFIC_DATAPATH  /* ARCADYAN #ARC_RC_SD2#, 2013/4/16, Dino, Make specific PPA path(rmv redundent) for wireless drv */
    if( ppa_hook_directpath_send_spec_fn && ppa_pltfrm_data.ppa_enable[osifp->os_unit] && (ppa_pltfrm_data.dev_if_index[osifp->os_unit] == dev->ifindex )) {
        ret = ppa_hook_directpath_send_spec_fn(ppa_pltfrm_data.ppa_if_id[osifp->os_unit], skb, skb->len, 0);
#else
...
//#endif
    ...
    if( ppa_hook_directpath_send_fn && ppa_pltfrm_data.ppa_enable[osifp->os_unit] && (ppa_pltfrm_data.dev_if_index[osifp->os_unit] == dev->ifindex )) {
        ...
        if(ret == 0){
            ...
            /* WAR for ppa_hook_directpath_send_fn() return -1 but still hold the skb */
#if 0  /* ARCADYAN #ARC_RC_DS2#, 2013/4/19, Dino, remove debug message */
       /* Under heavy traffic, while DMA to PPE is full, dropped packets will be log here and caused
        * slow response in wireless driver which made WatchDog reset.
        */
            printk("%s: ret=%d, os_unit=%d ppa_enable=%d ppa_dev_if_index=%d dev_if_index=%d \n", 
                __func__, ret, osifp->os_unit,  ppa_pltfrm_data.ppa_enable[osifp->os_unit], 
                ppa_pltfrm_data.dev_if_index[osifp->os_unit], dev->ifindex);
