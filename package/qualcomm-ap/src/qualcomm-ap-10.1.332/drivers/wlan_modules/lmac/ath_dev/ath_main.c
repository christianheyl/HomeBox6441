...
#endif
static u_int8_t* ath_get_tpc_tables(ath_dev_t dev);
#if ATH_SUPPORT_DFS
...
#if ATH_SUPPORT_SPECTRAL
        if (!p_sops->is_spectral_active(sc))
#endif /* ATH_SUPPORT_SPECTRAL */
        {
...
ath_calibrate(void *arg)
{
    ...
    if (work_to_do) {
        ...
        if (aniflag) {
            ...
            if ((sc->sc_dcs_enabled & ATH_CAP_DCS_WLANIM) && (sc->sc_opmode == HAL_M_HOSTAP) &&
        ...
        if (longcal || shortcal || multical) {
            ...
            if (sc->sc_curchan.channel_flags & CHANNEL_CW_INT) {
                /* CW interference has been detected. Inform the app layer if someone is listning */
                spectral_send_eacs_msg((struct ath_spectral*)sc->sc_spectral);
...
ath_set_proxysta(ath_dev_t dev, int enable)
{
    ...
    if ((enable && !sc->sc_enableproxysta) ||
        (!enable && sc->sc_enableproxysta)) 
    {
        DPRINTF(sc, ATH_DEBUG_ANY, "%s: set proxy sta mode to %d\n", __func__, enable);
        sc->sc_enableproxysta = enable ? 1 : 0;
...
ath_node_update_pwrsave(ath_dev_t dev, ath_node_t node, int pwrsave, int pause_resume)
{
    ...
    if (pwrsave) {
        ...
        if (!an->an_tim_set &&
#if LMAC_SUPPORT_POWERSAVE_QUEUE
            sc->sc_ath_ops.get_pwrsaveq_len(an->an_node, 0) &&
#else
...
/*
 * Function     : ath_dev_get_ctldutycycle
 * Description  :
 * Input        :
 * Ouput        :
 *
...
int ath_dev_get_ctldutycycle(ath_dev_t dev)
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    struct ath_spectral* p_spectral = (struct ath_spectral*)sc->sc_spectral;
    return get_eacs_control_duty_cycle(p_spectral);
}

/*
 * Function     : ath_dev_get_extdutycycle
 * Description  :
 * Input        :
 * Ouput        :
 *
...
int ath_dev_get_extdutycycle(ath_dev_t dev)
{
    struct ath_softc *sc = ATH_DEV_TO_SC(dev);
    struct ath_spectral* p_spectral = (struct ath_spectral*)sc->sc_spectral;
    return get_eacs_extension_duty_cycle(p_spectral);
}
...
static u_int8_t* ath_get_tpc_tables(ath_dev_t dev)
{
    ...
    if (ah->ah_get_tpc_tables)
        return ath_hal_get_tpc_tables(ah);
    else
        return NULL;
}
#if ATH_SUPPORT_DFS
...
#define   HAL_CAP_STRONG_DIV 2
     ...
     */
    if (ptr) {
        OS_MEMCPY(ptr, &sc->dfs_caps, sizeof(ptr));
    }
