...
#define    N(a)    (sizeof(a)/sizeof(a[0]))
    ...
    if (!ATH_TXQ_SETUP(sc, qnum)) {
        ...
        } else if (qtype == HAL_TX_QUEUE_WIFIPOS){
            // giving the parameters same as VOICE access class 
            txq->axq_minfree = 0 ;
#endif 
        } else {
...
#endif
#if ATH_SUPPORT_WIFIPOS
    printk("%s: Setting up WIFIPOSQ \n", __func__);
    sc->sc_wifiposq = ath_txq_setup(sc, HAL_TX_QUEUE_WIFIPOS, 3); // Giving the highest priority similar to WME_AC_VI
    if (sc->sc_wifiposq == NULL) {
...
#endif
#if ATH_SUPPORT_WIFIPOS
    case HAL_TX_QUEUE_WIFIPOS:
        qnum = sc->sc_wifiposq->axq_qnum;
        printk("%s: getting wifipos qnum: %d \n", __func__, qnum);
        break;
#endif
...
#define DESC2PA(_sc, _va)	\
    ...
    if (sc->sc_enhanceddmasupport) {
        ...
        */
        ...
        ath_hal_puttxbuf(ah, txq->axq_qnum, bf->bf_daddr);
/*
        if(txq->axq_qnum == sc->sc_wifiposq->axq_qnum) 
...
/*  Debugging tools for wifipos data
    if(txctl->qnum == sc->sc_wifiposq->axq_qnum){
        if(cnt_tx_wifipos == 0) {
...
ath_ratecode_to_ratekbps(struct ath_softc *sc, int ratecode)
{
	...
	rateindex = rt->rateCodeToIndex[ratecode];
   
	return sc->sc_hwmap[rateindex].rateKbps;
}
 ...
 * hard coded for 10 pkts. 
    if(bf->bf_qnum == sc->sc_wifiposq->axq_qnum) {
        cnt_tx_wifipos++;
...
#define VALID_TXBF_RATE(_rate,_nss) ((_rate >= 0x80) && (_rate < ((_nss == 2)?0x90:0x88)))
    ...
    ath_tx_update_hwretries_estimate(sc, bf, ts);
...
ath_buf_set_rate(struct ath_softc *sc, struct ath_buf *bf)
{
     ...
     */
    if (bf->bf_reftxpower == ATH_TX_POWER_SRM) 
        txpower = ATH_TX_POWER_SRM;
    ...
    if((flags & HAL_TXDESC_ENABLE_DURATION) || (bf->bf_ispspoll)){
