...
ath_coex_mci_setup(struct ath_softc *sc)
{
    ...
    adf_os_mem_set(mci->mci_sched_buf.bf_addr, MCI_GPM_RSVD_PATTERN, mci->mci_sched_buf.bf_len);
    ...
    /* Initialize the buffer */
    adf_os_mem_set(mci->mci_gpm_buf.bf_addr, MCI_GPM_RSVD_PATTERN, mci->mci_gpm_buf.bf_len);
