    ...
    case IEEE80211_POWER_STA_EVENT_DTIM:
        if (powersta->ips_use_pspoll ) {
            ...
            if (powersta->ips_vap->iv_uapsd != 0) {
                 ...
                 * This capability is used for TDLS Peer U-APSD wifi testing.
                */
                                                  ...
                                                  u_int32_t           pspoll_enabled)
{
    ...
    OS_INIT_TIMER(os_handle, &handle->ips_pause_notif_timer,                         
                  ieee80211_sta_power_pause_notif_timer, handle);
    ...
    OS_INIT_TIMER(os_handle, &handle->ips_wnmsleep_timeout_timer,
                  ieee80211_sta_power_wnmsleep_timeout_event_timer, handle);
#endif
...
{
    ieee80211_tdls_peer_uapsd_force_sleep(vap, enable);
    if (!vap->iv_pwrsave_sta) {
        return EINVAL; 
    }
 ...
 *  @return EOK  on success and non zero on failure.
 */
int ieee80211_pwrsave_uapsd_set_max_sp_length(struct ieee80211vap *vap,u_int8_t max_sp_val  )
{
    u_int8_t val;
    int status= EOK;
