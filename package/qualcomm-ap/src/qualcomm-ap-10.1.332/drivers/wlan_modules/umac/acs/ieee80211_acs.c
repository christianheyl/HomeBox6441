...
struct ieee80211_acs {
    ...
    int32_t                             acs_chan_maxrssi[IEEE80211_CHAN_MAX];    /* max rssi of these channels */
    ...
    u_int32_t                           acs_intr_ts[IEEE80211_CHAN_MAX];
    u_int8_t                            acs_intr_status[IEEE80211_CHAN_MAX];
#endif
    ...
    bool                                acs_sec_chan[IEEE80211_CHAN_MAX];       /*secondary channel flag */
};
...
static INLINE void ieee80211_acs_get_phymode_channels(ieee80211_acs_t acs, enum ieee80211_phymode mode)
{
    ...
    printk("%s ic_freq: %d ic_ieee: %d ic_flags: %d \n", __func__, channel->ic_freq, channel->ic_ieee, channel->ic_flags);
        ...
        if (mode == ieee80211_chan2mode(channel)) {
            int ieee_chan_num = ieee80211_chan2ieee(acs->acs_ic, channel);
...
#if ATH_SUPPORT_SPECTRAL
        acs->acs_channel_loading[ieee_chan_num] = ic->ic_get_control_duty_cycle(ic);
#endif
    }
