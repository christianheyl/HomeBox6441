...
static int
wpa_supplicant_ctrl_iface_wps_get_config(struct wpa_supplicant *wpa_s,
					 char *buf, size_t buflen)
{
	struct wpa_ssid *ssid;
	char str[255], strval[255], psk[255], psk_buf[255];
	char wpa_pairwise[255], ssid_str[64];
	char *prefix;
	int i, ret;

	os_memset(buf, '\0', buflen);
	ssid = wpa_config_get_valid_network(wpa_s->conf);
	if (ssid != NULL) {
		if (ssid->ssid) {
			os_memcpy(ssid_str, ssid->ssid, ssid->ssid_len);
			ssid_str[ssid->ssid_len] = '\0';
		} else
			ssid_str[0] = '\0';
		...
		if (ret >= 0 && buflen > (size_t) ret) {
			...
			return -1;
		ret = sprintf(str, "key_mgmt=%d\n", ssid->key_mgmt);
		if (ret >= 0 && buflen > (size_t) ret) {
			strcat(buf, str);
			buflen -= ret;
		} else
			return -1;
		if (ssid->wep_tx_keyidx >=0 && ssid->wep_tx_keyidx < 4) {
			int key_len;

			key_len = ssid->wep_key_len[ssid->wep_tx_keyidx];
			if (key_len) {
				char key[255], keyval[255];

				if (key_len == 10 || key_len == 23) {
					sprintf(key, "%s", ssid->wep_key[
							ssid->wep_tx_keyidx]);
				} else {
					for (i = 0; i < key_len; i++) {
						ret = sprintf(keyval, "%02x",
							      ssid->wep_key[ssid->wep_tx_keyidx][i]);
						strcat(key, keyval);
					}
				}
				ret = sprintf(str, "wep_key=%s\n", key);
				if (ret >= 0 && buflen > (size_t) ret) {
					strcat(buf, str);
					buflen -= ret;
				} else
					return -1;
				ret = sprintf(str, "wep_default_key=%d\n",
					      ssid->wep_tx_keyidx);
				if (ret >= 0 && buflen > (size_t) ret) {
					strcat(buf, str);
					buflen -= ret;
		...
		if (ssid->psk_set) {
			for (i = 0; i < PMK_LEN; i++) {
				sprintf(psk, "%02x", ssid->psk[i]);
				strcat(psk_buf, psk);
			}
			ret = sprintf(str, "wpa_psk=%s\n", psk_buf);
		} else {
			ret = sprintf(str, "wpa_passphrase=%s\n",
				      ssid->passphrase);
		}
		if (ret >= 0 && buflen > (size_t) ret) {
			strcat(buf, str);
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		reply_len = wpa_supplicant_ctrl_iface_wps_get_config(
