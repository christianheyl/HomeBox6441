...
ath_bar_rx(struct ath_softc *sc, struct ath_node *an, wbuf_t wbuf)
{
...
#if ATH_SUPPORT_AOW
    /* 
     * For now, we restrict the below fix to AoW builds.
     * 
     * If the rx reordering buffer is empty or the MPDUs
     * that have arrived have sequence number less than
     * BAR SSN - 1, then seq_next does not get incremented
     * to SSN. This leads to unnecessary waiting on MPDUs
     * which will not be arriving. To avoid this issue,
     * we force set seq_next to the BAR SSN.
     * 
     * Note that this is a simple fix which only handles cases
     * where BAR SSN deltas are low and index < baw_size
     * (as commonly found in low throughput scenarios like AoW).
     * 
     * There are other corner cases which are not handled right
     * now, such as one in which the index of the BAR SSN is
     * greater than baw_size but lesser than
     * IEEE80211_SEQ_MAX - (rxtid->baw_size << 2)). These cases
     * need to be fixed and tested. EV ID #78876 has been created
     * to track this.
     * 
     * A generic fix for all these cases might later supersede
     * the below simple fix. 
     */
    ...
    if ((rxtid->baw_head == rxtid->baw_tail) &&
        ...
        /*
         * If we hit this point,
         * a) Some consecutive MPDUs just before BAR seqno will not be
         *    arriving.
         * b) MPDUs starting with BAR seqno are yet to arrive.
         * c) rxtid->seq_next has not become equal to BAR seqno.
         * 
         * We explicitly set rxtid->seq_next equal to BAR seqno. This will
         * prevent unnecessary waiting on the MPDUs that will not arrive,
         * in other parts of the code. 
         */

        rxtid->seq_next = seqno;
    }
...
ath_ampdu_input(struct ath_softc *sc, struct ath_node *an, wbuf_t wbuf, ieee80211_rx_status_t *rx_status)
{
...
#if ATH_SUPPORT_AOW
#if ATH_SUPPORT_AOW_DYNRXTIMEOUT
    struct ath_rxbuf                   *local_rxbuf = NULL;
    int                                local_baw_head = 0;
    int32_t                            dyn_rx_timeout = -1;
#if ATH_SUPPORT_AOW_DYNRXTIMEOUT_PROFILING
    int16_t                            apktindex = -1;
#endif
#endif
    ...
    bf->bf_rx_tsftime = ath_hal_gettsf64(sc->sc_ah);
#endif
...
#if ATH_SUPPORT_AOW && ATH_SUPPORT_AOW_DYNRXTIMEOUT
            if (TID_TO_WME_AC(tid) == WME_AC_VO) {

                /* AoW performance optimization:
                   AoW is latency sensitive. So we deep inspect the queue and find
                   if any AoW frames are held up. If so, we adjust rxtimeout so
                   that the AoW frames are delivered upwards before their time-to-play
                   can possibly expire. */

                local_baw_head = rxtid->baw_head;

                /* We are sure the very first entry has a NULL wbuf. */
                INCR(local_baw_head, ATH_TID_MAX_BUFS);
                
                while (local_baw_head != rxtid->baw_tail)
                {
                    local_rxbuf = rxtid->rxbuf + local_baw_head;
                    
                    if (!local_rxbuf->rx_wbuf) {
                        INCR(local_baw_head, ATH_TID_MAX_BUFS);
                        continue;
                    }
                   
                    dyn_rx_timeout =
                        ath_aow_calc_timeremaining(sc,
                                                   local_rxbuf->rx_wbuf);
                ...
                if (dyn_rx_timeout < 0) {
                    /* Couldn't successfully find an AoW packet.
                       We use the default rxtimeout. */
#if ATH_SUPPORT_AOW_DYNRXTIMEOUT_PROFILING
                    IEEE80211_AOW_INFO_DPRINTF("AoW DRP:t=%llu,s=NA,ss=NA,r=Def,a=S\n",
                                               ath_hal_gettsf64(sc->sc_ah));
#endif
                    ATH_SET_RXTIMER_PERIOD(&rxtid->timer,
                                         sc->sc_rxtimeout[TID_TO_WME_AC(tid)]);
                } else {
#if ATH_SUPPORT_AOW_DYNRXTIMEOUT_PROFILING
                    apktindex = ATH_AOW_GET_AOWPKTINDEX(sc,
                                                        local_rxbuf->rx_wbuf);
                    /* Since !(dyn_rx_timeout < 0), we can be sure
                       apktindex is fine. */

                    IEEE80211_AOW_INFO_DPRINTF("AoW DRP:t=%llu,s=%u,ss=%u,r=%d,a=S\n",
                                               ath_hal_gettsf64(sc->sc_ah),
                                               ATH_AOW_ACCESS_APKT_SEQ(\
                                                    local_rxbuf->rx_wbuf,
                                                    apktindex),
                                               ATH_AOW_ACCESS_APKT_SUBSEQ(\
                                                    local_rxbuf->rx_wbuf,
                                                    apktindex),
                                               dyn_rx_timeout);
#endif

                    ATH_SET_RXTIMER_PERIOD(&rxtid->timer, dyn_rx_timeout/1000);
                }

                ATH_START_RXTIMER(&rxtid->timer);
            } else {
                /* In non-AoW builds, the below code is not bound in-within 
                   an 'else' */
#endif /* ATH_SUPPORT_AOW && ATH_SUPPORT_AOW_DYNRXTIMEOUT */
                ATH_SET_RXTIMER_PERIOD(&rxtid->timer, sc->sc_rxtimeout[TID_TO_WME_AC(tid)]);
                ATH_START_RXTIMER(&rxtid->timer);
#if ATH_SUPPORT_AOW && ATH_SUPPORT_AOW_DYNRXTIMEOUT
            } /* else of (TID_TO_WME_AC(tid) == WME_AC_VO) */
#endif
        }
    } else {
        if (ATH_RXTIMER_IS_ACTIVE(&rxtid->timer)) {
...
ath_rx_timer(void *context)
{
...
#if ATH_SUPPORT_AOW && \
    ATH_SUPPORT_AOW_DYNRXTIMEOUT && \
    ATH_SUPPORT_AOW_DYNRXTIMEOUT_PROFILING
    IEEE80211_AOW_INFO_DPRINTF("AoW DRP:t=%llu,s=NA,ss=NA,r=NA,a=E\n",
                               ath_hal_gettsf64(sc->sc_ah));
#endif
    ...
    while (baw_head != rxtid->baw_tail) {
        if (!rxtid->rxbuf) {
            KASSERT(rxtid->rxbuf, ("%s[%d] rxtid timer active = %d, baw_head = %d, baw_tail = %d\n",
        ...
        if (TID_TO_WME_AC(tidno) == WME_AC_VO) {
            /* Stop if a hole has just been detected and there is some time
               remaining to deliver the next AoW frame.
               While we want to deliver AoW frames upward as fast as possible,
               we also want to give missing frames the max possible opportunity
               to make it in.
             */
            time_remaining = ath_aow_calc_timeremaining(sc, rxbuf->rx_wbuf);
            
            if ((time_remaining >= AOW_DYNRXTIMEOUT_THRESHOLD) && (count > 0)) {
#if ATH_SUPPORT_AOW_DYNRXTIMEOUT_PROFILING
                apktindex = ATH_AOW_GET_AOWPKTINDEX(sc, rxbuf->rx_wbuf);
                
                /* Since !(time_remaining < 0), we can be sure apktindex is fine.*/

                IEEE80211_AOW_INFO_DPRINTF("AoW DRP:t=%llu,s=%u,ss=%u,r=%d,a=H\n",
                                            ath_hal_gettsf64(sc->sc_ah),
                                            ATH_AOW_ACCESS_APKT_SEQ(\
                                                    rxbuf->rx_wbuf,
                                                    apktindex),
                                            ...
                                            time_remaining);
#endif
                ATH_SET_RXTIMER_PERIOD(&rxtid->timer, time_remaining/1000);
                break;
            }
        } else {
            /* In non-AoW builds, the below code is not bound in-within 
...
#if ATH_SUPPORT_AOW && ATH_SUPPORT_AOW_DYNRXTIMEOUT
        } /* else of (TID_TO_WME_AC(tidno) == WME_AC_VO) */
#endif
#endif 
        __11nstats(sc, rx_recvcomp);
        ...
        && ATH_SUPPORT_AOW_DYNRXTIMEOUT \
        && ATH_SUPPORT_AOW_DYNRXTIMEOUT_PROFILING
        if ((0 == head_aowframe_found) &&
            !((apktindex =
                    ATH_AOW_GET_AOWPKTINDEX(sc, wbuf_to_indicate)) < 0)) {
            
            head_aowframe_found = 1;

            IEEE80211_AOW_INFO_DPRINTF("AoW DRP:t=%llu,s=%u,ss=%u,r=NA,a=D\n",
                                        ath_hal_gettsf64(sc->sc_ah),
                                        ATH_AOW_ACCESS_APKT_SEQ(\
                                                    wbuf_to_indicate,
                                                    apktindex),
