...
htt_t2h_stats_pdev_tidq_stats_print(struct wal_dbg_tidq_stats *wal_pdev_tidq_stats, int concise)
{
    ...
    struct wal_dbg_cong_bin_stats *cb_stats = &wal_pdev_tidq_stats->bin_stats;
    int i = 0;
    ...
    else
    {
     ...
     adf_os_print("\n\t------------------------------\n");
     adf_os_print("\n  Congestion stats for bins..\n");
     adf_os_print(" Current congestion stats per congestion index..\n");
     for(i=0; i < 15; i++)
      adf_os_print("\t%d", (unsigned short)cb_stats->curr_cong[i]);
     adf_os_print("\n Max congestion stats per congestion index \n");
     for(i=0; i < 15; i++)
       adf_os_print("\t%d", (unsigned short)cb_stats->max_cong[i]);
      adf_os_print("\n"); 
   }
}
static void
