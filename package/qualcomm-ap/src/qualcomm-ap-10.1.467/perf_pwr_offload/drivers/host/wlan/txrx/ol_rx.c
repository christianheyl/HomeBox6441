            ...
            for (i = 0; i < num_mpdus; i++) {
                ...
                if (msdu_chaining) {
		           ...
		           while (1) {
                    ...
                    if (adf_os_mem_cmp(wh->i_addr2, peer->mac_addr.raw, IEEE80211_ADDR_LEN) != 0) {
                        u_int32_t attn;
                        wrong_peer = 1;
                        attn = (u_int32_t)htt_rx_msdu_rx_attention_retrieve(htt_pdev, head_msdu);
                        printk("%s: peer addr %s ", __func__, ether_sprintf(peer->mac_addr.raw));
                        printk(" src addr %s ", ether_sprintf(wh->i_addr2));
                        printk(" rx_attention flags 0x%x\n", attn);
                    }
                    if (
#if WDS_VENDOR_EXTENSION
                        !accept ||
                         ...
                         */
                        if(!wrong_peer)   //If wrong_peer is set, don't check this ASSERT condition
