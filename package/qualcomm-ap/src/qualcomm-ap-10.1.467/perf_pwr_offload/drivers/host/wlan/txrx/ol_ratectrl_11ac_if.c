        ...
        struct peer_ratectrl_params_t *peer_ratectrl_params)
{
    ...
    if (peer_ratectrl_params->ni_htcap & IEEE80211_HTCAP_C_SM_ENABLED) {
        ...
        struct ol_txrx_vdev_t *vdev)
{
    ...
    g_pRATE = adf_os_mem_alloc(pdev->osdev, sizeof(*g_pRATE));
    if (!g_pRATE) {
        return NULL; /* failure */
    }

    adf_os_mem_set(g_pRATE, 0, sizeof(*g_pRATE));
        ...
        struct ol_txrx_peer_t *peer)
{
    ...
    rc_node = adf_os_mem_alloc(pdev->osdev, sizeof(*rc_node));
    if(!rc_node) {
        return NULL; /* failure */
    }

    adf_os_mem_set(rc_node, 0, sizeof(*rc_node));

    /* allocate per peer Rate control state */ 
    rc_node->txRateCtrl = adf_os_mem_alloc(pdev->osdev, sizeof(*(rc_node->txRateCtrl)));
    if(!rc_node->txRateCtrl) {
        adf_os_mem_free((void *)rc_node);
        return NULL; /* failure */
    }
...
void ol_ratectrl_peer_ctxt_detach(void *peer_ratectxt)
{
    ...
    if (rc_node != NULL) {
        if(!rc_node->txRateCtrl) {
            adf_os_mem_free((void *)rc_node->txRateCtrl);
        }
        adf_os_mem_free((void *)rc_node);
        rc_node = NULL;
    }
        ...
        void *args)
{
    ...
    adf_os_mem_set(rc_args, 0, MAX_SERIES_SUPPORTED * sizeof(RC_TX_DONE_PARAMS));
