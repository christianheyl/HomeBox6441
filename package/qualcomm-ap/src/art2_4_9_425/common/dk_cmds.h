...
//#ident	"ACI $Id: //depot/sw/branches/art2_main_per_cs/src/art2/common/dk_cmds.h#5 $, $Header: //depot/sw/branches/art2_main_per_cs/src/art2/common/dk_cmds.h#5 $"
...
enum COMMAND_IDS {
    ...
    COMMIT_CMD_ID,                          //196
    M_SET_MAC_ADDR_CMD_ID,                  //197
    M_SET_BT_MODE_CMD_ID,                   //198
    M_GET_FW_VERSION_ID,                    //199
};
...
/*
Do this prevent some windows project do not include path "olca\include"
Real TC_CMDS_SIZE_MAX define need to sync with "olca\includetestcmd.h"
*/
#ifndef TC_CMDS_SIZE_MAX       
#define TC_CMDS_SIZE_MAX 255   
#endif
/*
When transfer CMD_REPLY structure,
the whole CMD_REPLY will be put into TCMD->buf[TC_CMDS_SIZE_MAX+1].
So the max size of CMD_REPLY->cmdBytes[] should be depend on the size of 
TCMD->buf[TC_CMDS_SIZE_MAX+1].
(3*sizeof(A_UINT32)) is the size of replyCmdLen, replyCmdId and status.
*/
#define MAX_CMDBYTS_SIZE ((TC_CMDS_SIZE_MAX+1) - (3*sizeof(A_UINT32)))
...
typedef struct cmdReply {
	A_UINT32 replyCmdLen;
  	...
  	//A_UCHAR cmdBytes[MAX_CMDBYTS_SIZE]; // bytes of the command reply 
} CMD_REPLY;
...
#if (defined AR6004_BUILD)
	A_UINT8                rateMaskBitPosition[RATE_POWER_MAX_INDEX];
    A_UINT8                txPower[RATE_POWER_MAX_INDEX];
#else
...
enum commitCalDataOptions
{
    COMMIT_CAL_DATA_NONE = 0,
    COMMIT_CAL_DATA_FLASH,
    COMMIT_CAL_DATA_EEPROM,
    COMMIT_CAL_DATA_OTP,
    COMMIT_CAL_DATA_DONT_LOAD,
    COMMIT_CAL_DATA_FILE,
    COMMIT_CAL_DATA_DRAM,
};
