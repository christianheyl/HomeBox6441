    ...
    HAL_HT_MACMODE macmode, u_int8_t txchainmask, u_int8_t rxchainmask)
{
     ...
     * registers.
     */
    set_regs = ar9300_set_lean_channel_change_regs(ah);
    if (ar9300_get_power_mode(ah) != HAL_PM_FULL_SLEEP) {
        ...
        if (!stopped) {
            ...
            clear_regs = ar9300_clear_lean_channel_change_regs(ah);
...
#undef REG_WRITE
    ...
    OS_DELAY(100);
    OS_REG_WRITE(ah, AR_PHY_65NM_CH0_RXTX3 , (OS_REG_READ(ah, AR_PHY_65NM_CH0_RXTX3 ) & ~(1 << 19)));
    if (!AR_SREV_HORNET(ah) && !AR_SREV_POSEIDON(ah) && !AR_SREV_APHRODITE(ah)) {
        OS_REG_WRITE(ah, AR_PHY_65NM_CH1_RXTX3 , (OS_REG_READ(ah, AR_PHY_65NM_CH1_RXTX3 ) & ~(1 << 19)));
        //TODO :If any new chip 2x2 introduced,please add condition here as follows.
        if (!AR_SREV_WASP(ah) && !AR_SREV_JUPITER(ah)) {
            OS_REG_WRITE(ah, AR_PHY_65NM_CH2_RXTX3 , (OS_REG_READ(ah, AR_PHY_65NM_CH2_RXTX3 ) & ~(1 << 19)));
        }
    }
#endif    
