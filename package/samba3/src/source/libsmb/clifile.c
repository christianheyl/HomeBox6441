		 ...
		 uint8 SecuityFlags)
{
	...
	//Simon, dumb data to see what happen
	DEBUG(1,("dump outgoing data\n"));
	dump_data(2, cli->outbuf, smb_len(cli->outbuf) + 4+32);
	cli_send_smb(cli);
	
	
	if (!cli_receive_smb(cli)) {
		DEBUG(1,("cli_receive_smb failed\n"));
		return -1;
	}

	DEBUG(1,("dump incoming data\n"));
	show_msg2(cli->inbuf);
