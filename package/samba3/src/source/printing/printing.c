...
//Simon@2013/04/12, Skip writing on disk full.
#ifdef ARCADYAN_PRINTING_SKIP_WRITE_ON_DISK_FULL
	if(return_code < 0){
		DEBUG(0,("_spoolss_writeprinter: print_job_write fail, skip writing to disk.....\n"));
		return_code = size;
	}
#endif
...
//Simon@2014/05/19, TESTING
	...
	if (lp_minprintspace(snum)) {
		...
		if (sys_fsusage(path, &dspace, &dsize) == 0 &&
			...
			DEBUG(3, ("minimal print space = %d\n", lp_minprintspace(snum)));
			DEBUG(3, ("system free space = %d\n", dspace));
