		...
		const struct api_struct *api_rpc_cmds, int n_cmds)
{
	...
	DEBUG(2,("api_rpcTNP: %s op 0x%x - ", rpc_name, p->hdr_req.opnum));
	...
	for (fn_num = 0; fn_num < n_cmds; fn_num++) {
		if (api_rpc_cmds[fn_num].opnum == p->hdr_req.opnum && api_rpc_cmds[fn_num].fn != NULL) {
			DEBUG(2,("api_rpcTNP: rpc command: %s\n", api_rpc_cmds[fn_num].name));
			break;
		}
	}
	...
	if (p->bad_handle_fault_state) {
		DEBUG(2,("api_rpcTNP: bad handle fault return.\n"));
	...
	DEBUG(2,("api_rpcTNP: called %s successfully\n\n\n", rpc_name));
	...
	if ((DEBUGLEVEL >= 10) && 
		...
		DEBUG(2, ("api_rpcTNP: rpc input buffer underflow (parse error?)\n"));
		if (data) {
