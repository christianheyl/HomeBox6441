...
//Simon@2013/04/12, Do self termination on disk full.
#ifdef ARCADYAN_PRINTING_SELF_TERMINATION_ON_DISK_FULL
	if(r_u.status.w == WERR_NO_SPOOL_SPACE.w){
		DEBUG(0,("api_spoolss_writeprinter: Didk full, do self termination!!!\n"));
		r_u.status = _spoolss_enddocprinter(p, &q_u, &r_u);
	}
#endif		
