 ...
 * 		  --> unlock
 ******************************/
static BOOL pdb_unknown_user_event_check(const char *username)
{
	BOOL 			ret 						= True;
	FILE			*fp							= NULL;
	char    		*pstUserFilePath          	= NULL;
	struct timeval 	tv_new;
	time_t 			timep;
	struct tm		*tm_new;
	char            acLineBuf[128];
	int				nIndex;
	char 			szDataProf[4][128];
	char			*pstLogCmd					= NULL;

	/* temperaily */
	system("mkdir -p /tmp/blacklist/account");

	asprintf(&pstUserFilePath, "/tmp/blacklist/account/%s", username);	
	DEBUG(10, ( "pstUserFilePath = %s \n", pstUserFilePath));

	/* get current time */
	gettimeofday(&tv_new, NULL);
    time(&timep);
    tm_new = localtime(&timep);

	/* lock file, /tmp/blacklist/key */
	system("lock -s /tmp/blacklist/key");

	fp = fopen(pstUserFilePath, "rb");	
	if (fp)	/* file is existed */
	{
		/* 
 		 *	Index 0: block/unblock
 		 *	Index 1: timestamp - gettimeofday()
 		 *	Index 2: retry count
 		 *	Index 3: timestamp - gmtime() => tm_hour * 60 + tm_min 
 		*/
		while ((fgets(acLineBuf, sizeof(acLineBuf), fp) != NULL) || nIndex < 4)
		{
			DEBUG(10, ( "%d. line data = %s \n", nIndex, acLineBuf));
			strlcpy(szDataProf[nIndex], acLineBuf, 128);
			nIndex++;
		}
		close(fp);

		if (atoi(szDataProf[0]))	/* lock is enabled */
		{
			if (tv_new.tv_sec - atoi(szDataProf[1]) < 3600)	/* still lock in an hour */	
				ret = True;	
			else											/* lock time is exceed an hour, unlock */
			{
				fp = fopen(pstUserFilePath, "wb");
				if (fp)
				{
					DEBUG(5, ("unlock user, %s, for unknown user of F003\n", username));
					fprintf(fp, "%d\n", 0);										/* lock = 0 */
					fprintf(fp, "%d\n", tv_new.tv_sec);                         /* gettimeofday() */
					fprintf(fp, "%d\n", 1);										/* retry count = 1 */
					fprintf(fp, "%d\n", tm_new->tm_hour * 60 + tm_new->tm_min); /* gmtime(), hour * 60 + minute */
					...
					DEBUG(5, ("fail to open file, %s, for unknown user of F003\n", pstUserFilePath));
			}	
		}
		else						/* lock is disabled */
		{
			fp = fopen(pstUserFilePath, "wb");
			if (fp)
			{
				if (atoi(szDataProf[2]) == 5)		/* have retried over 5 time */
				{
					DEBUG(5, ("lock user, %s, for unknown user of F003\n", username));
					fprintf(fp, "%d\n", 1);             						/* lock = 1 */
					fprintf(fp, "%d\n", tv_new.tv_sec);                         /* gettimeofday() */
					fprintf(fp, "%d\n", atoi(szDataProf[2]) + 1);   			/* increase retry count */
					fprintf(fp, "%d\n", tm_new->tm_hour * 60 + tm_new->tm_min); /* gmtime(), hour * 60 + minute */				
				...
				{	
					fprintf(fp, "%d\n", 0);							/* lock = 0 */
					fprintf(fp, "%d\n", atoi(szDataProf[1]));		/* orginal time of gettimeofday*/
					fprintf(fp, "%d\n", atoi(szDataProf[2]) + 1);	/* retry count */
					fprintf(fp, "%d\n", atoi(szDataProf[3]));		/* orginal time of gmtime(), hour * 60 + minute  */
	...
	else	/* file isn't existed, create it and record */
	{
		fp = fopen(pstUserFilePath, "wb");
		if (fp)
		{
			...
			DEBUG(5, ( "fail to create file, %s, for unknown user of F003\n", pstUserFilePath));
	}

	/* unlock file, /tmp/blacklist/key */
    system("lock -u /tmp/blacklist/key");

	free(pstUserFilePath);
...
static struct smb_passwd *getsmbfilepwent(struct smbpasswd_privates *smbpasswd_state, FILE *fp)
{
	...
	#ifdef ARCADYAN_COMPARISON_OF_USERNAME_IS_SENSITIVE
	DEBUG(3, ("Enter function, \"%s\"\n", __func__));
	#endif
	...
	while (status && !feof(fp)) {
		...
		#ifdef ARCADYAN_COMPARISON_OF_USERNAME_IS_SENSITIVE
		DEBUG(3, ("getsmbfilepwent: got line |%s|\n", linebuf));
		#else
...
static BOOL mod_smbfilepwd_entry(struct smbpasswd_privates *smbpasswd_state, const struct smb_passwd* pwd)
{
	...
	while (status && !feof(fp)) {
...
#ifdef DEBUG_PASSWORD
		#ifdef ARCADYAN_COMPARISON_OF_USERNAME_IS_SENSITIVE
		DEBUG(3, ("mod_smbfilepwd_entry: got line |%s|\n", linebuf));
		#else
		...
		#ifdef ARCADYAN_COMPARISON_OF_USERNAME_IS_SENSITIVE
		DEBUG(3, ("smbpasswd_state->user_name is %s\n", smbpasswd_state->user_name));
		DEBUG(3, ("pwd->smb_name is %s\n", pwd->smb_name));

		if (strcsequal(user_name, smbpasswd_state->user_name))	/* sensitive case, and smbpasswd_state->user_name is the word including lowercase & uppercase char */ 
		{
			DEBUG(2, ("Found user, %s.\n", smbpasswd_state->user_name));
			found_entry = True;
			break;
		}
		#else
		if (strequal(user_name, pwd->smb_name)) {	/* in-sensitive case, pwd->smb_name is all-lowercase word */
			found_entry = True;
			break;
		}
		#endif
...
#ifdef DEBUG_PASSWORD
	#ifdef ARCADYAN_COMPARISON_OF_USERNAME_IS_SENSITIVE
	DEBUG(3,("mod_smbfilepwd_entry: "));
    dump_data(3, ascii_p16, wr_len);
	#else
				  ...
				  struct samu *sam_acct, const char *username)
{
	...
	#ifdef ARCADYAN_COMPARISON_OF_USERNAME_IS_SENSITIVE					/* comparison of username is sensitive */
	while ( ((smb_pw=getsmbfilepwent(smbpasswd_state, fp)) != NULL)&& (!strcsequal(smb_pw->smb_name, username)) )
	#else																/* comparison of username is in-sensitive */
	...
	#ifdef ARCADYAN_DT_724_SAMBA_EVENT_LOG
	{
		...
		{
			#if 1	/* if set to zero, send F003 for unknown user without any judgement */
			/* 
 			** True: 	lock
			** False: 	unlock
			*/
			if (pdb_unknown_user_event_check(username) == True)
				goto Error_End;
			#endif

			DEBUG(3,("User \"%s\", \"%s\" login failed with protocol as a result of \"Unknown user\"\n", username, client_addr()));
			...
			if (system(pstLogCmd) != 0)
        		DEBUG(0,( "falied to add Event Log, F003\n"));
    		else
        		DEBUG(0,( "successful to add Event Log, F003\n"));
        	...
        	if(strcmp(szMailAddressBuf, "empty") != 0)
        	{
            	...
            	/* send email */
            	utilSendMailByProfileID(id, szMailAddressBuf, "Speedport W 724V - E-Mail-Benachrichtigung", szF003ContentsOfEmail);
        	}
