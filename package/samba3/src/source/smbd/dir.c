...
static int iLinkTargetIsUnderUsbMountPoint(connection_struct *conn,struct smb_Dir *dirp)
{
	char buffer[PATH_MAX];
	
	memset(buffer,'\0', PATH_MAX);
	
	if ( SMB_VFS_READLINK(conn, dirp->dir_path, buffer, PATH_MAX) != -1 ) {
		if( (strncmp(buffer, "/tmp/media/",11)==0) && (buffer[11]!='\0')){
			DEBUG(3,("iLinkTargetIsUnderUsbMountPoint: buffer %s\n", buffer ));
			return 1;
		}
	}
	DEBUG(3,("iLinkTargetIsUnderUsbMountPoint: %s Blocked\n", buffer ));
	return -1;
}
#endif
/*******************************************************************
...
struct smb_Dir *OpenDir(connection_struct *conn, const char *name, const char *mask, uint32 attr)
{
	...
	DEBUG(3,("OpenDir: dirp->dir_path %s\n", dirp->dir_path ));
#ifdef DENY_SYMLINK_TARGET_OUT_OF_USB_MOUNT_POINT

	if (SMB_VFS_LSTAT(conn, dirp->dir_path, &sbuf) != -1) 
	{
		if (S_ISLNK(sbuf.st_mode)) {
			if(iLinkTargetIsUnderUsbMountPoint(conn, dirp)!=1) {
				goto fail;
			}else {
				goto next_step;
			}
