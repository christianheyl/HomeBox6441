					...
					enum file_close_type close_type)
{
	...
	/* arcadyan, add event log, F110, F111, for DT-724
	** One case for event log
	** --> i. delete file
	*/

	#ifdef ARCADYAN_DT_724_SAMBA_EVENT_LOG
	DEBUG(0,( "add Event Log, F110, F111\n"));
	DEBUG(10,( "conn->connectpath = %s\n", conn->connectpath));

	/* determine whether we need to add Event Log to umng System Log */
	checkDeviceCapacity(conn);	
