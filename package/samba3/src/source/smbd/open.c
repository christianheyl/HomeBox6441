...
/*************************************************
 ** arcadyan, check the Validity of Access Path
 ** --> 1. USB disk is valid
 **     2. Ramdisk is Invalid
 **        --> "/tmp/usb"
 **        --> "/tmp/usb/<Vendor-ID>-<Product ID>"
 ************************************************/
NTSTATUS check_access_path_validity(connection_struct *conn)
{
	NTSTATUS    status                          = NT_STATUS_OK; /* status of message */
    char        szSharedPathBuf[128]            = {'\0'};       /* for path of shared folder */
    char        *pstStr                         = NULL;         /* check path of shared folder if it's in Ramdisk */
    char        szCurrentDirectoryPathBuf[128]  = {'\0'};       /* for current path of directory */

    /* display requisite information for following judgement */
    DEBUG(10,("conn->dirpath = %s\n", conn->dirpath));
    DEBUGADD(10,("conn->connectpath = %s\n", conn->connectpath));
    DEBUGADD(10,("conn->origpath = %s\n", conn->origpath));
    DEBUGADD(10,("conn->user = %s\n", conn->user));

    /* check if root path of shared folder is in ramdisk */
    strlcpy(szSharedPathBuf, conn->connectpath, sizeof(szSharedPathBuf));
    DEBUG(10, ("szSharedPathBuf is %s\n", szSharedPathBuf));

	#if 0
    pstStr = szSharedPathBuf + strlen("/tmp/usb/");
    DEBUG(10, ("pstStr is %s\n", pstStr));
	#endif

	/* case 1, root = /tmp/usb */
	if (strcmp(conn->connectpath, "/tmp/usb") == 0)
	{
		/* check whether current path of directory is in root path of shared folder */
        strlcpy(szCurrentDirectoryPathBuf, conn->dirpath, sizeof(szCurrentDirectoryPathBuf));
        DEBUG(10, ("szCurrentDirectoryPathBuf is %s\n", szCurrentDirectoryPathBuf));

		/* which means in the root(1st level) path of shared folder, conn->dirpath = ./ */
        if (strcmp(szCurrentDirectoryPathBuf, "./") == 0)
        {
            DEBUG(10, ("Access of Ramdisk is denied\n"));
            DEBUGADD(10,("conn->connectpath = %s\n", conn->connectpath));
            DEBUGADD(10,("conn->dirpath = %s\n", conn->dirpath));
            status = NT_STATUS_ACCESS_DENIED;
        }
		/* which means in 2nd level path, conn->dirpath = <vendor-ID>-<product-ID> */
        else if (strchr(szCurrentDirectoryPathBuf, '/') == NULL)
		{
			DEBUG(10, ("Access of Ramdisk is denied\n"));
            DEBUGADD(10,("conn->connectpath = %s\n", conn->connectpath));
            DEBUGADD(10,("conn->dirpath = %s\n", conn->dirpath));
            ...
            DEBUG(10, ("Access of USB disk is permitted\n"));
	}
	/* case 2, root = /tmp/usb/<vendor-ID>-<product-ID>, if returning NULL, which means root path of shared folder is in ramdisk */
    else if (strchr(szSharedPathBuf + strlen("/tmp/usb/"), '/') == NULL)
    {
        /* check whether current path of directory is in root path of shared folder */
        strlcpy(szCurrentDirectoryPathBuf, conn->dirpath, sizeof(szCurrentDirectoryPathBuf));
        DEBUG(10, ("szCurrentDirectoryPathBuf is %s\n", szCurrentDirectoryPathBuf));

        /* if szCurrentDirectoryPathBuf is "./", which means in the root path of shared folder */		
        if (strcmp(szCurrentDirectoryPathBuf, "./") == 0)
        {
            DEBUG(10, ("Access of Ramdisk is denied\n"));
            DEBUGADD(10,("conn->connectpath = %s\n", conn->connectpath));
        ...
        DEBUG(10, ("path of shared folder is in partition of USB Device\n"));

    return status;
}

NTSTATUS check_validity_of_creating_file_or_directory(connection_struct *conn, const char *parent_dir, const char *path)
{
	NTSTATUS    status                          = NT_STATUS_OK; /* status of message */
	char        szConnectPathBuf[128]          	= {'\0'};      	/* for path of shared folder */
    char        szOpenPathBuf[128]          	= {'\0'};       /* for path of upload or create file */
    char*       pst1stLevelFolderName;                          /* for 1st level of folder name, Ex: Ext3 of path, "Ext3/folder/file.rar" */
    int         tid;                                            /* for API of mapi, mapi_ccfg_xxx() */
    char        acPartitionCount[4];                            /* for count of partition */
    int         nIndex;                                         /* for index of partition */
    char        *pstItemName                    = NULL;         /* for item name of ccfg */
    char        acPartitionPath[384];                           /* for device%d_mountpoint@storage_partition */
    char*       pstTmpStr;

	/* display requisite information for following judgement */
    DEBUG(10,("parent_dir = %s\n", parent_dir));
    DEBUGADD(10,("path = %s\n", path));
    DEBUGADD(10,("conn->dirpath = %s\n", conn->dirpath));
    DEBUGADD(10,("conn->connectpath = %s\n", conn->connectpath));
    DEBUGADD(10,("conn->origpath = %s\n", conn->origpath));
    ...
    strlcpy(szConnectPathBuf, conn->connectpath, sizeof(szConnectPathBuf));
    DEBUG(10, ("szConnectPathBuf is %s\n", szConnectPathBuf));

	#if 0
    /* case 1: open directory, so not need to check validity of path */
    if (S_ISDIR(psbuf->st_mode))
        DEBUG(3,("open directory, so not need to check validity of path\n"));
	#endif
    /* case 2: root = /tmp/usb/<vendor-ID>-<product-ID>, if returning NULL, which means root path of shared folder is in ramdisk */
    if (strchr(szConnectPathBuf + strlen("/tmp/usb/"), '/') == NULL)
    {
        /* case 2-1, location of uploading or creating file in "/tmp/usb/<vendor-ID>-<product-ID>/." */
        if (((strcmp(parent_dir, ".") == 0) && (strchr(path, '/') == NULL)))
            status = NT_STATUS_ACCESS_DENIED;
		#if 0	/* no need for this case */
        /* case 2-2, location of uploading or creating file in "/tmp/sub/<vendor-ID>-<product-ID>/<label or non-lebel>/..." */
        else
        {
            strlcpy(szOpenPathBuf, path, sizeof(szOpenPathBuf));
            DEBUG(10, ("szOpenPathBuf is %s\n", szOpenPathBuf));

            pst1stLevelFolderName = strtok(szOpenPathBuf, "/");
            DEBUG(10, ("pst1stLevelFolderName is %s\n", pst1stLevelFolderName));
            ...
            /* get info from ccfg, storage_device_count@storage_partition */
            mapi_ccfg_get_str(tid, "storage_partition", "storage_device_count", acPartitionCount, sizeof(acPartitionCount), "0");
            DEBUG(10, ("acPartitionCount is %s\n", acPartitionCount));

            /* if count of partition is greater than 0, start to prepare check validity of path */
            if (atoi(acPartitionCount) > 0)
            {
                for (nIndex = 0; nIndex < atoi(acPartitionCount); nIndex++)
                {
                    /* Ex: acPartitionPath = /tmp/usb/Sony-StorageMedia/Ext3 */
                    asprintf(&pstItemName, "device%d_mountpoint", nIndex);
                    mapi_ccfg_get_str(tid, "storage_partition", pstItemName, acPartitionPath, sizeof(acPartitionPath), "0");
                    DEBUG(10, ("acPartitionPath is %s\n", acPartitionPath));
                    if (acPartitionPath[0] != '0')
                    {
                        DEBUG(10, ("xxxxxxxxxxx\n"));
                        pstTmpStr = strchr(acPartitionPath + strlen("/tmp/usb/"), '/');
                        if (pstTmpStr != NULL)
                        {
                            DEBUG(10, ("pstTmpStr(lebel) is %s\n", pstTmpStr));
                            /* 1st level folder of path is the same as certain partition */
                            if (strcmp(pst1stLevelFolderName, pstTmpStr + 1) == 0)
                            {
                                goto FOUND_END;
                            }
                        }
                ...
                /* after comparing with each partition of device, do not match any of these */
                status = NT_STATUS_ACCESS_DENIED;
            }
			    ...
			    files_struct **result)
{
	...
	switch( create_disposition ) {
		...
		case FILE_CREATE:
			...
			/*
 			** arcadyan, new feature
 			** check if open location of file is ramdisk of system.
 			** If yes, deny this request
 	      	*/
    		#ifdef ARCADYAN_RAMDISK_ACCESS_TO_BE_DENIED
    		status = check_validity_of_creating_file_or_directory(conn, parent_dir, fname);

    		if (!NT_STATUS_IS_OK(status))
    		{
        		DEBUG(3,("Create file in invalid access path\n"));
				...
				SMB_STRUCT_STAT *psbuf)
{
 	...
 	*/
    #ifdef ARCADYAN_RAMDISK_ACCESS_TO_BE_DENIED
    status = check_validity_of_creating_file_or_directory(conn, parent_dir, name);

    if (!NT_STATUS_IS_OK(status))
    {
        DEBUG(3,("Create folder in invalid access path\n"));
        DEBUGADD(3,("status is %s\n", nt_errstr(status)));
        return status;
    }
    #endif
