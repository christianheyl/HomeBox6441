...
#ifdef ARC_PW_PRIVILEGE
    if ((line = my_strtok2(NULL, *PW_LINE_SEP)) == NULL) {   /* connect type */
        return 0;
    }
    pwinfo->nConnectPrivilege = atoi(line);	
#endif	// ARC_PW_PRIVILEGE
    
#ifdef ARC_ACCOUNT_READ_ONLY_SUPPORT
    if ((line = my_strtok2(NULL, *PW_LINE_SEP)) == NULL) {   /* Read-ONLY*/
        return 0;
    }
    pwinfo->nAccountReadOnly = atoi(line);		
#endif // ARC_ACCOUNT_READ_ONLY_SUPPORT
...
static int add_new_pw_line(FILE * const fp2, const PWInfo * const pwinfo)
{
	...
	if(pwinfo->nConnectPrivilege >=0){
        fprintf(fp2, "%d", pwinfo->nConnectPrivilege);	 /* Connection Type */
	}
#endif // ARC_PW_PRIVILEGE	
	...
	if(pwinfo->nAccountReadOnly >=0){
        fprintf(fp2, "%d", pwinfo->nAccountReadOnly);	 /* Read-only */
	}
#endif // ARC_ACCOUNT_READ_ONLY_SUPPORT
    if (fprintf(fp2, "\n") < 0) {
        return -1;
...
// for root account, kilin, 2014.0724	
	...
	//================================================
	//	kilin 2014.08.04
	//	enhance option to get password by cli
	//================================================	
	if(pwinfo.pwd == NULL){	
...
# endif
    ...
    while ((fodder =
#if defined(ARC_PW_CLI) && defined(ARC_PW_PRIVILEGE) && defined(ARC_ACCOUNT_READ_ONLY_SUPPORT)
            getopt(argc, argv, 
                   "A:c:d:D:f:F:g:hi:I:mn:N:OP:q:Q:r:R:t:T:u:y:z:")) != -1) {
#elif defined(ARC_PW_CLI) && defined(ARC_PW_PRIVILEGE)
		// option P: A:
            getopt(argc, argv, 
                   "A:c:d:D:f:F:g:hi:I:mn:N:P:q:Q:r:R:t:T:u:y:z:")) != -1) {
#elif defined(ARC_PW_CLI) && defined(ARC_ACCOUNT_READ_ONLY_SUPPORT)
		// option P: O
            getopt(argc, argv, 
                   "c:d:D:f:F:g:hi:I:mn:N:P:q:Q:r:R:t:T:u:y:z:")) != -1) {		
#elif defined(ARC_PW_PRIVILEGE) && defined(ARC_ACCOUNT_READ_ONLY_SUPPORT)
		// option A: O
            getopt(argc, argv, 
                   "A:c:d:D:f:F:g:hi:I:mn:N:Oq:Q:r:R:t:T:u:y:z:")) != -1) {		
#elif	ARC_PW_PRIVILEGE
			// option A:
            getopt(argc, argv, 
                   "A:c:d:D:f:F:g:hi:I:mn:N:q:Q:r:R:t:T:u:y:z:")) != -1) {
#elif	ARC_PW_CLI
			// option P:
            getopt(argc, argv, 
                   "c:d:D:f:F:g:hi:I:mn:N:P:q:Q:r:R:t:T:u:y:z:")) != -1) {
#elif	ARC_ACCOUNT_READ_ONLY_SUPPORT
			// option O
            getopt(argc, argv, 
                   "c:d:D:f:F:g:hi:I:mn:ON:q:Q:r:R:t:T:u:y:z:")) != -1) {
#else
            getopt(argc, argv, 
		...
		case 'A':{
			if (*optarg == 0) {
				...
				pwinfo.nConnectPrivilege = atoi(optarg);
            }			
            break;		
		}		
#endif // ARC_PW_PRIVILEGE
