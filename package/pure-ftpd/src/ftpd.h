...
#define arc_dbg_printf(fmt, args...) do { \
        ...
        if (fp) { \
			...
			sprintf(pszBuf, "echo %s >/dev/console", fmt); \
			sprintf(pszCmd, pszBuf, ## args); \
			system(pszCmd); \
			free(pszBuf); \
			free(pszCmd); \
		} \
...
#else
#define arc_dbg_printf(fmt, args...)
#endif // ARC_DEBUG

#ifdef ARC_RAM_READ_ONLY
//#ifdef STORAGE_MOUNT_ROOT_PATH
//#define FTP_USER_HOME	STORAGE_MOUNT_NAME_REVERSAL_PATH
//#else
#define FTP_USER_HOME	"/tmp/usbMountNameReversal/" 
//#endif
#endif // ARC_RAM_READ_ONLY

#ifdef ARC_OPEN_ACCESS_MODE
//#ifdef STORAGE_MOUNT_ROOT_PATH
//#define FTP_OPEN_ACCESS_HOME_DB	STORAGE_MOUNT_ROOT_PATH"/./" 
//#else
#define FTP_OPEN_ACCESS_HOME_DB	"/tmp/media/./"
//#endif
