...
#define IPV6_PREFIX_LEN_DEFAULT	64		//default ipv6 prefix length
#endif //ARC_IPV6_SUBNET

#ifdef ARC_IPV6_SUBNET
/**
*	purpose: keep prefix bits only, pad others with 0
*	input: addr - (struct in_addr *) or (struct in6_addr *)
*	input: len - prefix length (bits)
*	output: addr
*/
int ipm_inet_pfx(int family, void *addr, int plen)
{
	unsigned char *pA = (unsigned char *)addr;	// endian independent
	int i;
	unsigned char mask;
	int tlen;
	...
	if (family != AF_INET && family != AF_INET6)
		return -1;
	tlen = (family == AF_INET)? 4: 16;
	for (i=0; i < tlen && plen >= 8; i++, plen -=8) {};
...
/**
*	1:the same subnet
*	0:NOT the same subnet
*/
int CheckSubNetV6(const char *szServerIp, const struct sockaddr_storage * const peer){
	int nRtn = 0;
	arc_dbg_printf("CheckSubNetV6 server ip is %s", szServerIp);
	...
	STORAGE_FAMILY(svaddr) = AF_INET6;
	
	//if(inet_pton(AF_INET6, szServerIp, &svaddr.sin6_addr) <= 0){
	if(inet_pton(AF_INET6, szServerIp, &(STORAGE_SIN_ADDR6(svaddr))) <= 0){
		arc_dbg_printf("get server ip error in CheckSubNetV6");
	}else{
		arc_dbg_printf("get server ip Success in CheckSubNetV6");
	}
	
	struct sockaddr_storage peaddr = *peer;
	
	//get ipv6 prefix length
	int nPfxLen = IPV6_PREFIX_LEN_DEFAULT;
	arc_dbg_printf("V6 prefix len is %d", nPfxLen);

	ipm_inet_pfx(AF_INET6, &(STORAGE_SIN_ADDR6(svaddr)), nPfxLen);
	ipm_inet_pfx(AF_INET6, &(STORAGE_SIN_ADDR6(peaddr)), nPfxLen);
	if(memcmp(&(STORAGE_SIN_ADDR6(svaddr)), &(STORAGE_SIN_ADDR6(peaddr)), (nPfxLen/8)) == 0){
		//IPv6 server ip and client ipe are at the same sub-net
		arc_dbg_printf("IPv6 server ip and client ipe are at the same sub-net");
		nRtn = 1;
	}else{
		//IPv6 server ip and client ipe are NOT at the same sub-net
		arc_dbg_printf("IPv6 server ip and client ipe are NOT at the same sub-net");
		nRtn = 0;
	}	
	return nRtn;
}
...
#ifdef ARC_RAM_READ_ONLY	
void SetHomeDIR(char *szDest, char *szSrc){
	int nDirLen = strlen(szSrc);
	
	if(strstr(szSrc, "/./") != NULL){	
		strncpy(szDest, szSrc, nDirLen -3);   
		szDest[nDirLen - 3] = '\0';
	}else{
		strncpy(szDest, szSrc, nDirLen - 1);   
		szDest[nDirLen - 1] = '\0';	
	}
}

int CheckPartition(void){
	int nRtn = 0;
	char szFullDir[MAXPATHLEN]={'\0'};		//full path

	snprintf(szFullDir, MAXPATHLEN,"%s%s", szHomeDir, wd);
	szFullDir[MAXPATHLEN - 1] = '\0';
	arc_dbg_printf("XXXXXXXXXXXXXx %s, %s", szFullDir, FTP_USER_HOME);
	if(strncmp(szFullDir, FTP_USER_HOME, strlen(szFullDir)) == 0){
		nRtn = 0;
	}else{
		nRtn =1;
	}
...
/**
* 0: the upload position is not a real disk
* 1: the upload position is a real disk
*/
int isRealDisk(void){
	int nRtn = 0;
	if(CheckPartition() == 1){
...
#ifdef ARC_PW_PRIVILEGE
		auth_scan->auth->check(&result, account, password, sa, peer, nTlsCnt);
#else		
...
# endif
...
#ifdef ARC_PW_PRIVILEGE
	authresult = pw_check(account, password, &ctrlconn, &peer, nTlsCnt);
#else
	...
	if(authresult.nReadOnly){
		arc_dbg_printf("set read only");
		nIsReadOnly =1;
	}
...
#ifdef	ARC_RAM_READ_ONLY
	//set Home Dir
	SetHomeDIR(szHomeDir, authresult.dir);	
#endif // ARC_RAM_READ_ONLY
...
#ifdef DENY_SYMLINK_TARGET_OUT_OF_USB_MOUNT_POINT
#define USB_MOUNT_POINT "/ramdisk/tmp/usbMountNameReversal"
static int doIsolatedFlashArea(const char *name)
{
	...
	if ((resolved_path = malloc(sizeof_resolved_path)) == NULL) {
        logfile(LOG_ERR, "realpath() failure : [%s] => [%s]", name, strerror(errno));            
		goto out;
    }
    ...
    logfile(LOG_DEBUG, "resolved_path: %s; USB_MOUNT_POINT: %s", resolved_path, USB_MOUNT_POINT);

	if(strncmp(resolved_path, USB_MOUNT_POINT, strlen(USB_MOUNT_POINT))==0
		|| strncmp(resolved_path, "/ramdisk/tmp/media", strlen("/ramdisk/tmp/media"))==0) {
		//&& resolved_path[strlen(USB_MOUNT_POINT)]!='\0') {
		iRet = 0;
	}

out:
	if(resolved_path!=NULL) free(resolved_path);
...
void docwd(const char *dir)
{
	...
	#ifdef DENY_SYMLINK_TARGET_OUT_OF_USB_MOUNT_POINT
    logfile(LOG_DEBUG, "where %s", where);
	if( doIsolatedFlashArea(where) != 0) {
        logfile(LOG_DEBUG, "MSG_SANITY_FILE_FAILURE");
        addreply(553, MSG_SANITY_FILE_FAILURE, where);
        return;
	}
	#endif
...
#ifdef ARC_ACCOUNT_READ_ONLY_SUPPORT
	if(nIsReadOnly){
		addreply_noformat(553, MSG_ANON_CANT_DELETE);
		return;
	}
#endif // ARC_ACCOUNT_READ_ONLY_SUPPORT
...
#endif
#ifdef ARC_RAM_READ_ONLY
	//check the upload position, it must be real disk
	if(isRealDisk() != 1){
		arc_dbg_printf("you should do mkdir in real disk");
		addreply_noformat(553, MSG_CREATE_FAILURE);
		goto end;
	}
#endif // ARC_RAM_READ_ONLY
...
#ifdef ARC_ACCOUNT_READ_ONLY_SUPPORT
	if(nIsReadOnly){
		arc_dbg_printf("==========read only ==========");
		addreply_noformat(553, MSG_ANON_CANT_MKD);
		goto end;
	}
#endif // ARC_ACCOUNT_READ_ONLY_SUPPORT    
...
void dostor(char *name, const int append, const int autorename)
{
    ...
    logfile(LOG_DEBUG, "%s: name %s", __FUNCTION__,name);
    
#ifdef ARC_RAM_READ_ONLY
	//check the upload position, it must be real disk!
	if(isRealDisk() != 1){
		arc_dbg_printf("you should do store in real disk!!!!!!!!!!!");
		addreply_noformat(553, MSG_CREATE_FAILURE);
		goto end;
	}
	arc_dbg_printf("I am here.!!!!!!!");
#endif // ARC_RAM_READ_ONLY
...
void dornfr(char *name)
{
	...
	//check the upload position, it must be real disk!
	if(isRealDisk() != 1){
		arc_dbg_printf("you should do nfr in real disk");
		addreply_noformat(550, MSG_RENAME_ABORT);
		return;
	}
#endif	// ARC_RAM_READ_ONLY
