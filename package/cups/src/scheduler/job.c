...
#define CUPS_DBG(fmt, args...) do { \
    FILE *fp = fopen("/dev/console", "w"); \
    if (fp) { \
        fprintf(fp, "<%30s:%5d>: ", __func__, __LINE__); \
        fprintf(fp, fmt , ## args); \
        fclose(fp); \
    } \
} while (0)
...
#error XXX
  ...
  switch (strlen(attr->values[0].string.text))
  {
	...
	*/
        ...
        snprintf(lang, sizeof(lang), "LANG=%s.UTF-8", attr->values[0].string.text);
        break;
  ...
  for (i = 0; i < envc; i ++){
    if (!strncmp(envp[i], "AUTH_", 5)){
      cupsdLogJob(job, CUPSD_LOG_DEBUG, "envp[%d]=\"AUTH_%c****\"", i, envp[i][5]);
    }
	  ...
	  CUPS_DBG("envp[%d]=\"DEVICE_URI=%s\"\n", i, job->printer->sanitized_device_uri);
      cupsdLogJob(job, CUPSD_LOG_DEBUG, "envp[%d]=\"DEVICE_URI=%s\"", i, job->printer->sanitized_device_uri);
    }
  }
  ...
  */
  ...
  {
    if (filter->filter[0] != '/')
      snprintf(command, sizeof(command), "%s/filter/%s", ServerBin, filter->filter);
    else
    ...
    if (i < (cupsArrayCount(filters) - 1))
    {
      ...
      {
        abort_message = "Stopping job because the scheduler could not create the filter pipes.";
    ...
    else
    {
      if (job->current_file == 1)
      {
        if (strncmp(job->printer->device_uri, "file:", 5) != 0)
        {
          ...
          {
            abort_message = "Stopping job because the scheduler could not create the backend pipes.";
    ...
    if (pid == 0)
    {
      ...
      abort_message = "Stopping job because the scheduler could not execute a filter.";
  ...
  */
  ...
  if (strncmp(job->printer->device_uri, "file:", 5) != 0)
  {
	...
	backroot = 0;
      ...
      if (pid == 0)
      {
        CUPS_DBG("could not execute backend, goto abort\n");
        abort_message = "Stopping job because the sheduler could not execute the backend.";

        goto abort_job;
      }
      else
      {
      CUPS_DBG("start backend %s (PID %d)\n", command, pid);
	cupsdLogJob(job, CUPSD_LOG_INFO, "Started backend %s (PID %d)", command, pid);
      }
    }
