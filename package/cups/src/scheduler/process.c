    ...
    int         *pid)			/* O - Process ID */
{
  ...
  if (stat(command, &commandinfo))
  {
    ...
    *pid = 0;
    ...
    cupsdLogMessage(CUPSD_LOG_ERROR, "Unable to execute %s: %s", command, strerror(errno));


	CUPS_DBG("cupsdStartProcess(command=\"%s\", argv=%p, envp=%p, infd=%d, outfd=%d, errfd=%d, backfd=%d, sidefd=%d, root=%d, profile=%p, job=%p(%d), pid=%p) = %d\n", command, argv, envp, infd, outfd, errfd, backfd, sidefd,root, profile, job, job ? job->id : 0, pid, *pid);

	CUPS_DBG("Unable to execute %s: %s\n", command, strerror(errno));
           ...
           (!RunUser && commandinfo.st_uid))
  {
    *pid = 0;
	...
	CUPS_DBG("insecure file permissions\n");
           ...
           !(commandinfo.st_mode & S_IXOTH))
  {
    *pid = 0;
	...
	CUPS_DBG("no execute permissions\n");
...
#error XXX
  ...
  */
  ...
  {
    CUPS_DBG("Parent process continue...\n");
    if (!process_array)
