	   ...
	   size_t     length)		/* I - Number of bytes to write */
{
  ...
  */
  ...
  if (length > 0)
  {
    if (http->wused && (length + http->wused) > sizeof(http->wbuffer))
    {
	  ...
	  //CUPS_DBG("2httpWrite2: Flushing buffer (wused=%d, length=CUPS_LLFMT)\n", http->wused, CUPS_LLCAST length);
        ...
        length < sizeof(http->wbuffer))
    {
	 ...
	 //CUPS_DBG("2httpWrite2: Copying " CUPS_LLFMT " bytes to wbuffer...\n",CUPS_LLCAST length);
    ...
    else
    {
	 ...
	 //CUPS_DBG("2httpWrite2: Writing CUPS_LLFMT bytes to socket...\n",CUPS_LLCAST length);
  ...
  */
//CUPS_DBG("handle end of request\n");
      ...
      (http->data_encoding == HTTP_ENCODE_LENGTH && http->data_remaining == 0))
  {
    ...
    */
    ...
    //CUPS_DBG("2httpWrite: changing states...\n");
