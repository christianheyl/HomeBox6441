    ...
    size_t     length)			/* I - Number of bytes to write */
{
 ...
 //CUPS_DBG("cupsWriteRequestData(http=%p, buffer=%p, length=" CUPS_LLFMT ")\n", http, buffer, CUPS_LLCAST length);
  ...
  */
  ...
  wused = http->wused;
   ...
   //CUPS_DBG("do http write \n");
  if (httpWrite2(http, buffer, length) < 0){
  	//CUPS_DBG("return  http write error\n");
    return (HTTP_ERROR);
  }
  ...
  */
  ...
  {
    CUPS_DBG("check for response data... \n");
   /*
