    ...
    cups_option_t *options)		/* I - Options */
{
  ...
  CUPS_DBG("cupsPrintFiles(name=\"%s\", num_files=%d, "
                "files=%p, title=\"%s\", num_options=%d, options=%p)\n",
                name, num_files, files, title, num_options, options);
    ...
    cups_option_t *options)		/* I - Options */
{
  ...
  CUPS_DBG("cupsPrintFiles2(http=%p, name=\"%s\", num_files=%d, "
                "files=%p, title=\"%s\", num_options=%d, options=%p)\n",
                http, name, num_files, files, title, num_options, options);
  ...
  */
  ...
  for (i = 0; i < num_files; i ++)
  {
    ...
    */
    ...
    status = cupsStartDocument(http, name, job_id, docname, format, i == (num_files - 1));

    while (status == HTTP_CONTINUE && (bytes = cupsFileRead(fp, buffer, sizeof(buffer))) > 0){
    ...
    if (status != HTTP_CONTINUE || cupsFinishDocument(http, name) != IPP_OK)
    {
      ...
      */
      CUPS_DBG("Unable to queue\n");
      goto cancel_job;
    }
  }

  CUPS_DBG("return job_id %d", job_id);
  return (job_id);
