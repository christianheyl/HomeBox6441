...
print_disk_part_info ()
{
	...
	while read infoline; do
	  ...
	  info_mounted_on="/mnt/usb/`echo "$mountd_state"|grep "$2\.name$info_dev_no"|cut -d= -f2`"
	  info_fs="`echo "$mountd_state"|grep "$2\.fs$info_dev_no"|cut -d= -f2`"
...
print_disk_info ()
{
  ...
  if [ $count_disk_list -gt 0 ]; then
	...
	disk_rev=`echo "$mountd_state"|grep "$dsk\.rev"|cut -d= -f2`
        ...
        disk_nod=`echo "$mountd_state"|grep "$dsk\.disc"|cut -d= -f2`
...
disc_umount ()
{
	...
	for mnt_dir in $info_mounted_dirs; do
		info_update=1
		umount "/mnt/usb/$mnt_dir" >/dev/null 2>/dev/null
		if [ $? -ne 0 ]; then
			ls "/mnt/usb/$mnt_dir/" >/dev/null 2>/dev/null
			if [ $? -ne 0 ]; then
				umnt_success="$umnt_success, $mnt_dir"
			else umnt_busy="$umnt_busy, $mnt_dir"; fi
		else
			rm -f "/mnt/usb/$mnt_dir" 2>/dev/null;
			umnt_success="$umnt_success, $mnt_dir";
		fi
	done
