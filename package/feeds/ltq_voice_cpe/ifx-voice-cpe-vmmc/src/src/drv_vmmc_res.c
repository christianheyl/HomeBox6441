...
IFX_void_t VMMC_RES_StructuresInit (VMMC_DEVICE *pDev)
{
   ...
   for (i=0; i < pDev->caps.nHDLC; i++)
   {
	  ...
	  pDev->pResHdlc[i].iTransmitting = 0;
                                 ...
                                 IFX_int32_t nLen)
{
...
#if 1  /* Charles */
   if (pResHdlc->iTransmitting != 0)
   {
      if (pResHdlc->iTransmitting > 2)
         printk("%s: iTransmitting %d\n", __func__, pResHdlc->iTransmitting);
#else
...
#if 1  /* Charles */
   {
      ...
      if (!fifoEmpty (pResHdlc->pIngressFifo))
      {
         ...
         else
         {
            IFX_TAPI_VoiceBufferChOwn ((IFX_void_t*)pBuf, IFX_TAPI_BUFFER_OWNER_HDLC_VMMC);
                       ...
                       pBuf = (IFX_uint8_t *) fifoPeek(pResHdlc->pIngressFifo, &nLen);
                       if (pBuf != NULL)
      ...
      if ((ret != VMMC_statusHdlcFifoOverflow) && (pBuf != NULL))
#endif
   {
      ...
      if (!VMMC_SUCCESS (ret))
      {
         printk("%s: VMMC_MPS_Write error %x\n", __func__, ret);
         if (IFX_SUCCESS != fifoPut (pResHdlc->pIngressFifo, pBuf, nLen))
         {
