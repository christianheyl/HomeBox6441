...
extern IFX_uint32_t dchan_evt_cnt[], dchan_evt_served_cnt[];
/**
 * Create MPS D-Channel event counter proc file output.
 * MUST BE USED FOR DEBUGGING PURPOSE ONLY.
 *
 * \param   buf      Buffer to write the string to
 * \return  len      Lenght of data in buffer
 */
static IFX_int32_t ifx_mps_dchan_evt_get_proc(IFX_char_t *buf)
{
   IFX_int32_t len, i;

   /* this value is written by voice FW CPU */
   len = sprintf (buf, "FW_DCHAN_EVT_CNT_TOTAL = %u\n",
                         *((IFX_uint32_t *)KSEG1ADDR(0xBF2001B4)));
   len += sprintf (buf+len, "----------------------------------------------\n");
   len += sprintf (buf+len, "CH\tSW_REQ\tSW_SERV\n");
   for (i=0; i<16; i++)
   {
      if (dchan_evt_cnt[i] != 0)
      {
         len += sprintf (buf+len, "%d\t%u\t%u\n",
             i, dchan_evt_cnt[i], dchan_evt_served_cnt[i]);
      }
   }
   len += sprintf (buf+len, "----------------------------------------------\n");
...
#endif /* < Linux 2.6.30 */
      ifx_mps_proc_file =
         create_proc_read_entry ("dchan_evt", S_IFREG | S_IRUGO, ifx_mps_proc_dir,
                                 ifx_mps_read_proc,
                                 (IFX_void_t *) ifx_mps_dchan_evt_get_proc);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,30))
      if (ifx_mps_proc_file != IFX_NULL)
         ifx_mps_proc_file->owner = THIS_MODULE;
#endif /* < Linux 2.6.30 */
...
ifx_mps_cleanup_module (void)
{
   ...
   remove_proc_entry ("dchan_evt", ifx_mps_proc_dir);
