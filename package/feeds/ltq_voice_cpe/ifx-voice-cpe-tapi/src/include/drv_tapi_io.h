...
#ifdef TAPI_VERSION3
   #define TAPI_VERSION_CODE TAPI_API_VERSION(3,2,3)
...
#ifdef TAPI_VERSION4
   #define TAPI_VERSION_CODE TAPI_API_VERSION(4,1,2)
...
/** Retrieves an RTCP XR report block.
   ...
   \remarks Not all block types of RFC3611 may be supported.
...
/**
   ...
   \remark: For the return parameter a memory block of sufficient size needs
   to be passed. This IOCTL cannot check the size and will always copy the
   full list it has into the given memory area. To be safe from a potential
...
/** This service checks whether a specific capability is supported.
   ...
   if (ioctl (fd, IFX_TAPI_CAP_CHECK, (IFX_uintptr_t) &param) != IFX_SUCCESS)
      return IFX_ERROR;
...
/** Bitrate for different codec types.
...
typedef struct
{
   ...
   IFX_uint32_t   J:1;
   /** TTL or Hop Limit flag (ToH):  This field is not supported and set to
       zero. */
   IFX_uint32_t   ToH:2;
   ...
   /** The minimum TTL or Hop Limit value of data packets in the sequence
       number range. This field is not supported and set to a value of 0. */
   ...
   /** The standard deviation of TTL or Hop Limit values of data packets in the
       sequence number range. This field is not supported and set to a value
       of 0. */
...
typedef struct
{
   ...
   /** The fraction of RTP data packets within burst periods since the
       ...
       This field is not supported and set to a value of 0. */
   ...
   /** The most recently calculated round trip time between RTP interfaces,
       expressed in milliseconds. This field is not supported and set to a
       value of 0.
       RFC 3611 recommends to use NTP which only the application can use. So
       the application shall calculate this value and fill this field. */
   ...
   /** The voice signal relative level is defined as the ratio of the signal
       ...
       in two's complement form.  This is measured only for packets containing
   ...
   /** Receiver configuration byte according to the definition in RFC3611.
       This field is not supported and set to a value of 0. The application
...
/** Union to aggregate all supported RTCP XR block type structure. */
typedef union
{
   /** Array with the requested block data. The number of bytes which are
       ...
       See RFC3611 for a reference of all block data. */
   ...
   /** Detailed definiton of the VoIP Metrics Report Bock. */
   IFX_TAPI_PKT_RTCP_XR_BLOCK_VOIP_METRICS_REPORT  VoIPMetricsReport;
} IFX_TAPI_PKT_RTCP_XR_BLOCK_t;

/** Structure for retrieving RTCP XR blocks. RTCP XR is specified in RFC3611. */
typedef struct
{
   /** \tapiv3 Not used with the TAPI for CPE products. \endtapiv3
       \tapiv4 Device index.                            \endtapiv4 */
...
typedef struct
{
   ...
   /** Encoder bitrate, valid only for certain encoder types. */
