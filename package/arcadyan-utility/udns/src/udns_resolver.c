...
static __inline void qlist_remove(struct dns_qlist *list, struct dns_query *q)
{
	if (q->dnsq_prev)
	{
		q->dnsq_prev->dnsq_next = q->dnsq_next;
	}
	else
	{
		list->head = q->dnsq_next;
	}

	if (q->dnsq_next)
	{
		q->dnsq_next->dnsq_prev = q->dnsq_prev;
	}
	else
	{
		list->tail = q->dnsq_prev;
	}
}
...
static __inline void
qlist_insert_after(struct dns_qlist *list, struct dns_query *q, struct dns_query *prev)
{
...
int dns_set_opts(struct dns_ctx *ctx, const char *opts)
{
	...
	for (;;)
	{
		...
		for (i = 0; ; ++i)
		{
			if (i >= sizeof(dns_opts) / sizeof(dns_opts[0]))
			{
				++err;
				break;
			}
...
int dns_open(struct dns_ctx *ctx, char *localip, int localport)
{
	int sock;
	unsigned i;
	int port;
	...
	for (i = 0; i < ctx->dnsc_nserv; ++i)
	{
		...
		{
			if (!sns->sin6.sin6_port)
			{
				sns->sin6.sin6_port = (unsigned short)port;
			}
...
#endif
		{
			...
			if (!sns->sin.sin_port)
			{
				sns->sin.sin_port = (unsigned short)port;
			}
		}
	}
              ...
              unsigned servi, time_t now)
{
...

	{
		...
		if (!(q->dnsq_flags & DNS_NORD))
		{
			p[DNS_H_F1] |= DNS_HF1_RD;
		}

		if (q->dnsq_flags & DNS_AAONLY)
		{
			p[DNS_H_F1] |= DNS_HF1_AA;
		}

		if (q->dnsq_flags & DNS_SET_CD)
		{
			p[DNS_H_F2] |= DNS_HF2_CD;
		}
	...
	printf("\n#[%s] %d, qlen=%d \n", __FUNCTION__, __LINE__, qlen);

	while ((len = sendto(ctx->dnsc_udpsock, (void *)ctx->dnsc_pbuf, qlen, 0,
...
again: /* receive the reply */
	...
	if (cnt > 0)
	{
		...
		if (n > 0 && FD_ISSET(ctx->dnsc_udpsock, &rfd))
		{
		}
		else
		{
