...
static void dns_set_srch_internal(struct dns_ctx *ctx, char *srch)
{
	...
	//for(srch = strtok_s(srch, space, &next_token); srch; srch = strtok_s(NULL, space, &next_token))
...
static int dns_initns_registry2(struct dns_ctx *ctx)    //http://msdn.microsoft.com/en-us/library/aa365968(VS.85).aspx
{
#if 0
	//char valBuf[1024];
	char *_ptr;
...
// the necessary size into the ulOutBufLen variable
	...
	if (dwRetVal = GetNetworkParams(pFixedInfo, &ulOutBufLen) == NO_ERROR)
	{
		...
		_num = sprintf_s(dns_server, _numBuf, "%s ", pFixedInfo->DnsServerList.IpAddress.String);
		_ptr = dns_server + _num;
		_numBuf -= _num;
		...
		while (pIPAddr)
		{
			...
			_num = sprintf_s(_ptr, _numBuf, "%s ", pIPAddr->IpAddress.String);
			_ptr = _ptr + _num;
			_numBuf -= _num;
...
int dns_init_server(struct dns_ctx *ctx)
{
	if (!ctx)
	{
		ctx = &dns_defctx;
...
int dns_init(struct dns_ctx *ctx, int do_open) {
  ...
  dns_set_serv_internal(ctx, dns_server);
...
struct dns_ctx *udns_init(struct dns_ctx *ctx, char *dnsserver)
{
	...
	dns_set_serv_internal(ctx, dnsserver);
