...
#include "ltq_tantos_reg.h"
#include "ltq_ar9_reg.h"
...
#define GSWIP_GET_BITS(x, msb, lsb)           (((x) & ((1 << ((msb) + 1)) - 1)) >> (lsb))
#define GSWIP_SET_BITS(x, msb, lsb, value)    (((x) & ~(((1 << ((msb) + 1)) - 1) ^ ((1 << (lsb)) - 1))) | (((value) & ((1 << (1 + (msb) - (lsb))) - 1)) << (lsb)))
#define IFX_ETHSW_DEV_MAX               2
#define SWITCH_API_MODULE_NAME          "IFX ETHSW SWITCH API"
#define SWITCH_API_DRIVER_VERSION       "1.2.6"
...
#elif ( defined(VR9)  || defined(AR10) || defined(HN1) )
...
/* PHY Link Status structure */
typedef struct ltq_phy_link_status {
	IFX_uint8_t	port_num;	/* Link status	*/
	int (*ltq_gphy_int_handler)(struct ltq_phy_link_status *, int status);
} ltq_phy_link_status_t;
