...
extern int32_t (*ppa_hook_directpath_send_spec_fn)(uint32_t rx_if_id, PPA_BUF *buf, int32_t len, uint32_t flags);
//--Add by Maxx_chen
#endif
...
*/
#ifdef NO_DOXY
extern int32_t (*ifx_ppa_hook_set_qos_rate)( uint32_t portid, uint32_t queueid, int32_t shaperid, uint32_t rate, uint32_t burst, uint32_t flag);
#else
...
*/
#ifdef NO_DOXY
extern int32_t (*ifx_ppa_hook_get_qos_rate)( uint32_t portid, uint32_t queueid,int32_t *shaperid,  uint32_t *rate, uint32_t *burst, uint32_t flag); 
#else
...
/*!
    ...
    \param[in] indexm imq index: 0 for PPA Directpath and 1 for future PPA indirect path
