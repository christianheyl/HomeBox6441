...
#define IFX_PPA_CAPWAP_NOT_ADDED                -1
#define IFX_PPA_CAPWAP_EXISTS                   0 
#define IFX_PPA_CAPWAP_ADDED                    1 
#endif
...
/*!
    \brief IFX_INDEX_OVERFLOW
*/
#define IFX_INDEX_OVERFLOW                      -100 
...
/*!
    \brief IFX_MAX_PORT_NUM
*/
#define IFX_MAX_PORT_NUM                            16   /*!< Maximum PPE FW Port number supported in PPA layer. */


/*!
    \brief IFX_MAX_QOS_QUEUE_NUM
*/
#define IFX_MAX_QOS_QUEUE_NUM                       16   /*!< Maximum PPE FW QoS Queue number supported in PPA layer for one port. */
...
/*!
    \brief IFX_PPA_MC_SESSION_VIOLATION
*/
#define IFX_PPA_MC_SESSION_VIOLATION            -2 /*!< PPA MC Session lookup failed. This can happen when you search IGMP V2 entry but hit one IGMPv3 entry with same \n
                                                            Multicast Group IP or vise versa*/
/*!
    \brief IFX_PPA_SESSION_FILTED
*/
#define IFX_PPA_SESSION_FILTED                 -1  /*!< Indicates session is  not qualified to be accelerated yet */

/*!
    \brief IFX_PPA_SESSION_NOT_FILTED
*/

#define IFX_PPA_SESSION_NOT_FILTED              0  /*!< Indicates session can be further processed */
...
#define SESSION_VALID_PPPOL2TP                  0x08000000
#endif
...
*/
typedef struct{  
    PPA_PORT_MIB_INFO mib_info[IFX_MAX_PORT_NUM];     /*!<  mib array: eth0, eth1, DSL, DSL(IPOA/PPPOA), ext1, ext2, ext3, ext4 */
...
typedef struct
{
    ...
    PPE_QOS_MIB_INFO  mib[IFX_MAX_QOS_QUEUE_NUM];   /*!<  Qos mib buffer */
    PPA_QOS_INTERVAL queue_internal[IFX_MAX_QOS_QUEUE_NUM];   /*!<  internal qos queue parameters */
    PPA_QOS_DESC_CFG_INTERNAL desc_cfg_interanl[IFX_MAX_QOS_QUEUE_NUM];/*!<  internal desc cfg parameters */
    uint32_t res;   /*!<  res flag for qos status succeed or not: possible value is IFX_SUCCESS OR IFX_FAILURE  */
...
#define ENUM_STRING(x)   #x
...
#if defined(MIB_MODE_ENABLE) && MIB_MODE_ENABLE
  int32_t ifx_ppa_set_mib_mode(uint8_t );
  int32_t ifx_ppa_get_mib_mode(uint8_t* );
#endif
  ...
  int32_t ifx_ppa_mc_group_get(IP_ADDR_C, IP_ADDR_C, PPA_MC_GROUP *, uint32_t);
  int32_t ifx_ppa_mc_entry_modify(IP_ADDR_C,IP_ADDR_C, PPA_MC_GROUP *, PPA_SESSION_EXTRA *, uint32_t);
  int32_t ifx_ppa_mc_entry_get(IP_ADDR_C, IP_ADDR_C, PPA_SESSION_EXTRA *, uint32_t);

#if defined(RTP_SAMPLING_ENABLE) && RTP_SAMPLING_ENABLE
  int32_t ifx_ppa_mc_entry_rtp_get(IP_ADDR_C, IP_ADDR_C, uint8_t*);
  int32_t ppa_mc_entry_rtp_set(PPA_MC_GROUP*);
#endif

#if defined(CAP_WAP_CONFIG) && CAP_WAP_CONFIG
   int32_t ifx_ppa_capwap_update(PPA_CMD_CAPWAP_INFO *);
   int32_t ifx_ppa_capwap_delete(PPA_CMD_CAPWAP_INFO *);
#endif
