...
{   // Try to return IFX_SUCCESS with ioctl, but make sure put real return value in internal variable. 
    if ( copy_from_user( &cmd_info->qos_status_info, (void *)arg, sizeof(cmd_info->qos_status_info)) != 0 )
        return IFX_FAILURE;

    if( ppa_update_qos_mib(&cmd_info->qos_status_info.qstat, 1, cmd_info->qos_status_info.flags ) != IFX_SUCCESS )
    {
        cmd_info->qos_status_info.qstat.res = IFX_FAILURE;
    }
    else 
        cmd_info->qos_status_info.qstat.res = IFX_SUCCESS;
...
int32_t ifx_ppa_get_qos_qnum( uint32_t portid, uint32_t flag)
{
    ...
    ppa_debug(DBG_ENABLE_MASK_QOS, "ifx_ppa_get_qos_qnum for portid:%d\n", count.portid );
    if( ifx_ppa_drv_get_qos_qnum( &count, 0) != IFX_SUCCESS ) 
    {
        ppa_debug(DBG_ENABLE_MASK_QOS, "ifx_ppa_drv_get_qos_qnum failed\n");
        count.num = 0;
    }
    else
    {
...
int32_t ifx_ppa_set_qos_rate( uint32_t portid, uint32_t queueid, int32_t shaperid, uint32_t rate, uint32_t burst, uint32_t flag )
#else
...
#if defined(MBR_CONFIG) && MBR_CONFIG
int32_t ifx_ppa_get_qos_rate( uint32_t portid, uint32_t queueid, int32_t *shaperid, uint32_t *rate, uint32_t *burst, uint32_t flag)
#else
...
int32_t ifx_ppa_set_qos_shaper( int32_t shaperid, uint32_t rate, uint32_t burst )
{
    PPE_QOS_RATE_SHAPING_CFG rate_cfg={0};
    ...
    //return ifx_ppa_drv_set_qos_rate( &rate_cfg, 0);
    return ifx_ppa_drv_set_qos_shaper( &rate_cfg, 0);
}

int32_t ifx_ppa_get_qos_shaper( int32_t *shaperid, uint32_t *rate, uint32_t *burst, uint32_t flag)
{
    PPE_QOS_RATE_SHAPING_CFG rate_cfg={0};
    int32_t res = IFX_FAILURE;
    ...
    //res = ifx_ppa_drv_get_qos_rate( &rate_cfg, 0);
    res = ifx_ppa_drv_get_qos_shaper( &rate_cfg, 0);
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    //res = ifx_ppa_set_qos_rate(cmd_info->qos_rate_info.portid, cmd_info->qos_rate_info.queueid, cmd_info->qos_rate_info.shaperid, cmd_info->qos_rate_info.rate, cmd_info->qos_rate_info.burst, cmd_info->qos_rate_info.flags);
    res = ifx_ppa_set_qos_shaper(cmd_info->qos_rate_info.shaperid, cmd_info->qos_rate_info.rate, cmd_info->qos_rate_info.burst);
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    //res = ifx_ppa_get_qos_rate(cmd_info->qos_rate_info.portid, cmd_info->qos_rate_info.queueid, &cmd_info->qos_rate_info.shaperid, &cmd_info->qos_rate_info.rate, &cmd_info->qos_rate_info.burst, cmd_info->qos_rate_info.flags);
    res = ifx_ppa_get_qos_shaper(cmd_info->qos_rate_info.shaperid, &cmd_info->qos_rate_info.rate, &cmd_info->qos_rate_info.burst, cmd_info->qos_rate_info.flags);
