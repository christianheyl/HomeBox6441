...
extern uint32_t ifx_ppa_is_init(void);
...
#define WRPFN_1(RT,FNAME,T1, P1, IDX)                   \
    ...
    {                                                   \
        ...
        RT (*pfn)(T1) = NULL;                           \
        ppa_rcu_read_lock();                            \
        if(ifx_ppa_is_init()&&g_expfn_table[IDX].used_flag && g_expfn_table[IDX].hook_flag ){ \
...
#ifdef CONFIG_QUALCOMM_AP_PPA
//++Add by Maxx_chen
extern int32_t ppa_directpath_send_spec_rpfn(uint32_t rx_if_id, PPA_BUF *skb, int32_t len, uint32_t flags);
//--Add by Maxx_chen
#endif
