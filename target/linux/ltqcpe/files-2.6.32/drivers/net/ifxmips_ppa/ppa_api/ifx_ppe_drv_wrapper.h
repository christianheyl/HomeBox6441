...
int ifx_ppa_drv_ppe_pwm_change(unsigned int arg, unsigned int flags);   //  arg - parameter, flags - 1: clock gating on/off, 2: power gating on/off
...
extern uint32_t ifx_ppa_drv_set_wan_mc_rtp( PPE_MC_INFO *entry);
extern uint32_t ifx_ppa_drv_get_mc_rtp_sampling_cnt( PPE_MC_INFO *entry);
#endif

#if defined(MIB_MODE_ENABLE) && MIB_MODE_ENABLE
extern uint32_t ifx_ppa_drv_set_mib_mode(PPE_MIB_MODE_ENABLE *cfg, uint32_t );
extern uint32_t ifx_ppa_drv_get_mib_mode(PPE_MIB_MODE_ENABLE *cfg);
#endif

#if defined(CAP_WAP_CONFIG) && CAP_WAP_CONFIG
extern uint32_t ifx_ppa_drv_add_capwap_entry(PPA_CMD_CAPWAP_INFO *entry , uint32_t flag);
extern uint32_t ifx_ppa_drv_delete_capwap_entry(PPA_CMD_CAPWAP_INFO *entry , uint32_t flag);
extern uint32_t ifx_ppa_drv_get_capwap_mib(PPA_CMD_CAPWAP_INFO *entry,uint32_t flag); 
#endif
...
extern uint32_t ifx_ppa_drv_add_l2tptunnel_entry(PPA_L2TP_INFO *entry, uint32_t flag);

extern uint32_t ifx_ppa_drv_del_l2tptunnel_entry(PPA_L2TP_INFO *entry, uint32_t flag);
#endif
...
extern uint32_t ifx_ppa_drv_get_mc_entry_bytes(PPE_MC_INFO *entry, uint32_t flag);
...
extern uint32_t ifx_ppa_drv_add_ipv6_entry(PPE_IPV6_INFO *entry, uint32_t flag);

extern uint32_t ifx_ppa_drv_del_ipv6_entry(PPE_IPV6_INFO *entry, uint32_t flag);
...
extern uint32_t ifx_ppa_drv_set_qos_shaper( PPE_QOS_RATE_SHAPING_CFG *cfg , uint32_t flag);
extern uint32_t ifx_ppa_drv_get_qos_shaper( PPE_QOS_RATE_SHAPING_CFG *cfg , uint32_t flag);
#endif
#endif
...
extern uint32_t ifx_ppa_drv_test_and_clear_mc_hit_stat(PPE_MC_INFO *entry, uint32_t flag);

extern uint32_t ifx_ppa_drv_test_and_clear_bridging_hit_stat(PPE_BR_MAC_INFO *entry, uint32_t flag);

extern uint32_t ifx_ppa_drv_set_value( PPA_CMD_VARIABLE_VALUE_INFO *v, uint32_t flag);
extern uint32_t ifx_ppa_drv_get_value( PPA_CMD_VARIABLE_VALUE_INFO *v, uint32_t flag);

extern uint32_t ifx_ppa_set_wan_itf( PPE_WANITF_CFG *cfg, uint32_t flag);

extern uint32_t ifx_ppa_drv_hal_init(uint32_t flag);
extern uint32_t ifx_ppa_drv_hal_exit(uint32_t flag);
extern uint32_t ifx_ppa_get_session_hash( PPE_SESSION_HASH *cfg, uint32_t flag);


extern int32_t ifx_ppa_hook_set_lan_seperate_flag( uint32_t flag);
extern int32_t ifx_ppa_hook_get_lan_seperate_flag( uint32_t flag);
extern uint32_t ifx_ppa_hook_set_wan_seperate_flag( uint32_t flag);
extern uint32_t ifx_ppa_hook_get_wan_seperate_flag( uint32_t flag);
