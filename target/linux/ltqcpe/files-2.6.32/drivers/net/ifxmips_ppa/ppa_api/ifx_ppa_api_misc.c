    ...
    case PPA_CMD_INIT:
        {
            ...
            {
                ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"ifx_ppa_init fail");
                goto EXIT_EIO;
            }
#if defined(MIB_MODE_ENABLE) && MIB_MODE_ENABLE
            if ( ifx_ppa_set_mib_mode(cmd_info->init_info.mib_mode) != IFX_SUCCESS )
                goto EXIT_EIO;
#endif
            else
                goto EXIT_ZERO;
    ...
    case PPA_CMD_SET_MIB_MODE:
        {
            ...
            if ( ifx_ppa_set_mib_mode(cmd_info->mib_mode_info.mib_mode) != IFX_SUCCESS )
                goto EXIT_EIO;
            else
                goto EXIT_ZERO;
    ...
    case PPA_CMD_GET_MIB_MODE:
        {
            ...
            if ( ifx_ppa_get_mib_mode(&cmd_info->mib_mode_info.mib_mode) != IFX_SUCCESS )
                goto EXIT_EIO;
    ...
    case PPA_CMD_MODIFY_MC_ENTRY:
        {
            ...
            if ( ifx_ppa_mc_entry_modify(cmd_info->mc_entry.mcast_addr,cmd_info->mc_entry.source_ip, NULL, &cmd_info->mc_entry.mc_extra, cmd_info->mc_entry.flags) != IFX_SUCCESS )
                goto EXIT_EIO;
            else
                goto EXIT_ZERO;
        }
    ...
    case PPA_CMD_GET_MC_ENTRY:
        {
            ...
            if ( ifx_ppa_mc_entry_get(cmd_info->mc_entry.mcast_addr,cmd_info->mc_entry.source_ip, &cmd_info->mc_entry.mc_extra, -1) != IFX_SUCCESS ) //-1: to get all flag related parameters
                goto EXIT_EIO;
    ...
    case PPA_CMD_SET_RTP:
        {
            ...
            if ( res != IFX_SUCCESS )
            {
                ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"ifx_ppa_mc_entry_rtp_set fail\n");
                goto EXIT_EIO;
            }
            else
                goto EXIT_ZERO;
    ...
    case PPA_CMD_ADD_VLAN_FILTER_CFG:
        {
            ...
            {
                ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"ifx_ppa_vlan_filter_add fail\n");
                goto EXIT_EIO;
            }
            else
                goto EXIT_ZERO;
    ...
    case PPA_CMD_ADD_MC:
        {
            ...
            if ( res != IFX_SUCCESS )
            {
                ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"ifx_ppa_mc_group_update fail\n");
                goto EXIT_EIO;
            }
            else
                goto EXIT_ZERO;
    ...
    case PPA_CMD_ADD_CAPWAP:
        {
            ...
            res = ifx_ppa_capwap_update(&cmd_info->capwap_add_info);

            if ( res != IFX_SUCCESS )
            {
                ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"ifx_ppa_capwap_group_update fail\n");
                goto EXIT_EIO;
            }
            else
                goto EXIT_ZERO;
   ...
   case PPA_CMD_DEL_CAPWAP:
        {
            ...
            res = ifx_ppa_capwap_delete(&cmd_info->capwap_add_info);

            if ( res != IFX_SUCCESS )
            {
                ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"ifx_ppa_capwap_delete fail\n");
                goto EXIT_EIO;
            }
            else
                goto EXIT_ZERO;
    ...
    case PPA_CMD_GET_CAPWAP_GROUPS:
        {
            ...
            {
               if( ppa_capwap_start_iteration(i, &pp_item) == IFX_SUCCESS )
               {
                  ...
                  if ( ifx_ppa_drv_get_capwap_mib(&ppa_capwap_entry,0 ) == IFX_SUCCESS )
                  {
            ...
            if ( ppa_mc_group_start_iteration(&pos, &pp_item) == IFX_SUCCESS )
            {
                ...
                do
                {
                    ...
                    if ( 1 /*pp_item->flags & SESSION_ADDED_IN_HW */ )
                    {
                        ...
                        {
                           if ( ppa_hw_get_mc_rtp_sampling_cnt( pp_item ) != IFX_SUCCESS )
                           { 
    ...
    case PPA_CMD_GET_LAN_WAN_SESSIONS:
        {
            ...
            uint32_t count=0, ret=IFX_SUCCESS;
           ...
           {    
              ...
              buff_len = NUM_ENTITY(tmp_session_buf);
                ...
                if( (ret == ppa_session_get_items_via_index(pos, &p_item_one_index,buff_len, &tmp_num, cmd_info->session_info.count_info.stamp_flag)) == IFX_INDEX_OVERFLOW ) //end of list
                {   
                    ret = IFX_SUCCESS;
                    break;
                }
    ...
    case PPA_CMD_GET_VERSION:
        {
            ...
            ifx_ppa_subsystem_id(&cmd_info->ver.ppa_subsys_ver.family, &cmd_info->ver.ppa_subsys_ver.type, &cmd_info->ver.ppa_subsys_ver.itf, &cmd_info->ver.ppa_subsys_ver.mode, &cmd_info->ver.ppa_subsys_ver.major, &cmd_info->ver.ppa_subsys_ver.mid, &cmd_info->ver.ppa_subsys_ver.minor, &cmd_info->ver.ppa_subsys_ver.tag);
             if( ifx_ppa_drv_hal_generic_hook )
            {
                ...
                ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"Why ifx_ppa_drv_hal_generic_hook is NULL\n");
...
static int proc_read_ver(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    ifx_ppa_subsystem_id(&ver.family, &ver.type, &ver.itf, &ver.mode, &ver.major, &ver.mid, &ver.minor, &ver.tag);
...
int32_t ppa_ioctl_modify_session(unsigned int cmd, unsigned long arg, PPA_CMD_DATA *cmd_info)
{
    ...
    if( cmd_info->session_extra_info.flags == 0 ) return IFX_SUCCESS; //no information need to update
    ...
    if ( ppa_session_start_iteration(&pos, &p_item) == IFX_SUCCESS )
    {
        do
        {
            ...
            /*below code copy from ifx_ppa_session_modify ---start*/
            ...
            {   //update parameters to PPE FW SESSION TABLE
                if ( ppa_hw_update_session_extra(p_item, cmd_info->session_extra_info.flags) != IFX_SUCCESS )
                {
                    //  update failed
                    ppa_hw_del_session(p_item);
                    p_item->flags &= ~SESSION_ADDED_IN_HW;                        
            ...
            /*below code copy from ifx_ppa_session_modify ---end*/
...
void ifx_ppa_reg_expfn(void)
{
    {
        //extern uint32_t ppa_enable_rpfn, ppa_get_status_rpfn;
        ppa_reg_export_fn(PPA_ENABLE_FN,        (uint32_t)ifx_ppa_enable, "ppa_enable", (uint32_t *)&ppa_hook_enable_fn,(uint32_t)ppa_enable_rpfn );
        ppa_reg_export_fn(PPA_GET_STATUS_FN,    (uint32_t)ifx_ppa_get_status, "ppa_get_status", (uint32_t *)&ppa_hook_get_status_fn, (uint32_t)ppa_get_status_rpfn);
    }

    {
        //extern uint32_t ppa_session_add_rpfn, ppa_session_del_rpfn,ppa_session_modify_rpfn, ppa_session_get_rpfn;
        ppa_reg_export_fn(PPA_SESSION_ADD_FN,   (uint32_t)ifx_ppa_session_add, "ppa_session_add",(uint32_t *)&ppa_hook_session_add_fn, (uint32_t)ppa_session_add_rpfn );
        ppa_reg_export_fn(PPA_SESSION_DEL_FN,   (uint32_t)ifx_ppa_session_delete, "ppa_session_delete",(uint32_t *)&ppa_hook_session_del_fn, (uint32_t)ppa_session_del_rpfn);
        ppa_reg_export_fn(PPA_SESSION_MODIFY_FN,(uint32_t)ifx_ppa_session_modify, "ppa_session_modify", (uint32_t *)&ppa_hook_session_modify_fn,(uint32_t)ppa_session_modify_rpfn );
        ppa_reg_export_fn(PPA_SESSION_GET_FN,   (uint32_t)ifx_ppa_session_get, "ppa_session_get", (uint32_t *)&ppa_hook_session_get_fn, (uint32_t)ppa_session_get_rpfn);
    }

    {
        ...
        ppa_reg_export_fn(PPA_MC_GROUP_UPDATE_FN,   (uint32_t)ifx_ppa_mc_group_update, "ppa_mc_group_update", (uint32_t *)&ppa_hook_mc_group_update_fn,(uint32_t)ppa_mc_group_update_rpfn );
        ppa_reg_export_fn(PPA_MC_GROUP_GET_FN,      (uint32_t)ifx_ppa_mc_group_get, "ifx_ppa_mc_group_get",(uint32_t *)&ppa_hook_mc_group_get_fn,(uint32_t)ppa_mc_group_get_rpfn  );
        ppa_reg_export_fn(PPA_MC_ENTRY_MODIFY_FN,   (uint32_t)ifx_ppa_mc_entry_modify, "ppa_mc_entry_modify", (uint32_t *)&ppa_hook_mc_entry_modify_fn,(uint32_t)ppa_mc_entry_modify_rpfn );
        ppa_reg_export_fn(PPA_MC_ENTRY_GET_FN,      (uint32_t)ifx_ppa_mc_entry_get, "ppa_mc_entry_get", (uint32_t *)&ppa_hook_mc_entry_get_fn,(uint32_t)ppa_mc_entry_get_rpfn );
        ppa_reg_export_fn(PPA_MC_PKT_SRCIF_ADD_FN,  (uint32_t)ifx_ppa_multicast_pkt_srcif_add, "ppa_multicast_pkt_srcif_add",(uint32_t *)&ppa_hook_multicast_pkt_srcif_add_fn,(uint32_t)ppa_mc_pkt_srcif_add_rpfn);
    }
    {
        //extern uint32_t ppa_inactivity_status_rpfn, ppa_set_inactivity_rpfn;    
        ppa_reg_export_fn(PPA_INACTIVITY_STATUS_FN, (uint32_t)ifx_ppa_inactivity_status, "ppa_inactivity_status",(uint32_t *)&ppa_hook_inactivity_status_fn, (uint32_t)ppa_inactivity_status_rpfn );
        ppa_reg_export_fn(PPA_SET_INACTIVITY_FN,    (uint32_t)ifx_ppa_set_session_inactivity, "ppa_set_session_inactivity",(uint32_t *)&ppa_hook_set_inactivity_fn, (uint32_t)ppa_set_inactivity_rpfn );
    }
    {    
        ...
        ppa_reg_export_fn(PPA_BRIDGE_ENTRY_ADD_FN,              (uint32_t)ifx_ppa_bridge_entry_add, "ppa_bridge_entry_add", (uint32_t *)&ppa_hook_bridge_entry_add_fn,(uint32_t)ppa_bridge_entry_add_rpfn );
        ppa_reg_export_fn(PPA_BRIDGE_ENTRY_DELETE_FN,           (uint32_t)ifx_ppa_bridge_entry_delete, "ppa_bridge_entry_delete",(uint32_t *)&ppa_hook_bridge_entry_delete_fn, (uint32_t)ppa_bridge_entry_delete_rpfn );
        ppa_reg_export_fn(PPA_BRIDGE_ENTRY_HIT_TIME_FN,         (uint32_t)ifx_ppa_bridge_entry_hit_time, "ppa_bridge_entry_hit_time",(uint32_t *)&ppa_hook_bridge_entry_hit_time_fn,(uint32_t)ppa_bridge_entry_hit_time_rpfn  );
        ppa_reg_export_fn(PPA_BRIDGE_ENTRY_INACTIVITY_STATUS_FN,(uint32_t)ifx_ppa_bridge_entry_inactivity_status, "ppa_bridge_entry_inactivity_status", (uint32_t *)&ppa_hook_bridge_entry_inactivity_status_fn,(uint32_t)ppa_bridge_entry_inactivity_status_rpfn );
        ppa_reg_export_fn(PPA_SET_BRIDGE_ENTRY_TIMEOUT_FN,      (uint32_t)ifx_ppa_set_bridge_entry_timeout, "ppa_set_bridge_entry_timeout",(uint32_t *)&ppa_hook_set_bridge_entry_timeout_fn, (uint32_t)ppa_set_bridge_entry_timeout_rpfn );
        ppa_reg_export_fn(PPA_BRIDGE_ENABLE_FN,                 (uint32_t)ifx_ppa_hook_bridge_enable, "ppa_hook_bridge_enable",(uint32_t *)&ppa_hook_bridge_enable_fn,(uint32_t)ppa_bridge_enable_rpfn  );
    }
    {
        ...
        ppa_reg_export_fn(PPA_SET_BRIDGE_IF_VLAN_CONFIG_FN, (uint32_t)ifx_ppa_set_bridge_if_vlan_config, "ppa_set_bridge_if_vlan_config",(uint32_t *)&ppa_hook_set_bridge_if_vlan_config_fn, (uint32_t)ppa_set_bridge_if_vlan_config_rpfn );
        ppa_reg_export_fn(PPA_GET_BRIDGE_IF_VLAN_CONFIG_FN, (uint32_t)ifx_ppa_get_bridge_if_vlan_config, "ppa_get_bridge_if_vlan_config",(uint32_t *)&ppa_hook_get_bridge_if_vlan_config_fn, (uint32_t)ppa_get_bridge_if_vlan_config_rpfn );
        ppa_reg_export_fn(PPA_VLAN_FILTER_ADD_FN,           (uint32_t)ifx_ppa_vlan_filter_add,"ppa_vlan_filter_add", (uint32_t *)&ppa_hook_vlan_filter_add_fn, (uint32_t)ppa_vlan_filter_add_rpfn );
        ppa_reg_export_fn(PPA_VLAN_FILTER_DEL_FN,           (uint32_t)ifx_ppa_vlan_filter_del, "ppa_vlan_filter_del", (uint32_t *)&ppa_hook_vlan_filter_del_fn, (uint32_t)ppa_vlan_filter_del_rpfn);
        ppa_reg_export_fn(PPA_VLAN_FILTER_GET_ALL_FN,       (uint32_t)ifx_ppa_vlan_filter_get_all, "ppa_vlan_filter_get_all", (uint32_t *)&ppa_hook_vlan_filter_get_all_fn,(uint32_t)ppa_vlan_filter_get_all_rpfn );
        ppa_reg_export_fn(PPA_VLAN_FILTER_DEL_ALL_FN,       (uint32_t)ifx_ppa_vlan_filter_del_all, "ppa_vlan_filter_del_all", (uint32_t *)&ppa_hook_vlan_filter_del_all_fn,(uint32_t)ppa_vlan_filter_del_all_rpfn );
    }
    {
        //extern uint32_t ppa_get_if_stats_rpfn,ppa_get_accel_stats_rpfn;
        ppa_reg_export_fn(PPA_GET_IF_STATS_FN,      (uint32_t)ifx_ppa_get_if_stats, "ppa_get_if_stats",(uint32_t *)&ppa_hook_get_if_stats_fn,(uint32_t)ppa_get_if_stats_rpfn);
        ppa_reg_export_fn(PPA_GET_ACCEL_STATS_FN,   (uint32_t)ifx_ppa_get_accel_stats, "ppa_get_accel_stats", (uint32_t *)&ppa_hook_get_accel_stats_fn, (uint32_t)ppa_get_accel_stats_rpfn);
    }
    {
        //extern uint32_t ppa_set_if_mac_address_rpfn, ppa_get_if_mac_address_rpfn;
        ppa_reg_export_fn(PPA_SET_IF_MAC_ADDRESS_FN, (uint32_t)ifx_ppa_set_if_mac_address, "ppa_set_if_mac_address",(uint32_t *)&ppa_hook_set_if_mac_address_fn,(uint32_t)ppa_set_if_mac_address_rpfn  );
        ppa_reg_export_fn(PPA_GET_IF_MAC_ADDRESS_FN, (uint32_t)ifx_ppa_get_if_mac_address, "ppa_get_if_mac_address",(uint32_t *)&ppa_hook_get_if_mac_address_fn, (uint32_t)ppa_get_if_mac_address_rpfn );
    }
    {
        //extern uint32_t ppa_add_if_rpfn, ppa_del_if_rpfn, ppa_get_if_rpfn;
        ppa_reg_export_fn(PPA_ADD_IF_FN, (uint32_t)ifx_ppa_add_if, "ppa_add_if",(uint32_t *)&ppa_hook_add_if_fn,(uint32_t)ppa_add_if_rpfn) ;
        ppa_reg_export_fn(PPA_DEL_IF_FN, (uint32_t)ifx_ppa_del_if, "ppa_del_if", (uint32_t *)&ppa_hook_del_if_fn,(uint32_t)ppa_del_if_rpfn );
        ppa_reg_export_fn(PPA_GET_IF_FN, (uint32_t)ifx_ppa_get_if, "ppa_get_if", (uint32_t *)&ppa_hook_get_if_fn,(uint32_t)ppa_get_if_rpfn );
    }
    {    
        ...
        ppa_reg_export_fn(PPA_DIRECTPATH_REGISTER_DEV_FN,   (uint32_t)ifx_ppa_directpath_register_dev, "ppa_directpath_register_dev", (uint32_t *)&ppa_hook_directpath_register_dev_fn,(uint32_t)ppa_directpath_register_dev_rpfn );
        ppa_reg_export_fn(PPA_DIRECTPATH_SEND_FN,           (uint32_t)ifx_ppa_directpath_send, "ppa_directpath_send", (uint32_t *)&ppa_hook_directpath_send_fn,(uint32_t)ppa_directpath_send_rpfn );
#ifdef CONFIG_QUALCOMM_AP_PPA
//++Add by Maxx_chen
		ppa_reg_export_fn(PPA_DIRECTPATH_SEND_SPEC_FN,      (uint32_t)ifx_ppa_directpath_send_spec, "ppa_directpath_send_spec", (uint32_t *)&ppa_hook_directpath_send_spec_fn,(uint32_t)ppa_directpath_send_spec_rpfn );
//--Add by Maxx_chen
#endif
        ppa_reg_export_fn(PPA_DIRECTPATH_RX_STOP_FN,        (uint32_t)ifx_ppa_directpath_rx_stop, "ppa_directpath_rx_stop", (uint32_t *)&ppa_hook_directpath_rx_stop_fn, (uint32_t)ppa_directpath_rx_stop_rpfn);
        ppa_reg_export_fn(PPA_DIRECTPATH_RX_RESTART_FN,     (uint32_t)ifx_ppa_directpath_rx_restart, "ppa_directpath_rx_restart",(uint32_t *)&ppa_hook_directpath_rx_restart_fn, (uint32_t)ppa_directpath_rx_restart_rpfn);
        ppa_reg_export_fn(PPA_GET_NETIF_FOR_PPA_IFID_FN,    (uint32_t)ifx_ppa_get_netif_for_ppa_ifid, "ppa_get_netif_for_ppa_ifid", (uint32_t *)&ppa_hook_get_netif_for_ppa_ifid_fn,(uint32_t)ppa_get_netif_for_ppa_ifid_rpfn );
        ppa_reg_export_fn(PPA_GET_IFID_FOR_NETIF_FN,        (uint32_t)ifx_ppa_get_ifid_for_netif, "ppa_get_ifid_for_netif",(uint32_t *)&ppa_hook_get_ifid_for_netif_fn,(uint32_t)ppa_get_ifid_for_netif_rpfn  );

#if defined(CONFIG_ACCL_11AC) || defined(CONFIG_ACCL_11AC_MODULE)
        ppa_reg_export_fn(PPA_DTLK_REGISTER_DEV_FN, (uint32_t)ifx_ppa_directlink_register_dev, "ppa_directlink_register_dev", (uint32_t *)&ppa_hook_directlink_register_dev_fn,(uint32_t)ppa_directlink_register_dev_rpfn );
    ...
    {    
        ...
        ppa_reg_export_fn(PPA_WAN_MII0_VLAN_RANGE_ADD_FN,   (uint32_t)ifx_ppa_hook_wan_mii0_vlan_range_add,"ppa_hook_wan_mii0_vlan_range_add", (uint32_t *)&ppa_hook_wan_mii0_vlan_range_add_fn,(uint32_t)ppa_wan_mii0_vlan_range_add_rpfn );
        ppa_reg_export_fn(PPA_WAN_MII0_VLAN_RANGE_DEL_FN,   (uint32_t)ifx_ppa_hook_wan_mii0_vlan_range_del, "ppa_hook_wan_mii0_vlan_range_del",(uint32_t *)&ppa_hook_wan_mii0_vlan_range_del_fn,(uint32_t)ppa_wan_mii0_vlan_range_del_rpfn  );
        ppa_reg_export_fn(PPA_WAN_MII0_VLAN_RANGES_GET_FN,  (uint32_t)ifx_ppa_hook_wan_mii0_vlan_ranges_get, "ppa_hook_wan_mii0_vlan_ranges_get",(uint32_t *)&ppa_hook_wan_mii0_vlan_ranges_get_fn, (uint32_t)ppa_wan_mii0_vlan_ranges_get_rpfn);
    }
    {
        ...
        ppa_reg_export_fn(PPA_GET_DSL_MIB_FN,   (uint32_t)ifx_ppa_get_dsl_mib, "ppa_get_dsl_mib", (uint32_t *)&ifx_ppa_hook_get_qos_qnum,(uint32_t)ppa_get_qos_qnum_rpfn );
        ppa_reg_export_fn(PPA_GET_PORT_MIB_FN,  (uint32_t)ifx_ppa_get_ports_mib, "ppa_get_ports_mib",(uint32_t *)&ifx_ppa_hook_get_qos_mib,(uint32_t)ppa_get_qos_mib_rpfn  );
#endif /* CONFIG_IFX_PPA_QOS_RATE_SHAPING */
#endif /* CONFIG_IFX_PPA_QOS */
    }
    {
        ppa_reg_export_fn(PPA_SET_LAN_SEPARATE_FLAG_FN,   (uint32_t)ifx_ppa_hook_set_lan_seperate_flag, "ppa_hook_set_lan_seperate_flag",(uint32_t *)&ppa_hook_set_lan_seperate_flag_fn,(uint32_t)ppa_hook_set_lan_seperate_flag_rpfn );
        ppa_reg_export_fn(PPA_GET_LAN_SEPARATE_FLAG_FN,   (uint32_t)ifx_ppa_hook_get_lan_seperate_flag, "ppa_hook_get_lan_seperate_flag",(uint32_t *)&ppa_hook_get_lan_seperate_flag_fn,(uint32_t)ppa_hook_get_lan_seperate_flag_rpfn  );
        ppa_reg_export_fn(PPA_SET_WAN_SEPARATE_FLAG_FN,   (uint32_t)ifx_ppa_hook_set_wan_seperate_flag, "ppa_hook_set_wan_seperate_flag",(uint32_t *)&ppa_hook_set_wan_seperate_flag_fn,(uint32_t)ppa_hook_set_wan_seperate_flag_rpfn );
        ppa_reg_export_fn(PPA_GET_WAN_SEPARATE_FLAG_FN,   (uint32_t)ifx_ppa_hook_get_wan_seperate_flag, "ppa_hook_get_wan_seperate_flag",(uint32_t *)&ppa_hook_get_wan_seperate_flag_fn,(uint32_t)ppa_hook_get_wan_seperate_flag_rpfn  );
    }
#if defined(CONFIG_LTQ_PPA_API_SW_FASTPATH)
    {
...
#if defined(CONFIG_IFX_PPA_API_DIRECTPATH)
    if ( ppa_api_directpath_create() != IFX_SUCCESS )
