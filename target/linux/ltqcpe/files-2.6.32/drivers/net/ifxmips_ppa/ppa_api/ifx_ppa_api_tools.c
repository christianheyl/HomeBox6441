...
//++Add by Maxx_chen
int32_t ppa_directpath_send_spec_rpfn(uint32_t rx_if_id, PPA_BUF *skb, int32_t len, uint32_t flags)
{
    int32_t (*pfn)(uint32_t rx_if_id, PPA_BUF *skb, int32_t len, uint32_t flags) = NULL;
    uint8_t  drop_flag = 0;
    int32_t ret = IFX_EINVAL;
    ...
    if( drop_flag )
    {
        ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT, "ppa_directpath_send_spec_rpfn drop packet\n");
        if( skb )
            PPA_SKB_FREE(skb);
        return IFX_EINVAL;
    }
...
EXPORT_SYMBOL(ppa_directpath_send_spec_rpfn);
//--Add by Maxx_chen
#endif
...
{ /*note, arg is a pointer from ioctl, not normally pointer  */
    ...
    if( !ifx_ppa_is_init() ) goto GET_HOOK_EXIT;
...
{    
	...
	{
		ifx_ppa_hook_set_lan_seperate_flag(cmd_info->var_value_info.value);
		res = 0;
		goto EXIT;
	}
    else if( cmd_info->var_value_info.id == PPA_WAN_SEPARATE_FLAG)
	{
		ifx_ppa_hook_set_wan_seperate_flag(cmd_info->var_value_info.value);
		res = 0;
		goto EXIT;
	}
    res = ifx_ppa_drv_set_value( &cmd_info->var_value_info, 0 );
...
{    
    ...
    if( cmd_info->var_value_info.id == PPA_LAN_SEPARATE_FLAG)
	{
		cmd_info->var_value_info.value=ifx_ppa_hook_get_lan_seperate_flag(0);
		res = 0;
		goto EXIT;
	}
    else if( cmd_info->var_value_info.id == PPA_WAN_SEPARATE_FLAG)
	{
		cmd_info->var_value_info.value=ifx_ppa_hook_get_wan_seperate_flag(0);
		res = 0;
		goto EXIT;
	}
    res = ifx_ppa_drv_get_value( &cmd_info->var_value_info, 0 );
