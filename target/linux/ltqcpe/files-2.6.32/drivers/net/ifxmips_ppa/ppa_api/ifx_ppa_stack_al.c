...
// why define this? linghong_tan 2014-01-27 (!defined(CONFIG_PPPOL2TP) && !defined(CONFIG_PPPOL2TP_MODULE)) || (!defined(CONFIG_IFX_PPA_API) && !defined(CONFIG_IFX_PPA_API_MODULE))
//#if defined(CONFIG_PPPOL2TP) || defined(CONFIG_PPPOL2TP_MODULE)
//anne: the above 2 lines would make redefine compile error. Duplicate with 131-ppa-pppol2tp-support.patch.
#if (!defined(CONFIG_PPPOL2TP) && !defined(CONFIG_PPPOL2TP_MODULE)) || (!defined(CONFIG_IFX_PPA_API) && !defined(CONFIG_IFX_PPA_API_MODULE))
...
{
#if defined(CONFIG_PPPOL2TP) || defined(CONFIG_PPPOL2TP_MODULE)
    return ppa_pppol2tp_get_base_netif(netif,phy_ifname) == IFX_SUCCESS ? 1 : 0;
#else
    return IFX_EPERM;
#endif
}
...
uint32_t ppa_check_is_pppol2tp_netif(PPA_NETIF *netif)
{
    ...
    return ppa_pppol2tp_get_l2tp_addr(netif, &pa) == IFX_SUCCESS ? 1 : 0;
...
int32_t ppa_pppol2tp_get_dst_mac(PPA_NETIF *netif, uint8_t *mac)
{
    ...
    if( (ret = ppa_pppol2tp_get_l2tp_dmac(netif,mac)) != IFX_SUCCESS )
    {
...
static int32_t ppa_get_6rd_dst_mac(struct net_device *dev, uint8_t *mac,uint32_t daddr)
{
    ...
    if ( ppa_check_is_ppp_netif(phy_dev) )
    {
        ...
        {
            ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"not pppoe netif: %s\n", phy_dev->name);
        ...
        if(ppa_get_6rd_dmac_fn != NULL && ppa_get_6rd_dmac_fn(dev, mac, daddr) == IFX_SUCCESS){
            return IFX_SUCCESS;
        }
     }
...
int32_t ppa_get_dst_mac(PPA_BUF *ppa_buf, PPA_SESSION *p_session, uint8_t mac[PPA_ETH_ALEN],uint32_t daddr)
{
    ...
    if(ppa_buf->dev->type == ARPHRD_SIT){
        return ppa_get_6rd_dst_mac(netif, mac,daddr);
    }

    if(ppa_buf->dev->type == ARPHRD_TUNNEL6){
        return ppa_get_dslite_dst_mac(netif, mac);
    }
back_to_pppoe:
    /* First need to check if PPP output interface */
...
skip_pppoe:
	...
	{
            ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"ppa_get_netif_hwaddr : Cannot get PPPOL2TP ppa netif %s address!", netif->name);
            return IFX_ENOTAVAIL;
	}
	if(ppa_check_is_ppp_netif(netif))
	{
...
int32_t ppa_get_br_dst_port(PPA_NETIF *netif, PPA_BUF *ppa_buf, PPA_NETIF **p_netif)
{
    ...
    //dest = ppa_buf->data;
    if ( (ret = ppa_get_dst_mac(ppa_buf, NULL, dest,0)) != IFX_SUCCESS )
...
uint32_t ppa_check_is_special_session(PPA_BUF *ppa_buf, PPA_SESSION *p_session)
{
    ...
    if ( helper_ret = ppa_get_session_helper(p_session) )
    {
#if defined (CONFIG_NF_CONNTRACK_SUPPORT) || defined (CONFIG_NF_CONNTRACK)
		struct nf_conntrack_helper *helper = helper_ret;
		if (strcmp(helper->name,"CONENAT")) {
			return 1;
		}
#else
        return 1;
