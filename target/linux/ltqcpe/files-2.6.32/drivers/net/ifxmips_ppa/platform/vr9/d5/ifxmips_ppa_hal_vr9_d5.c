...
#endif
{
...
#if defined(MBR_CONFIG) && MBR_CONFIG
	shaperid = queueid;
	set_qos_shaper(shaperid,rate,burst,flag);
	 /* put address of shaper date into Mapping table : one to one map */   
    	if ( queueid < TX_QOS_CFG->eth1_eg_qnum )
	__ETH1_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr = __ETH1_QoS_Q_SHAPING_SCHEDULING_CFG(shaperid);
#endif
    }
#if defined(MBR_CONFIG) && MBR_CONFIG
    else
    {
		...
		*__QOS_NEW_FEATURE = 1;
		__ETH1_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr = __ETH1_QoS_Q_SHAPING_SCHEDULING_CFG(shaperid);
     		for( shaper=0; shaper<=7; shaper++ )
     		{
     			*__ETH1_QoS_Q_SHAPER_QUEUE_MAP(shaper) &= ~(1 << queueid);
     		}
      
		*__ETH1_QoS_Q_SHAPER_QUEUE_MAP(shaperid) |= (1 << queueid);
		*__ETH1_QoS_Q_DEFAULT_SHAPER_QUEUE_MAP &= ~(1 << queueid);
...
#endif
{
    ...
    else
    {
#if defined(MBR_CONFIG) && MBR_CONFIG
#if 1
         if((__ETH1_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr) == __ETH1_QoS_DEF_SHAPING_SCH_CFG_SB)
         {
            if ( rate )
               *rate = 0;
            if ( burst )
         ...
         {
            *shaperid = (__ETH1_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr - __ETH1_QoS_Q_SHAPING_BASE)/4;
            qos_cfg = *__ETH1_QoS_Q_SHAPING_SCH_CFG_SB_STRUCT(*shaperid); //queue based rate shaping
        
         }
#endif
#else
        qos_cfg = *__ETH1_QoS_Q_SHAPING_SCH_CFG_SB_STRUCT(queueid); //queue based rate shaping
#endif
...
#if defined(MBR_CONFIG) && MBR_CONFIG
        if ( queueid < TX_QOS_CFG->eth1_eg_qnum )  //QOS note: regard it as port based rate shaping
        *shaperid = (WTX_Q_SHAPER_MAPPING_CFG(queueid)->shaper_base_addr - SHAPING_CFG_BASE)/4;
         else
             *shaperid = -1;
#endif
#endif
...
int32_t reset_qos_rate( uint32_t portid, uint32_t queueid, uint32_t flag )
{
    ...
    else
    {
     	...
     	*__QOS_NEW_FEATURE = 0;
     	*__ETH1_QoS_Q_DEFAULT_SHAPER_QUEUE_MAP = 0xFF;
     	for( shaper=0; shaper<=7; shaper++ )
     	{
     		*__ETH1_QoS_Q_SHAPER_QUEUE_MAP(shaper) &= ~(1 << queueid);
     	}
     	__ETH1_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr = __ETH1_QoS_DEFAULT_SHAPING_SCHEDULING_CFG;
    }
#endif
...
int32_t init_qos_rate(void)
{
     ...
     *__QOS_NEW_FEATURE = 0;
     *__ETH1_QoS_Q_DEFAULT_SHAPER_QUEUE_MAP = 0xFF;
/*Configure the Queue-Shapper mapping Table with default shaper*/
//      __ETH1_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr = __ETH1_QoS_DEFAULT_SHAPING_SCHEDULING_CFG;
#endif
    return IFX_SUCCESS;
}
...
int32_t set_qos_shaper( int32_t shaperid, uint32_t rate, uint32_t burst, uint32_t flag)
{
    ...
    {
         qos_cfg = *__ETH1_QoS_Q_SHAPING_SCH_CFG_SB_STRUCT(shaperid);
         p_qos_cfg = __ETH1_QoS_Q_SHAPING_SCH_CFG_SB_STRUCT(shaperid);
...
int32_t set_qos_wfq( uint32_t portid, uint32_t queueid, uint32_t weight_level, uint32_t flag)
{
    ...
    qos_cfg = *WTX_EG_Q_SHAPING_CFG(queueid);
    *__ETH1_QoS_Q_SHAPER_WEIGHT_NEGATIVE_MAP &= ~(1 << queueid);
