 ...
 */
  ...
  #define IFX_PPE_ORG(idx)                      ((idx) == 0 ? 0x1E200000 : ((idx) == 1 ? g_slave_ppe_base_addr : g_ppe_base_addr))
  extern unsigned int g_slave_ppe_base_addr;
  extern unsigned int g_ppe_base_addr;
#else
  #define IFX_PPE_ORG(idx)                      0x1E200000
#endif
#define IFX_PPE(idx)                            KSEG1ADDR(IFX_PPE_ORG(idx))
#define PP32_DEBUG_REG_ADDR(i, x)               ((volatile unsigned int*)(IFX_PPE(2) + (((x) + 0x000000 + (i) * 0x00010000) << 2)))
#define CDM_CODE_MEMORY(i, x)                   ((volatile unsigned int*)(IFX_PPE(2) + (((x) + 0x001000 + (i) * 0x00010000) << 2)))
#define CDM_DATA_MEMORY(i, x)                   ((volatile unsigned int*)(IFX_PPE(2) + (((x) + 0x004000 + (i) * 0x00010000) << 2)))
#define SB_RAM0_ADDR(idx, x)                    ((volatile unsigned int*)(IFX_PPE(idx) + (((x) + 0x008000) << 2)))
#define SB_RAM1_ADDR(idx, x)                    ((volatile unsigned int*)(IFX_PPE(idx) + (((x) + 0x009000) << 2)))
#define SB_RAM2_ADDR(idx, x)                    ((volatile unsigned int*)(IFX_PPE(idx) + (((x) + 0x00A000) << 2)))
#define SB_RAM3_ADDR(idx, x)                    ((volatile unsigned int*)(IFX_PPE(idx) + (((x) + 0x00B000) << 2)))
#define PPE_REG_ADDR(idx, x)                    ((volatile unsigned int*)(IFX_PPE(idx) + (((x) + 0x00D000) << 2)))
#define QSB_CONF_REG_ADDR(x)                    ((volatile unsigned int*)(IFX_PPE(2) + (((x) + 0x00E000) << 2)))
#define SB_RAM6_ADDR(idx, x)                    ((volatile unsigned int*)(IFX_PPE(idx) + (((x) + 0x018000) << 2)))
...
#define WTX_EG_Q_PORT_SHAPING_CFG(i)            ((volatile struct wtx_eg_q_shaping_cfg *)   SB_BUFFER(1, 0x5BD8 + (i) * 4)) /*  i < 1   */ 
#define WTX_PTM_EG_Q_PORT_SHAPING_CFG(i)        ((volatile struct wtx_eg_q_shaping_cfg *)   SB_BUFFER(1, 0x26A4 + (i) * 4)) /*  i < 4   */

#define WTX_EG_Q_DEF_SHAPING_CFG_ADDR           0x5BDC
#define WTX_EG_Q_SHAPING_BASE			0x5BE0
#define WTX_EG_Q_SHAPING_CFG_ADDR(i)            (0x5BE0 + (i) * 4) /*  i < 8   */ /* 0x5BE0 - 0x5BFF*/
#define WTX_EG_Q_DEF_SHAPING_CFG                ((volatile struct wtx_eg_q_shaping_cfg *)   SB_BUFFER(1, WTX_EG_Q_DEF_SHAPING_CFG_ADDR))            /* 0x5BDC */
#define WTX_EG_Q_SHAPING_CFG(i)                 ((volatile struct wtx_eg_q_shaping_cfg *)   SB_BUFFER(1, WTX_EG_Q_SHAPING_CFG_ADDR(i))) /*  i < 8   */ /* 0x5BE0 - 0x5BFF*/
//Added for PPA2.12 Minimum Bandwidth Reservation
//0x2FD8-0x2FDF
#define WTX_QoS_Q_SHAPER_MAPPING_CFG(i)         SB_BUFFER(1,0x2FD8+(i)) /* i < 8 , 0x2FD8 - 0x2FDF */
#define WTX_QoS_Q_SHAPER_MAPPING_CFG_SB(i)         ((volatile struct wtx_q_shaper_mapping_cfg *) SB_BUFFER(1,0x2FD8+(i))) 
//Minimum Bandwidth Reservation Negative Map and Bit Map
#define __ETH1_QoS_Q_SHAPER_WEIGHT_NEGATIVE_MAP SB_BUFFER(1, 0x3F14)
//Shaper vs queue mapping (default shaper and 8 queue shapers) 
//0x3F15-0x3F1D shaper to queue bit map
#define WTX_QoS_Q_DEF_SHAPER_QUEUE_MAP          SB_BUFFER(1,0x3F15)
#define WTX_QoS_Q_SHAPER_QUEUE_MAP(i)           SB_BUFFER(1,0x3F16 + (i)) /* i < 8, 0x3F16 - 0x3F1D*/              

#define WTX_QoS_Q_SHAPER_NUM                    9
#define __QOS_NEW_FEATURE                       SB_BUFFER(1,0x3F1E)
...
#if defined(L2TP_CONFIG) && L2TP_CONFIG
#define __L2TPTUNNLEL_CONFIG_TABLE_BASE		0x5B70
#define L2TPTUNNEL_CONFIG_TBL(i)                ((volatile struct l2tptunnel_config_tbl *) SB_BUFFER(0,__L2TPTUNNLEL_CONFIG_TABLE_BASE + (i) * 12))
#endif
