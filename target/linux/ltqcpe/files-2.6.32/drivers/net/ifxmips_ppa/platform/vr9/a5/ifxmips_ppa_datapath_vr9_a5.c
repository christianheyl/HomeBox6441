...
static int ppe_send(struct atm_vcc *vcc, struct sk_buff *skb)
{
    ...
    if ( byteoff || skb_headroom(skb) < sizeof(struct sk_buff *) + byteoff || skb->end - skb->data < 1564 /* 1518 (max ETH frame) + 22 (reserved for firmware) + 10 (AAL5) + 6 (PPPoE) + 4 x 2 (2 VLAN) */ || skb_shared(skb) || skb_cloned(skb) )
    {
...
static INLINE void proc_file_create(void)
{
    /*if no parent folder name provided, register function will take the nearest proc_dir_entry by default.
     ...
     // Parent dir,    Name,    IS_DIR,   RD_fn,     WR_fn,     Enable
        ...
        {NULL,  "queue",    0, proc_read_queue,             proc_write_queue,     1 },
        ...
        {NULL,  "htu",      0, proc_read_htu,               NULL,                 1 },
        ...
        {NULL,  "send",     0, proc_read_send,              proc_write_send,      1 },
        ...
        {NULL,  "dsl_vlan", 0, proc_read_dsl_vlan,          proc_write_dsl_vlan,  1 },
#if defined(DEBUG_MIRROR_PROC) && DEBUG_MIRROR_PROC
...
static int proc_read_mib(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    len += sprintf(page + len, "  wrx_correct_oam_cell: %10u\n", DSL_WAN_MIB_TABLE->wrx_correct_cell);
    ...
    len += sprintf(page + len, "  wtx_total_oam_cell:   %10u\n", DSL_WAN_MIB_TABLE->wtx_total_cell);
