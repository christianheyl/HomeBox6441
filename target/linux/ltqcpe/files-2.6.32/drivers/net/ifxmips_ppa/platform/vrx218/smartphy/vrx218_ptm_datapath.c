 ...
 */
//EOAM
extern int EOAM_HANDLER(struct sk_buff *, int);
extern int EOAMAPI_GET_EOAM_DEBUG(void);
...
static int ptm_stop(struct net_device *dev)
{
    ...
    if(ifx_ppa_drv_datapath_mac_entry_setting){
        ifx_ppa_drv_datapath_mac_entry_setting(dev->dev_addr, 0, 6, 10, 1, 2);
    }
