...
static void add_tunnel_6rd_header(struct net_device *dev, volatile struct iphdr *iph);
  ...
  extern int32_t ppa_get_pp32_clock(void);
  ...
  #define PPE_MAX_ETH1_QOS_QUEUE 8
        ...
        uint32_t  default_qos_rateshaping_burst = 6000;
            ...
            int32_t get_qos_rate( uint32_t portid, uint32_t queueid, int32_t* shaperid, uint32_t *rate, uint32_t *burst, uint32_t flag);
        #else
...
static uint32_t g_pppol2tp_entry_counter[MAX_PPPOL2TP_ENTRIES] = {0};
static PPE_LOCK g_pppol2tp_lock;
#endif
...
static int proc_read_qos(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    llen = sprintf(pstr, "  PPE clk=%u MHz, basic tick=%u\n",(u32)ppa_get_pp32_clock()/1000000, (u32)get_basic_time_tick());
    pstr += llen;
    len += llen;
#ifdef CONFIG_IFX_PPA_QOS_WFQ
...
int xet_phy_wan_port(uint32_t port, uint32_t *flags, uint32_t wanmode, int xmode)
{
    ...
    if(xmode == 0){//Get
        if(wan_mode == -1){
            ...
            *flags = IFX_PPA_PHYS_PORT_FLAGS_MODE_ATM_WAN_VALID; 
        }else if(wan_mode == 1){
            *flags = IFX_PPA_PHYS_PORT_FLAGS_MODE_ETH_WAN_VALID;
        }
    }else if(xmode == 1){//Set
        ...
        switch(port)
        {
            ...
            case 7: 
			    if(*CFG_WAN_PORTMAP & (1 << port)){
                    ...
                    if(*p_flags == IFX_PPA_PHYS_PORT_FLAGS_MODE_ETH_WAN_VALID && ifname){
                        strcpy(ifname, "ptm0");
                    }
				}else{
                    *p_flags = IFX_PPA_PHYS_PORT_FLAGS_MODE_EXT_LAN_VALID | IFX_PPA_PHYS_PORT_FLAGS_EXT_CPU0;
			  ...
			  )
{
    ...
    if(ifx_ppa_drv_hal_get_mpoa_type_hook != NULL){//ATM force to remove outer vlan which is used for PVCs differentiation for normal path
...
#if defined(L2TP_CONFIG) && L2TP_CONFIG
    if(l2tptunnel_idx & 0x01){
        if(f_is_lan){
    		action.l2tp_encap_en = 1;
		action.l2tp_hdr_idx = (l2tptunnel_idx >> 1) & 0x07;
	}
	else{
    		action.l2tp_encap_en = 1;
	}
    ...
    if(f_is_lan){
        if(ifx_ppa_drv_hal_get_mpoa_type_hook && !ifx_ppa_drv_hal_get_mpoa_type_hook(dslwan_qid, &mpoa_type)){
...
uint32_t add_6rd_tunnel_entry(struct net_device *dev, uint32_t *tunnel_idx)
{
    ...
    for(entry = 0; entry < MAX_6RD_TUNNEL_ENTRIES; entry ++){
        if(!g_6rd_tunnel_counter[entry] && empty_entry >= MAX_6RD_TUNNEL_ENTRIES){
            ...
            if(iph->saddr == t->parms.iph.saddr){
...
#if defined(L2TP_CONFIG) && L2TP_CONFIG
void add_tunnel_l2tp_header(uint32_t entry, uint32_t tunnel_id, uint32_t session_id, uint32_t src_ip, uint32_t dst_ip, uint16_t ip_chksum, uint16_t udp_chksum)
{

    /* Ipv4 Header */

    /*  0h  */
    L2TPTUNNEL_CONFIG_TBL(entry)->ip_ver = 0x4;
    
    L2TPTUNNEL_CONFIG_TBL(entry)->ip_hl = 0x5;
	
    L2TPTUNNEL_CONFIG_TBL(entry)->ip_tos = 0x0;

    L2TPTUNNEL_CONFIG_TBL(entry)->ip_len = 0x0;

    /*  1h  */
    L2TPTUNNEL_CONFIG_TBL(entry)->ip_id = 0x0;

    L2TPTUNNEL_CONFIG_TBL(entry)->ip_flag = 0x0;

    L2TPTUNNEL_CONFIG_TBL(entry)->ip_frag_off = 0x0;

    /*  2h  */
    L2TPTUNNEL_CONFIG_TBL(entry)->ip_ttl = 0x40;

    L2TPTUNNEL_CONFIG_TBL(entry)->ip_proto = 0x11;

    L2TPTUNNEL_CONFIG_TBL(entry)->ip_cksum = ip_chksum;

    /*  3h  */
    L2TPTUNNEL_CONFIG_TBL(entry)->ip_sadd = src_ip;

    /*  4h  */
    L2TPTUNNEL_CONFIG_TBL(entry)->ip_dadd = dst_ip;

    /*  UDP Header */

    /*  5h  */
    L2TPTUNNEL_CONFIG_TBL(entry)->udp_s_port = 0x06a5;

    L2TPTUNNEL_CONFIG_TBL(entry)->udp_d_port = 0x06a5;

    /*  6h  */
    L2TPTUNNEL_CONFIG_TBL(entry)->udp_tot_len = 0x0;

    L2TPTUNNEL_CONFIG_TBL(entry)->udp_cksum = udp_chksum;

    /*  L2TP Header  */

    /*  7h  */
    L2TPTUNNEL_CONFIG_TBL(entry)->l2tp_flag = 0x020;

    L2TPTUNNEL_CONFIG_TBL(entry)->l2tp_ver = 0x2;

    L2TPTUNNEL_CONFIG_TBL(entry)->l2tp_tunnel_id = tunnel_id;
	
    /*  8h  */

    L2TPTUNNEL_CONFIG_TBL(entry)->l2tp_session_id = session_id;

    L2TPTUNNEL_CONFIG_TBL(entry)->l2tp_off_size = 0x0;


    /*  9h  */
/*
    L2TPTUNNEL_CONFIG_TBL(entry)->l2tp_nr = 0x0;

    L2TPTUNNEL_CONFIG_TBL(entry)->l2tp_ns = 0x0; 
*/

    /*  10h  */
/*
    L2TPTUNNEL_CONFIG_TBL(entry)->l2tp_off_pad = 0x0;
*/
    /*  PPP Header  */

    /*  11h  */
    L2TPTUNNEL_CONFIG_TBL(entry)->ppp_addr = 0xff;

    L2TPTUNNEL_CONFIG_TBL(entry)->ppp_ctrl = 0x03;

    L2TPTUNNEL_CONFIG_TBL(entry)->ppp_proto = 0x0021;
    L2TPTUNNEL_CONFIG_TBL(entry)->us_mib = 0x0;
    L2TPTUNNEL_CONFIG_TBL(entry)->ds_mib = 0x0;
    
    return;
}

/*
  \fn int32_t add_l2tp_tunnel_entry(uint32_t tunnel_id,
                        uint32_t *p_entry)
  \ingroup AMAZON_S_PPA_PPE_D5_HAL_GLOBAL_FUNCTIONS
  \brief add PPPoL2TP Entry
  \param tunnel_id     PPPoL2TP tunnel ID
  \param p_entry        a data pointer to get entry number
  \return 0: OK, otherwise: fail
 */
int32_t add_l2tp_tunnel_entry(uint32_t tunnel_id,uint32_t session_id,uint32_t src_ip,uint32_t dst_ip,uint16_t ip_chksum,uint16_t udp_chksum,uint32_t *p_entry)
{
    uint32_t entry, empty_entry = MAX_PPPOL2TP_ENTRIES;

    ppe_lock_get(&g_pppol2tp_lock);

    for ( entry = 0; entry < MAX_PPPOL2TP_ENTRIES; entry++ ){
        if ( (!g_pppol2tp_entry_counter[entry]) && (empty_entry >= MAX_PPPOL2TP_ENTRIES))
	{
            empty_entry = entry;
	}
        else {
	    if(tunnel_id == L2TPTUNNEL_CONFIG_TBL(entry)->l2tp_tunnel_id)
	    {
    ...
    entry = empty_entry;
    if ( empty_entry >= MAX_PPPOL2TP_ENTRIES )
    {
        ppe_lock_release(&g_pppol2tp_lock);
        return IFX_EAGAIN;
    }
    else
    {
    	add_tunnel_l2tp_header(entry,tunnel_id,session_id,src_ip,dst_ip,ip_chksum,udp_chksum);
    }

ADD_PPPOL2TP_ENTRY_GOON:
    g_pppol2tp_entry_counter[entry]++;
...
void del_l2tp_tunnel_entry(uint32_t entry)
{
    if ( entry >= MAX_PPPOL2TP_ENTRIES )
        return;
    volatile uint32_t *p;

    ppe_lock_get(&g_pppol2tp_lock);

    if ( g_pppol2tp_entry_counter[entry] && !--g_pppol2tp_entry_counter[entry] ){
        
	p = (volatile uint32_t *)L2TPTUNNEL_CONFIG_TBL(entry);
...
*/
#if defined(MBR_CONFIG) && MBR_CONFIG
    int32_t set_qos_rate( uint32_t portid, uint32_t queueid, int32_t shaperid, uint32_t rate, uint32_t burst, uint32_t flag)
#else
...
#endif
{
    ...
    if(shaper_id == -1){
        ...
        {
            *__QOS_NEW_FEATURE = 0; //compatible mode
...
#if defined(MBR_CONFIG) && MBR_CONFIG
        shaper_id = queueid;
        //set_qos_shaper(shaper_id,rate,burst,flag);
         /* put address of shaper date into Mapping table : one to one map */   
        if ( queueid < TX_QOS_CFG->eth1_eg_qnum )
            WTX_EG_Q_SHAPER_MAPPING_CFG(queueid)->shaper_base_addr = WTX_EG_Q_SHAPER_ADDR(shaper_id);
#endif
    ...
    else
    {
        ...
        if ( queueid < TX_QOS_CFG->eth1_eg_qnum ){
            *__QOS_NEW_FEATURE = 1;
            cur_shaperid = WTX_EG_Q_SHAPER_ID(queueid);
            if(cur_shaperid < 0){//current shaper is default shaper
                *WTX_EG_Q_DEF_SHAPER_QUEUE_MAP &= ~(1 << queueid);
            }else{
                *WTX_EG_Q_SHAPER_QUEUE_MAP(cur_shaperid) &= ~(1 << queueid);
            }
            
                *WTX_EG_Q_DEF_SHAPER_QUEUE_MAP &= ~(1 << queueid);
            WTX_EG_Q_SHAPER_MAPPING_CFG(queueid)->shaper_base_addr = WTX_EG_Q_SHAPER_ADDR(shaper_id);
            *WTX_EG_Q_SHAPER_QUEUE_MAP(shaper_id) |= (1 << queueid);
...
#if defined(MBR_CONFIG) && MBR_CONFIG
    shaper_id = WTX_EG_Q_SHAPER_ID(queueid);
    if(shaper_id < 0){//default shaper
        //printk("\nfunction=%s,line number=%d\n",__FUNCTION__,__LINE__);
        *shaperid = shaper_id;
        qos_cfg   = *WTX_EG_Q_DEF_SHAPING_CFG;
    }else{
        *shaperid = shaper_id;
        qos_cfg   = *WTX_EG_Q_SHAPING_CFG(shaper_id);
    }
...
int32_t reset_qos_rate( uint32_t portid, uint32_t queueid, uint32_t flag )
{
    ...
    if ( queueid >= TX_QOS_CFG->eth1_eg_qnum ){  //regard it as port based rate shaping
         ...
         set_qos_shaper(-1, 1000000, default_qos_rateshaping_burst, flag); //reset default shaper as well
    }
    else{
     	*__QOS_NEW_FEATURE = 0;
     	*WTX_EG_Q_DEF_SHAPER_QUEUE_MAP |= (1 << queueid);
        for(shaperid = 0; shaperid < MAX_SHAPER_NUM; shaperid ++)
     	    *WTX_EG_Q_SHAPER_QUEUE_MAP(shaperid) &= ~(1 << queueid);
        
     	WTX_EG_Q_SHAPER_MAPPING_CFG(queueid)->shaper_base_addr = WTX_EG_Q_DEF_SHAPER_ADDR;
        //set_qos_rate(portid, queueid, -1, 1000000, default_qos_rateshaping_burst, flag);
    }
#else
...
int32_t set_qos_shaper( int32_t shaperid, uint32_t rate, uint32_t burst, uint32_t flag)
{
    ...
    {
         if(shaperid >= 0){
            qos_cfg = *WTX_EG_Q_SHAPING_CFG(shaperid);
            p_qos_cfg = WTX_EG_Q_SHAPING_CFG(shaperid);
         }else{
            qos_cfg = *WTX_EG_Q_DEF_SHAPING_CFG;
            p_qos_cfg = WTX_EG_Q_DEF_SHAPING_CFG;
         }
...
int32_t set_qos_wfq( uint32_t portid, uint32_t queueid, uint32_t weight_level, uint32_t flag)
{
    ...
    qos_cfg = *WTX_EG_Q_SHAPING_CFG(queueid);
    *WTX_EG_Q_SHAPER_WEIGHT_NEGATIVE_MAP &= ~(1 << queueid);
    ...
    if ( PS_MC_GENCFG3->vrx218_en ) {
        ...
        case PPA_GENERIC_HAL_GET_PPE_FW_VERSION:
            {
                ...
                if ( version->index == 0 )
                    break;
                version->index -= 1;
                ret = ifx_ppe_ext_datapath_generic_hook != NULL ? ifx_ppe_ext_datapath_generic_hook(PPA_GENERIC_DATAPATH_GET_PPE_VERION, buffer, flag) : IFX_FAILURE;
                version->index += 1;
                return ret;
            }
        ...
        case PPA_GENERIC_HAL_CLEAR_DSL_MIB:
            if ( PS_MC_GENCFG3->vrx218_atm )
                return ifx_ppe_ext_hal_generic_hook != NULL ? ifx_ppe_ext_hal_generic_hook(cmd, buffer, flag) : IFX_FAILURE;
            else
                return IFX_FAILURE;
...
#if defined(L2TP_CONFIG) && L2TP_CONFIG
			  , route->l2tptnnl_info.tunnel_idx
#endif
			);
            }
...
#if defined(L2TP_CONFIG) && L2TP_CONFIG
    case PPA_GENERIC_HAL_ADD_L2TP_TUNNEL_ENTRY:
        {
            PPA_L2TP_INFO *l2tp_info = (PPA_L2TP_INFO *)buffer;
            return add_l2tp_tunnel_entry(l2tp_info->tunnel_id,l2tp_info->session_id,
					 l2tp_info->source_ip,l2tp_info->dest_ip,
					 l2tp_info->ip_chksum,l2tp_info->udp_chksum,
					 &l2tp_info->tunnel_idx);
        }
    case PPA_GENERIC_HAL_DEL_L2TP_TUNNEL_ENTRY:
        {
            PPA_L2TP_INFO *tnnl_info = (PPA_L2TP_INFO *)buffer;
            del_l2tp_tunnel_entry(tnnl_info->tunnel_idx);
            return IFX_SUCCESS;
        }
#endif
