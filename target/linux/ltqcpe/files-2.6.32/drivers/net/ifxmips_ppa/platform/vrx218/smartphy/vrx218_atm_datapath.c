 ...
 */
  ...
  static int (*ifx_mei_atm_showtime_enter)(struct port_cell_info *, void *) = NULL;
  static int (*ifx_mei_atm_showtime_exit)(void) = NULL;
#endif
...
int proc_read_prio(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    int i, j;
...
int proc_write_prio(struct file *file, const char *buf, unsigned long count, void *data)
{
    char local_buf[2048];
    int len;
    char *p1, *p2;
...
int proc_read_pcie_rst(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
...
int proc_read_dsl_vlan(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    int i;
...
int proc_write_dsl_vlan(struct file *file, const char *buf, unsigned long count, void *data)
{
    char local_buf[2048];
    int len;
    char *p1, *p2;
...
int proc_write_cell(struct file *file, const char *buf, unsigned long count, void *data)
{
    char local_buf[2048];
    int len;
    char *p1, *p2;
...
int __init vrx218_atm_datapath_init(const ifx_pcie_ep_dev_t *p_vrx218_dev)
{
    ...
    for ( i = 0; i < ATM_PORT_NUMBER; i++ )
    {
        ...
        g_atm_priv_data.port[i].dev = atm_dev_register("ltq_atm", &g_ppe_atm_ops, -1, 0UL);
    ...
    ifx_ppa_drv_get_atm_qid_with_pkt_hook    = get_atm_qid_with_pkt;
    ifx_ppa_drv_hal_get_mpoa_type_hook       = get_mpoa_type;
...
void __exit vrx218_atm_datapath_exit(void)
{
    ...
    ifx_ppa_drv_get_atm_qid_with_pkt_hook    = NULL;
    ifx_ppa_drv_hal_get_mpoa_type_hook       = NULL;
