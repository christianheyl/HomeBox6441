...
static void add_tunnel_6rd_header(struct net_device *dev, volatile struct iphdr *iph);
    ...
    static uint32_t set_qos_port_id(void)
    {
        ...
        if( wan_itf & IFX_PPA_DEST_LIST_ETH0 ) qos_queue_portid = PPA_WAN_QOS_ETH0;
        else if( wan_itf & IFX_PPA_DEST_LIST_ETH1 ) qos_queue_portid = PPA_WAN_QOS_ETH1;
        else if ( wan_itf & IFX_PPA_DEST_LIST_ATM ) qos_queue_portid = PPA_WAN_QOS_PTM0;
        else
        {
            qos_queue_portid = -1; //invalid
            res = IFX_FAILURE;
...
#endif
{
	...
	WTX_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr = WTX_EG_Q_SHAPING_CFG_ADDR(shaperid);
#endif
    }
#if defined(MBR_CONFIG) && MBR_CONFIG
    else
    {
    	...
    	{
		*__QOS_NEW_FEATURE = 1;
		/*Configure the Queue-Shapper mapping Table*/
		WTX_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr = WTX_EG_Q_SHAPING_CFG_ADDR(shaperid);
     		for( shaper=0; shaper<=7; shaper++ )
     		{
     			*WTX_QoS_Q_SHAPER_QUEUE_MAP(shaper) &= ~(1 << queueid);
     		}
		*WTX_QoS_Q_SHAPER_QUEUE_MAP(shaperid) |= (1 << queueid);
		*WTX_QoS_Q_DEF_SHAPER_QUEUE_MAP &= ~(1 << queueid);
...
#endif
{
	...
	*shaperid = -1;
    ...
    else
    {
#if defined(MBR_CONFIG) && MBR_CONFIG
#if 1
         if((WTX_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr) == WTX_EG_Q_DEF_SHAPING_CFG)
         {
            if ( rate )
               *rate = 0;
            if ( burst )
         ...
         {
            *shaperid = (WTX_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr - WTX_EG_Q_SHAPING_BASE)/4;
            qos_cfg = *WTX_EG_Q_SHAPING_CFG(*shaperid); //queue based rate shaping
...
int32_t reset_qos_rate( uint32_t portid, uint32_t queueid, uint32_t flag )
{
    ...
    {
     *__QOS_NEW_FEATURE = 0;
     *WTX_QoS_Q_DEF_SHAPER_QUEUE_MAP = 0xFF;
     for( shaper=0; shaper<=7; shaper++ )
     {
     	*WTX_QoS_Q_SHAPER_QUEUE_MAP(shaper) &= ~(1 << queueid);
     }
/*Configure the Queue-Shapper mapping Table with default shaper*/
      WTX_QoS_Q_SHAPER_MAPPING_CFG_SB(queueid)->shaper_base_addr = WTX_EG_Q_DEF_SHAPING_CFG_ADDR;
    }
#endif
