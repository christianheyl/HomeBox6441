...
//  SDMA Flow Control Threshold 
#define SDMA_FCTHR(n)                           VR9_SWIP_MACRO_REG(0xB41 + (n)) /* n < 7 */
...
static int proc_read_powersaving(char *, char **, off_t, int, int *, void *);
static int proc_write_powersaving(struct file *, const char *, unsigned long, void *);
#ifdef CONFIG_IFX_PPE_E5_OFFCHIP_BONDING
  ...
  static int proc_write_bonding(struct file *, const char *, unsigned long, void *);
#endif
...
static INLINE void init_internal_tantos(void)
{
    ...
    *PMAC_HD_CTL_REG = 0x05DC;  //  PMAC Head  + enable Flow Control
    ...
    /* Add support for DSL retransmission */
    *SDMA_FCTHR(0) = 0;
    *SDMA_FCTHR(1) = 0;
    *SDMA_FCTHR(2) = 0x40;
    *SDMA_PCTRL_REG(6) = 0x4103;
...
static INLINE int init_local_variables(void)
{
    ...
    if ( g_dsl_bonding == 2 )   //  off chip bonding
    {
        ...
        g_pci_dev_ppe_addr_off = (unsigned int)CPHYSADDR(g_slave_ppe_base_addr - IFX_PPE_ORG(0));
        ...
        g_slave_ppe_base_addr = IFX_PPE_ORG(0);
#endif
...
static INLINE void init_qos_queue(void)
{
    ...
    //by default, all queues map to default shaper
    for(i = 0; i < __ETH_WAN_TX_QUEUE_NUM; i ++){
        *WTX_QoS_Q_SHAPER_MAPPING_CFG(i) = WTX_EG_Q_DEF_SHAPING_CFG_ADDR;
        //default shaper to queue bitmap map to all queues.
        *WTX_QoS_Q_DEF_SHAPER_QUEUE_MAP |= 1 << i;
    }
...
static int dsl_proc_read_status(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    if ( g_dsl_bonding)
    {
        len += sprintf(page + len, BOND_CONF->bond_mode == L1_PTM_TC_BOND_MODE ? "Bonding Mode: PTM TC\n" : "Bonding Mode: ETH Trunk\n");
    }
...
static INLINE void proc_file_create(void)
{
    /*if no parent folder name provided, register function will take the NEAREST proc_dir_entry by default.
     ...
     // Parent dir,    Name,    IS_DIR,   RD_fn,     WR_fn,     Enable
        ...
        {NULL,  "status",   0, dsl_proc_read_status,        dsl_proc_write_status,1 },
        {"proc","eth",      1, NULL,                        NULL,                 1 },
        {NULL,  "ver",      0, proc_read_ver,               NULL,                 1 },
        {NULL,  "mib",      0, proc_read_mib,               proc_write_mib,       1 },
        {NULL,  "wanmib",   0, proc_read_wanmib,            proc_write_wanmib,    1 },
        ...
        {NULL,  "gamma",    0, proc_read_gamma,             NULL,                 1 },
#if defined(DEBUG_MIRROR_PROC) && DEBUG_MIRROR_PROC
...
#ifdef CONFIG_IFX_PPE_E5_OFFCHIP_BONDING
        {NULL,  "dev",      0, proc_read_dev,               proc_write_dev,       1 },
#endif
#if defined(DEBUG_BONDING_PROC) && DEBUG_BONDING_PROC
        {NULL,  "bonding",  0, proc_read_bonding,           proc_write_bonding,   1 },
#endif
        {NULL,"powersaving",0, proc_read_powersaving,       proc_write_powersaving, 1 },
...
static int proc_read_wanmib(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    len += sprintf(page + len, "RX (Bearer Channels):\n");
    len += sprintf(page + len, "   wrx_bc_overdrop:");
    for ( i = 0; i < 2; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ",             ");
        len += sprintf(page + len, "%10u", IFX_REG_R32(wrx_bc_overdrop_cnt[i]) - last_wrx_bc_overdrop_cnt[PPE_ID][i]);
    }
    len += sprintf(page + len, "\n");
    ...
    for ( i = 0; i < 2; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ",             ");
        len += sprintf(page + len, "%10u", IFX_REG_R32(RECEIVE_NON_IDLE_CELL_CNT(i)));
    }
    len += sprintf(page + len, "\n");
    ...
    for ( i = 0; i < 2; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ",             ");
        len += sprintf(page + len, "%10u", IFX_REG_R32(RECEIVE_IDLE_CELL_CNT(i)));
    }
    len += sprintf(page + len, "\n");

    len += sprintf(page + len, "RX (Gamma Interfaces):\n");
    len += sprintf(page + len, "  wrx_total_pdu:   ");
    for ( i = 0; i < 4; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", IFX_REG_R32(wrx_total_pdu[i]) - last_wrx_total_pdu[PPE_ID][i]);
    }
    len += sprintf(page + len, "\n");
    ...
    for ( i = 0; i < 4; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", IFX_REG_R32(wrx_cv_cw_cnt[i]) - last_wrx_cv_cw_cnt[PPE_ID][i]);
    }
    len += sprintf(page + len, "\n");
    ...
    for ( i = 0; i < 4; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", IFX_REG_R32(wrx_crc_err_pdu[i]) - last_wrx_crc_error_total_pdu[PPE_ID][i]);
    }
    len += sprintf(page + len, "\n");
    ...
    len += sprintf(page + len, "TX (Bearer Channels):\n");
    len += sprintf(page + len, "  total_tx_cw:     %10u,             %10u\n", IFX_REG_R32(TRANSMIT_CELL_CNT(0)), IFX_REG_R32(TRANSMIT_CELL_CNT(1)));

    len += sprintf(page + len, "TX (QoS Queues):\n");
    len += sprintf(page + len, "  wrx_total_pdu:           ");
    for ( i = 0; i < 8; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_TX_MIB_TABLE(i)->wrx_total_pdu);
    }
    len += sprintf(page + len, "\n");
    len += sprintf(page + len, "  wrx_total_bytes:         ");
    for ( i = 0; i < 8; i++ )
    {
        if ( i != 0 )
            len += sprintf(page + len, ", ");
        len += sprintf(page + len, "%10u", WAN_TX_MIB_TABLE(i)->wrx_total_bytes);
    }
    len += sprintf(page + len, "\n");
...
static int proc_read_fw(char *page, char **start, off_t off, int count, int *eof, void *data)
{
...

    {
        ...
        len += sprintf(page + len, "SFSM Status:  bc0                         bc1  \n\n");
        len += sprintf(page + len, "Local State = %d [%-22s], %d [%-22s]\n"
        ...
        len += sprintf(page + len, "  dbase     = 0x%04x  ( 0x%08x )    , 0x%04x  ( 0x%08x )\n",
        ...
        len += sprintf(page + len, "  cbase     = 0x%04x  ( 0x%08x )    , 0x%04x  ( 0x%08x )\n",
        ...
        len += sprintf(page + len, "  sen       = %-26d, %d\n", SFSM_CFG(0)->sen, SFSM_CFG(1)->sen );
        len += sprintf(page + len, "  idlekeep  = %-26d, %d\n", SFSM_CFG(0)->idlekeep, SFSM_CFG(1)->idlekeep );
        len += sprintf(page + len, "  pnum      = %-26d, %d\n", SFSM_CFG(0)->pnum, SFSM_CFG(1)->pnum );
        len += sprintf(page + len, "  pptr      = %-26d, %d\n", SFSM_PGCNT(0)->pptr, SFSM_PGCNT(1)->pptr);
        len += sprintf(page + len, "  upage     = %-26d, %d\n", SFSM_PGCNT(0)->upage, SFSM_PGCNT(1)->upage);
        len += sprintf(page + len, "  rdptr     = %-26d, %d\n", RX_BC_CFG(0)->rx_cw_rdptr, RX_BC_CFG(1)->rx_cw_rdptr );
        len += sprintf(page + len, "\n");


        len += sprintf(page + len, "FFSM Status:  bc0                         bc1  \n\n");
        len += sprintf(page + len, "  dbase     = 0x%04x  ( 0x%08x )    , 0x%04x  ( 0x%08x )\n",
...
static int proc_read_prio(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    if ( g_eth_wan_mode == 0 )
    {
        ...
        for ( j = 0; j < NUM_ENTITY(g_ptm_prio_queue_map); j++ )
            len += sprintf(page + len," %2d", g_ptm_prio_queue_map[j]);
        len += sprintf(page + len,    "\n");
    }
...
static int proc_read_powersaving(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
...
static int proc_write_powersaving(struct file *file, const char *buf, unsigned long count, void *data)
{
    char local_buf[2048];
    int len;
    char *p1, *p2;
...
static int proc_read_dev(char *buf, char **start, off_t offset, int count, int *eof, void *data)
{
    ...
    len += sprintf(buf + len, "  g_slave_ppe_base_addr  = 0x%08x\n", g_slave_ppe_base_addr);
    len += sprintf(buf + len, "  g_pci_dev_ppe_addr_off = 0x%08x\n", g_pci_dev_ppe_addr_off);
    len += sprintf(buf + len, "  g_ppe_base_addr        = 0x%08x\n", g_ppe_base_addr);
    len += sprintf(buf + len, "  IFX_PMU                = 0x%08x\n", IFX_PMU);
    len += sprintf(buf + len, "  g_bm_addr              = 0x%08x\n", g_bm_addr);
    len += sprintf(buf + len, "  g_pci_host_ddr_addr    = 0x%08x\n", g_pci_host_ddr_addr);
...
static int proc_read_bonding(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    len += sprintf(page + len, "  CURR_TIME [0x200C]       = 0x%08x\n\n", *(volatile u32 *)TIME_STAMP);
...
static int proc_write_bonding(struct file *file, const char *buf, unsigned long count, void *data)
{
    char str[2048];
    char *p;
    char * tokens[5];
...
static INLINE int print_route(char *buf, int i, int f_is_lan, struct rout_forward_compare_tbl *pcompare, struct rout_forward_action_tbl *pwaction)
{
    ...
    if(f_is_lan){
        ...
        len += sprintf(buf + len,      "      l2tp_encap tunnel: %s\n", pwaction->l2tp_encap_en ? "yes":"no");
        len += sprintf(buf + len,      "      tunnel iphdr idx: %d\n", pwaction->tnnl_hdr_idx);
        len += sprintf(buf + len,      "      tunnel l2tphdr idx: %d\n", pwaction->l2tp_hdr_idx);
    }else{
