...
extern int proc_read_prio(char *, char **, off_t, int, int *, void *);
extern int proc_write_prio(struct file *, const char *, unsigned long, void *);
extern int proc_read_dsl_vlan(char *, char **, off_t, int, int *, void *);
extern int proc_write_dsl_vlan(struct file *, const char *, unsigned long, void *);
extern int proc_write_cell(struct file *, const char *, unsigned long, void *);
...
//  Parent dir,     Name,           IS_DIR, RD_fn,              WR_fn,              Enable
    {NULL,          "eth/vrx318",   1,      NULL,               NULL,               1,  NULL, NULL},
    ...
    {NULL,          "wanmib",       0,      proc_read_wanmib,   proc_write_wanmib,  1,  NULL, NULL},
    {NULL,          "queue",        0,      proc_read_queue,    NULL,               1,  NULL, NULL},
    {NULL,          "htu",          0,      proc_read_htu,      NULL,               1,  NULL, NULL},
    {NULL,          "prio",         0,      proc_read_prio,     proc_write_prio,    1,  NULL, NULL},
    {NULL,          "dsl_vlan",     0,      proc_read_dsl_vlan, proc_write_dsl_vlan,1,  NULL, NULL},
    {NULL,          "cell",         0,      NULL,               proc_write_cell,    1,  NULL, NULL},
...
static int __init init_vrx218_module(const ifx_pcie_ep_dev_t *dev)
{
    ...
    set_vrx218_ppe_clk(432, dev->phy_membase);
    /* clear mailbox */
...
static int proc_read_ver(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    len += print_fw_ver(page + len, count - len, *FW_VER_ID);
...
static int proc_read_pp32(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    for ( pp32 = 0; pp32 < 2; pp32++ )
    {
        ...
        if ( f_stopped )
        {
            ...
            {
                for ( j = 0; j < 4; j++ )
                    len += sprintf(page + len, "   %2d: %08x", i + j * 4, *PP32_GP_CONTEXTi_REGn(g_vrx218_dev.phy_membase, pp32, cur_context, i + j * 4));
...
static int proc_read_wanmib(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    len += sprintf(page + off + len, "DSL RX QUEUE MIB:\n");
    len += sprintf(page + off + len, "  idx     pdu       bytes\n");
    for ( i = 0; i < 16; i++ )
        len += sprintf(page + off + len, "   %2d %10u %10u\n", i, DSL_QUEUE_RX_MIB_TABLE(i)->pdu, DSL_QUEUE_RX_MIB_TABLE(i)->bytes);
    len += sprintf(page + off + len, "DSL TX QUEUE MIB:\n");
    len += sprintf(page + off + len, "  idx     pdu       bytes\n");
    for ( i = 0; i < 16; i++ )
...
static int print_htu(char *buf, int i)
{
    int len = 0;
