...
const IFX_FLOW_PCE_MICROCODE pce_mc_max_ifx_tag_m = {
    //------------------------------------------------------------------------------------------
...
IFX_int32_t ifx_pce_tm_find_vlan_id_fd_index (IFX_PCE_HANDLE_t *pTmHandle,IFX_FLOW_VLAN_tableEntry_t *VLAN_Table_Entry ) 
{
	IFX_ETHSW_XWAYFLOW_PCE_TABLE_ENTRY_t pcetable_vlan;
	IFX_int32_t	table_index, vid_index = 0x7F;
	IFX_void_t *pDevCtx =IFX_NULL;
...
IFX_return_t ifx_ethsw_xwayflow_pce_table_write(IFX_void_t *pDevCtx, IFX_ETHSW_XWAYFLOW_PCE_TABLE_ENTRY_t *pData)
{
  ...
  ifx_ethsw_ll_DirectAccessWrite(pDevCtx, VR9_PCE_TBL_KEY_8_KEY8_OFFSET,	\
    VR9_PCE_TBL_KEY_8_KEY8_SHIFT, VR9_PCE_TBL_KEY_8_KEY8_SIZE, pData->key[8]);
	...
	ifx_ethsw_ll_DirectAccessWrite(pDevCtx, GSWIP_PCE_TBL_VAL_6_VAL6_OFFSET,	\
		GSWIP_PCE_TBL_VAL_6_VAL6_SHIFT, GSWIP_PCE_TBL_VAL_6_VAL6_SIZE, pData->val[6]);
	ifx_ethsw_ll_DirectAccessWrite(pDevCtx, GSWIP_PCE_TBL_VAL_5_VAL5_OFFSET,	\
		GSWIP_PCE_TBL_VAL_5_VAL5_SHIFT, GSWIP_PCE_TBL_VAL_5_VAL5_SIZE, pData->val[5]);
...
IFX_return_t ifx_ethsw_xwayflow_pce_table_read(IFX_void_t *pDevCtx, IFX_ETHSW_XWAYFLOW_PCE_TABLE_ENTRY_t *pData)
{
  ...
  // pData->key['X'] = PCE_TBL_KEY_'X'.KEY'X' //done for all key values
  	ifx_ethsw_ll_DirectAccessRead(pDevCtx, VR9_PCE_TBL_KEY_8_KEY8_OFFSET,	\
    	VR9_PCE_TBL_KEY_8_KEY8_SHIFT, VR9_PCE_TBL_KEY_8_KEY8_SIZE, &value);
    pData->key[8] = value;
  ...
  // pData->val['X'] = PCE_TBL_VAL_'X' //done for all values
  	ifx_ethsw_ll_DirectAccessRead(pDevCtx, GSWIP_PCE_TBL_VAL_6_VAL6_OFFSET,	\
    	GSWIP_PCE_TBL_VAL_6_VAL6_SHIFT, GSWIP_PCE_TBL_VAL_6_VAL6_SIZE, &value);
    pData->val[6] = value;
    ifx_ethsw_ll_DirectAccessRead(pDevCtx, GSWIP_PCE_TBL_VAL_5_VAL5_OFFSET,	\
    	GSWIP_PCE_TBL_VAL_5_VAL5_SHIFT, GSWIP_PCE_TBL_VAL_5_VAL5_SIZE, &value);
    pData->val[5] = value;
...
IFX_return_t ifx_ethsw_xwayflow_pce_table_key_read(IFX_void_t *pDevCtx, IFX_ETHSW_XWAYFLOW_PCE_TABLE_ENTRY_t *pData)
{
    IFX_uint32_t value;
	...
	ifx_ethsw_ll_DirectAccessWrite(pDevCtx, VR9_PCE_TBL_CTRL_OPMOD_OFFSET,	\
		VR9_PCE_TBL_CTRL_OPMOD_SHIFT, VR9_PCE_TBL_CTRL_OPMOD_SIZE, IFX_ETHSW_LOOKUP_TABLE_ACCESS_OP_MODE_KSRD);
	// start the table access:
  // PCE_TBL_CTRL.BAS = BUSY //(start the read access)
	ifx_ethsw_ll_DirectAccessWrite(pDevCtx, VR9_PCE_TBL_CTRL_BAS_OFFSET,	\
    VR9_PCE_TBL_CTRL_BAS_SHIFT, VR9_PCE_TBL_CTRL_BAS_SIZE, IFX_TRUE);
...
IFX_return_t ifx_ethsw_xwayflow_pce_table_key_write(IFX_void_t *pDevCtx, IFX_ETHSW_XWAYFLOW_PCE_TABLE_ENTRY_t *pData)
{
    IFX_uint32_t value;
...
/*   IFXOS_PRINT_INT_RAW(" ERROR: %s:%s:%d (pFet->sip_msb_idx:%d,pFet->sip_lsb_idx:%d ) \n",	\
   	 __FILE__, __FUNCTION__, __LINE__,pFet->sip_msb_idx,pFet->sip_lsb_idx); */
...
/*			tab_index = ifx_pce_tm_find_vlan_id_index(&pHandle->pce_sub_tbl,pAct->nVLAN_Id ); */
			tab_index = ifx_pce_tm_find_vlan_id_fd_index(&pHandle->pce_sub_tbl, &VLAN_Table_Entry);
			if (tab_index != 0x7F ) {
...
/*			case IFX_FLOW_PCE_ACTION_METER_2:
   ...
   /** Action "RMON" Group. RMON action enable */
	...
	if(pAct->bPortBitMapMuxControl == IFX_TRUE)
		pcetable.val[6] |= (1 << 0); /* Port bitmap is used for STAG port member */
	else
		pcetable.val[6] &= ~(1 << 0); /* Port bitmap is used for alternative port map  function */
	/* Trunking action enable */
	if(pAct->bPortTrunkAction == IFX_TRUE)
		pcetable.val[5] |= (1 << 0); /* action enable */
	else
		pcetable.val[5] &= ~(1 << 0); /* action disable */
		/* Port Link Selection control */
	if(pAct->bPortLinkSelection == IFX_TRUE)
		pcetable.val[5] |= (1 << 1); /* higher port */
	else
		pcetable.val[5] &= ~(1 << 1); /* lower port */
