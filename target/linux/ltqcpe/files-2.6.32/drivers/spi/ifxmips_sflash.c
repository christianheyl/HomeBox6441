...
static INLINE int 
ifx_spi_set_4byte(ifx_spi_dev_t *dev, int enable)
{
    u8 cmd;
    u8 data = 0;
    ...
    switch (dev->manufacturer_id) {
        case JED_MANU_MX:
            ...
            return ifx_sflash_session(dev, cmd, NULL, 0, NULL, 0, NULL, 0);
        default:
            /* Spansion style */
            cmd = OPCODE_BRWR;
            data = enable << 7;
            return ifx_sflash_session(dev, cmd, NULL, 0, &data, 1, NULL, 0);
        }
}
