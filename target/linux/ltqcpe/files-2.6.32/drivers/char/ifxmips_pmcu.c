...
/**
   ...
   <HR> 
   \n\n 
   <B>"Internal PMCU Flow to interact with the registered interfaces triggered by a state_change_request"</B>.
   \n\n
   \image html pre_state_post_processing.jpg
   \n
   \n
   <HR> 
   \n\n 
...
#define IFX_PMCU_DRV_VERSION     _MKSTR(DRV_PMCU_VER_MAJOR) "." \
                                 _MKSTR(DRV_PMCU_VER_MINOR) "." \
                                 _MKSTR(DRV_PMCU_VER_STEP)  "." \
                                 _MKSTR(DRV_PMCU_VER_TYPE)

/** driver version, what string */
#define DRV_PMCU_WHAT_STR "@(#)PMCU, Version " IFX_PMCU_DRV_VERSION
...
/*============================================================================*/
static ssize_t ifx_pmcu_read( struct file *filp, IFX_char_t *buf, size_t count,
							  loff_t *ppos );
static ssize_t ifx_pmcu_write( struct file *filp, const IFX_char_t *buf, 
							   size_t count, loff_t *ppos );
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)) 
	static int ifx_pmcu_ioctl(struct inode *inode, struct file *file,
							  unsigned int cmd, unsigned long arg);
#else 
	static long ifx_pmcu_ioctl(struct file *file,unsigned int cmd,
							   unsigned long arg);
#endif 
...
#ifdef CONFIG_PROC_FS
static IFX_int_t ifx_pmcu_proc_device_list(IFX_char_t *page, IFX_char_t **start,
										   off_t offset, IFX_int_t count, 
										   IFX_int_t *eof, IFX_void_t *data);
static IFX_int_t ifx_pmcu_proc_version(IFX_char_t *buf, IFX_char_t **start, 
									   off_t offset, IFX_int_t count, 
									   IFX_int_t *eof, IFX_void_t *data);
static IFX_int_t ifx_pmcu_proc_get_internal_map(IFX_char_t *buf, 
												IFX_char_t **start, 
												off_t offset, IFX_int_t count, 
												IFX_int_t *eof, 
												IFX_void_t *data);
...
static IFX_void_t ifx_pmcu_process_req_tasklet(unsigned long data)
{
	...
	while (ifx_pmcu_reqBufferSize > 0) {
		...
		if (reqState.reqId == IFX_PMCU_NO_PENDING_REQ) {
			PMCU_ERR(PMCU, "No valid PMCU Request in buffer, but tasklet is "
						   "scheduled!!!!!\n");
			return;
		}
		/* handle PMCU request */
		...
		//PMCU_DEBUG_PRINT_ACCU_DEPLIST(IFX_PMCU_ACCU_DEPLIST_MAX); 
		//PMCU_DEBUG_PRINT_MOD_MEM();

		/* prepare pmcuModuleState for pre step */
		/* --> start pre processing by calling preCallBack functions of all
		   affected modules */
		count_start = ltq_count0_read();
		ret = ifx_pmcu_prechange(&pmcuPreCallIndex);
		//ltq_count0_diff(count_start, ltq_count0_read(),NULL,0);
		...
		if (ret == IFX_PMCU_RETURN_SUCCESS) {
			...
			//ltq_count0_diff(count_start, ltq_count0_read(),NULL,1);
			if (ret != IFX_PMCU_RETURN_SUCCESS) {
			...
			/* signalize successful state change to message buffer used by read
			   operation */
			pmcu_msg_g = pmcuModuleState.pmcuState;
			wake_up(&ifx_pmcu_wait_q);
		}
		...
		//ltq_count0_diff(count_start, ltq_count0_read(),NULL,2);
		if (ret != IFX_PMCU_RETURN_SUCCESS) {
...
static IFX_PMCU_RETURN_t ifx_pmcu_pwr_feature_switch (IFX_PMCU_MODULE_PWR_FEATURE_STATE_t pwrFeaSwitch)
{
	...
	/* check for ALL or single power switch */
	if (pwrFeaSwitch.pmcuModule == IFX_PMCU_MODULE_PMCU) {
		/* ALL */
		i = 0;
	} else {
		i = pwrFeaSwitch.pmcuModule;
	}

	for (;i < IFX_PMCU_MODULE_ID_MAX ; i++) {
		for (j=0;j < MAX_PMCU_SUB_MODULE; j++) {
			if (pwrFeaSwitch.pmcuModule != IFX_PMCU_MODULE_PMCU) {
				j = pwrFeaSwitch.pmcuModuleNr;
			}
			/* check if the module supports enable/disable of power feature(s) */
			if (ifx_pmcu_map[i].pmcuRegister[j].ifx_pmcu_pwr_feature_switch == NULL) {
				if (pwrFeaSwitch.pmcuModule != IFX_PMCU_MODULE_PMCU) {
					PMCU_WARNING(PMCU, "Module %s has no valid Callback defined for pwr_feature_switch\n",ifx_pmcu_mod[i]);
					return IFX_PMCU_RETURN_DENIED;
				}
				continue;
			}
			ret = ifx_pmcu_map[i].pmcuRegister[j].ifx_pmcu_pwr_feature_switch(pwrFeaSwitch.pmcuPwrFeatureState);
			if (ret != IFX_PMCU_RETURN_SUCCESS) {
				...
				if (pwrFeaSwitch.pmcuPwrFeatureState == IFX_PMCU_PWR_STATE_ON) {
					ifx_pmcu_map[i].dbgFlag &= ~(1 << j);
				} else {
					ifx_pmcu_map[i].dbgFlag |= 1 << j;
				}
			}  
			/* update power state in pmcu_map[] */
			pmcuModState.pmcuModule = (IFX_PMCU_MODULE_t)i;
			...
			if (ret != IFX_PMCU_RETURN_SUCCESS) {
				...
				ifx_pmcu_map[i].pmcuOldState[j] = pmcuModState.pmcuState;
			}
			if (pwrFeaSwitch.pmcuModule != IFX_PMCU_MODULE_PMCU) {
				return IFX_PMCU_RETURN_SUCCESS;
			}
...
#if 0
static char *msg_Ptr = "Lantiq PMCU device\n";
...
#endif 
{
	...
	IFX_PMCU_STATE_t supported_power_states[4];
	IFX_CPUFREQ_STAT_t* freq_tab = (IFX_CPUFREQ_STAT_t*)0xFF;
	IFX_CPUFREQ_STAT_t* freq_tab_org = (IFX_CPUFREQ_STAT_t*)0xFF;
	...
	switch (cmd) {
		...
		case IFX_PMCU_SET_SUPPORTED_POWER_STATES:
			{
				PMCU_FUNC(PMCU, "%s IFX_PMCU_SET_SUPPORTED_POWER_STATES is called\n",__FUNCTION__);
				copy_from_user(&supported_power_states, (IFX_PMCU_STATE_t *)arg, sizeof(supported_power_states));
				if(ifx_cpufreq_freqtab_get(NULL,&freq_tab,&freq_tab_org) == IFX_SUCCESS){
					if((freq_tab != (IFX_CPUFREQ_STAT_t*)0xFF)&&(freq_tab_org != (IFX_CPUFREQ_STAT_t*)0xFF)){
						for(i=0;i<4;i++){
							if(supported_power_states[i] == 0){
								(freq_tab + i)->cpu_freq = 0;
							}else{
								(freq_tab + i)->cpu_freq = (freq_tab_org + i)->cpu_freq;
							}
						}
					}
				}
...
#endif   /* MODULE */
	.read		= ifx_pmcu_read,
	.write		= ifx_pmcu_write,
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
    .unlocked_ioctl = ifx_pmcu_ioctl,
#else
    .ioctl		= ifx_pmcu_ioctl,
#endif
	.open		= ifx_pmcu_open,
	.release	= ifx_pmcu_release,
	.poll		= ifx_pmcu_poll,
};
...
//	PMCU_INFO(PMCU_1,"buf=%p, offset=%lu, count=%d, datap=%p, eof=%p, len=%d\n",buf,(unsigned long)offset,count,data,eof,len);
	...
	if(ifx_cpufreq_freqtab_get(cpu_freq,NULL,NULL) == IFX_PMCU_RETURN_SUCCESS){
		len += sprintf(buf+len, "Available CPU Frequencies: = ");
		for(i=0;i<4;i++){
			if(cpu_freq[i] == 0){
				break;
			}
			len += sprintf(buf+len, "%dMHz ",cpu_freq[i]);
		}
	}
	len += sprintf(buf+len, "\n\n");
	...
	len += sprintf(buf+len, "\tf\t :power-saving-feature disabled (0=enabled, 1=disabled)\n");
	len += sprintf(buf+len, "\tCPU/Sub0 :frequency scaling for CPU and DDR.\n");
	len += sprintf(buf+len, "\tCPU/Sub1 :mips_core clock gating feature. Only feature switch possible\n");
	len += sprintf(buf+len, "\tVE/Sub0  :tapi frequency change device\n");
	len += sprintf(buf+len, "\tVE/Sub1  :vmmc phone detection feature. Only feature switch possible\n\n");
...
static int ifx_pmcu_proc_version(char *buf, char **start, off_t offset, int count, int *eof, void *data)
{
	...
	len += sprintf(buf+len, "Compiled on %s, %s for Kernel version %s\n",__DATE__, __TIME__, UTS_RELEASE);
...
static int ifx_pmcu_proc_EntriesInstall(void)
{
	...
	g_ifx_pmcu_dir_pmcu = proc_mkdir("driver/pmcu", NULL);
	...
	g_ifx_pmcu_dir_debug = proc_mkdir("driver/pmcu/debug", NULL);
