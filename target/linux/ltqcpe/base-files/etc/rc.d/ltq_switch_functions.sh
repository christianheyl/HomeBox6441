...
select_wan_mode() {
   ...
   if [ "$CONFIG_PACKAGE_QUANTENNA_TYPE_TWO_RGMII" = "1" ]; then
        case "$CONFIG_PACKAGE_QUANTENNA_ETH_WAN_PORT" in 
	"1")
   		local wan_reg_mii1="0x2"
	;;
	"2")
   		local wan_reg_mii1="0x4"
	;;
	"4")
   		local wan_reg_mii1="0x10"
	;;
	*)
		echo "Wrong QUANTENNA_ETH_WAN_PORT !"
	;;
	esac
   else
...
################################################
   ...
   if [ "$CONFIG_IFX_CONFIG_CPU" != "AMAZON_S" ]; then
   	switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_FLOW_REGISTER_SET nRegAddr=0xCCD nData=$wan_reg	
   fi
}
...
###############################################################################################################
create_vlanid() {
    switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_ID_CREATE nVId=$1 nFId=$2
}
...
###############################################################################################################
delete_vlanid() {
    switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_ID_DELETE nVId=$1
}
###############################################################################################################
...
###############################################################################################################
config_port_pvid() {
    switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_PORT_CFG_SET nPortId=$1 nPortVId=$2 bVLAN_UnknownDrop=0 bVLAN_ReAssign=0 eVLAN_MemberViolation=3 eAdmitMode=0 bTVM=$3
}
...
###############################################################################################################
reset_port_pvid() {
    switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_PORT_CFG_SET nPortId=$1 nPortVId=0 bVLAN_UnknownDrop=0 bVLAN_ReAssign=0 eVLAN_MemberViolation=0 eAdmitMode=0 bTVM=0
}
...
config_port_member() {
      ...
      local port=$2
      local egress_tag=$3
      switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_PORT_MEMBER_ADD nVId=$vid nPortId=$port bVLAN_TagEgress=$egress_tag
}
...
reset_port_member() {
      ...
      local vid=$1
      local port=$2
      switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_PORT_MEMBER_REMOVE nVId=$vid nPortId=$port
}
...
# By default GPHY LEDs are configured by GPHY driver. Some additional configs are needed to support new
# platforms or board designs
###############################################################################################################
config_led() {
# config GPHY1 led for AR10
if [ "$CONFIG_IFX_CONFIG_CPU" = "XRX3XX" ]; then
        switch_cli  IFX_ETHSW_MMD_DATA_WRITE nAddressDev=0x1 nAddressReg=0x1F01e2 nData=0x42
        switch_cli  IFX_ETHSW_MMD_DATA_WRITE nAddressDev=0x1 nAddressReg=0x1F01e3 nData=0x10
        switch_cli  IFX_ETHSW_MMD_DATA_WRITE nAddressDev=0x1 nAddressReg=0x1F01e4 nData=0x70
        switch_cli  IFX_ETHSW_MMD_DATA_WRITE nAddressDev=0x1 nAddressReg=0x1F01e5 nData=0x03
fi
# config GPHY3 led for GRX390
if [ "$CONFIG_PACKAGE_KMOD_LTQCPE_GRX_390" = "1" ]; then
        switch_cli  IFX_ETHSW_MMD_DATA_WRITE nAddressDev=0x3 nAddressReg=0x1F01e2 nData=0x42
        switch_cli  IFX_ETHSW_MMD_DATA_WRITE nAddressDev=0x3 nAddressReg=0x1F01e3 nData=0x10
        switch_cli  IFX_ETHSW_MMD_DATA_WRITE nAddressDev=0x3 nAddressReg=0x1F01e4 nData=0x70
        switch_cli  IFX_ETHSW_MMD_DATA_WRITE nAddressDev=0x3 nAddressReg=0x1F01e5 nData=0x03
fi
# change GPHY2 SOUT definition for EASY300/330 board.
if [ "$CONFIG_PACKAGE_KMOD_LTQCPE_EASY_3X0_BOND_EXTENSION" = "1" ]; then
...
switch_init() {
	...
	output=`switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_PORT_CFG_GET nPortId=$lan_port_1| grep -w nPortVId | awk '{ print $2 }'`
	
	if [ $output = $lan_vid_all -o $output = $lan_vid_port_start ]; then
		switch_uninit
	fi
	
	switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_DISABLE
	switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_CFG_SET eMAC_TableAgeTimer=3 bVLAN_Aware=1 nMaxPacketLen=1536 bPauseMAC_ModeSrc=0 nPauseMAC_Src=00:00:00:00:00:00        
	...
	# egress tagging enabled on CPU port for that VLAN.
	   ...
	   if [ $i = $wan_port_mii1 -o $i = $wan_port_mii0 ]; then
		...
		config_port_pvid $i $wan_vid 0
	   else
           	config_port_pvid $i $wan_vid 1
	   fi
        done

 	# Setting PVID of CPU port as 500	
        config_port_pvid $cpu_port $cpu_vid 0 

	# Setting PVID of LAN ports as 501
	for i in $lan_port_all; do
          config_port_pvid $i $lan_vid_all 1
        done  
	...
	switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_ENABLE
	
	# spilt the lan and wan interrupts (DMA channels)
	split_lan_wan_dma_channel

	# configu link aggregation for QUANTENNA 11ac coneected with 2 RGMII
        if [ "$CONFIG_PACKAGE_QUANTENNA_TYPE_TWO_RGMII" = "1" ]; then
		config_link_aggregation
	fi
	...
	# Config GPHY LEDs
        ...
        #switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_PORT_MEMBER_READ 
}
...
switch_uninit() {
	...
	if [ "$CONFIG_IFX_CONFIG_CPU" != "AMAZON_S" ]; then
#		switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_HW_INIT
   		delete_vlanid $lan_vid_all
	    	delete_vlanid $cpu_vid
    		delete_vlanid $wan_vid
	else
		switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_HW_INIT eInitMode=0
	fi	
	switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_ENABLE
}
...
add_mii1_to_lan() {
	...
	select_wan_mode
   	if [ config_wan != "2" -a sec_wan != "2" ]; then
		switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_FLOW_REGISTER_SET nRegAddr=0xCCD nData=$wan_reg_mii1
   		switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_PORT_CFG_SET nPortId=$wan_port_mii1 eEnable=1 bLearningMAC_PortLock=1 
		ifconfig eth1 up   
		...
		switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_MAC_TABLE_CLEAR
	else
...
del_mii1_frm_lan() {
	select_wan_mode
	if [ config_wan != "2" -a sec_wan != "2" ]; then
		switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_PORT_CFG_SET nPortId=$wan_port_mii1 eEnable=1 bLearningMAC_PortLock=0		
...
multicast_setting() {
     ...
     for i in $lan_port $wlan_port $cpu_port $wan_port; do
       switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_MULTICAST_ROUTER_PORT_REMOVE nPortId=$i
     done
     echo "Setting multicast router port= $wan_port"
     for i in $wan_port; do
       switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_MULTICAST_ROUTER_PORT_ADD nPortId=$i
     done
...
###############################################################################################################
enable_pppoa_conf() {
    switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_MAC_TABLE_ENTRY_ADD nFId=$wan_fid nPortId=6 nAgeTimer=0 bStaticEntry=1 nMAC=00:00:00:00:00:00
}

disable_pppoa_conf() {
    switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_MAC_TABLE_ENTRY_REMOVE nFId=$wan_fid nMAC=00:00:00:00:00:00
}
...
enable_separation() {
	...
	j=0
        for i in $lan_port; do
          config_port_pvid $i `expr $lan_vid_port_start \+ $j` 1
  	  j=`expr $j \+ 1`
        done   
...
enable_driver_separation() {
	insmod /lib/modules/`uname -r`/ethsw.ko
	j=0
        for i in $lan_port; do
...
disable_driver_separation() {
	...
	brctl addif br0 eth0
	rmmod /lib/modules/`uname -r`/ethsw.ko
}
      ...
      if [ $output != $lan_vid_port_start ]; then
         ...
         for i in $lan_port; do
           if [ $i = $1 ]; then
             ...
             for i in $lan_port; do
		switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_PORT_MEMBER_REMOVE nVId=`expr $lan_vid_port_start \+ $1` nPortId=$i
             done
           fi     
         done
      fi
      ...
      if [ $output != $lan_vid_port_start ]; then
         ...
         for i in $lan_port; do
           if [ $i = $1 ]; then
             echo "Joining port $i"
             for i in $lan_port; do
		switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_PORT_MEMBER_ADD nVId=`expr $lan_vid_port_start \+ $1` nPortId=$i bVLAN_TagEgress=0
             done
           fi     
         done
      fi
...
debug_dump() {
       ...
       echo -n "CCD          " 
       switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_FLOW_REGISTER_GET nRegAddr=0xCCD|grep nData
       echo -n "Multicast = " 
       switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_MULTICAST_ROUTER_PORT_READ
       echo "========MAC Table====================="
       switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_MAC_TABLE_ENTRY_READ
       sleep 1
       echo -e "\n"
       echo "========Multicast Table==============="
       switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_MULTICAST_TABLE_ENTRY_READ
       echo "========VLAN Membership==============="
       switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_VLAN_PORT_MEMBER_READ 
       echo "============= RMON Counts============="
       for i in $lan_port $cpu_port 5; do
         echo  "+++++++++++++++++++++ Port $i +++++++++++++++++++++"
	 switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_ETHSW_RMON_GET nPortId=$i|grep "xGood\|xError\|xDrop\|Filter"
       done
       echo "============= PCE Table============="
       i=0
       while [ $i -le 63 ]
       do
	 output=`switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_FLOW_PCE_RULE_READ pattern.nIndex=$i| grep -w pattern.bEnable | awk '{ print $2 }'`
         if [  $output != "FALSE" ]; then
            echo  "+++++++++++++++++++++ Port $i +++++++++++++++++++++"	
	    switch_cli dev=$CONFIG_LTQ_SWITCH_DEVICE_ID IFX_FLOW_PCE_RULE_READ pattern.nIndex=$i
         fi
         i=`expr $i + 1`
       done
}
