...
typedef struct
{
   ...
   IFX_boolean_t                                bAggregateEnable;
...
/** Configures the WRED threshold parameter per queue.
    The min. and max. values are given in number of packet
    buffer segments. The size of a segment can be
    retrieved using \ref IFX_ETHSW_CAP_GET.
...
typedef struct
{
   ...
   /** Discard Unknown IP Multicast Packets.
       ...
       - IFX_TRUE: Drop unknown multicast packets.
       - IFX_FALSE: Forward unknown multicast packets for the
...
typedef struct
{
   ...
   */
   ...
   /** Accept or discard MAC spoofing and port MAC locking violation packets.
       ...
       - IFX_TRUE: Forward
   */
   IFX_boolean_t                                bMAC_SpoofingAction;
...
typedef struct
{
   ...
   /** MAC spoofing detection. Identifies ingress packets that carry
       ...
       This parameter is only taken into account when 'bLearning' is enabled. */
   IFX_boolean_t                                bMAC_SpoofingDetection;
...
{
   /** WoL MAC address. */
   IFX_uint8_t                                  nWolMAC[IFX_MAC_ADDRESS_LENGTH];
   /** WoL password. */
   IFX_uint8_t                                  nWolPassword[IFX_MAC_ADDRESS_LENGTH];
   /** WoL password enable. */
