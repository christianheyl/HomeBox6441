...
define Kernel/MenuConfig
	$(call Kernel/MenuConfig/Default)
endef

define SetupDriverLink
	$(call SetupKernelLink/Default,$(1),$(2))
endef

define SetupKernelLinks
	$(call Kernel/SetupLinks/Default)
endef

define RemoveKernelLinks
	$(call Kernel/RemoveLinks/Default)
endef
...
ifeq ($(strip $(CONFIG_EXTERNAL_KERNEL_TREE)),"")
  ...
  oldconfig menuconfig: $(STAMP_PREPARED) $(STAMP_CHECKED) FORCE
	...
	[ -e "$(LINUX_DIR)"/.config ] || ( $(Kernel/MenuConfig) )
	$(_SINGLE)$(MAKE) -C $(LINUX_DIR) $(KERNEL_MAKEOPTS) $$@

  change_default:$(STAMP_PREPARED) FORCE
	[ -e "$(LINUX_CONFIG)" ] || touch "$(LINUX_CONFIG)"
	[ -e "$(LINUX_DIR)/.config" ] || ( $(Kernel/MenuConfig) )
	@if [ -w "$(if $(LINUX_SUBCONFIG),$(LINUX_SUBCONFIG),$(LINUX_CONFIG))" ]; then \
		$(SCRIPT_DIR)/kconfig.pl '>' $(GENERIC_LINUX_CONFIG) $(LINUX_DIR)/.config > $(if $(LINUX_SUBCONFIG),$(LINUX_SUBCONFIG),$(LINUX_CONFIG)); \
	else \
		echo "can not modify $(if $(LINUX_SUBCONFIG),$(LINUX_SUBCONFIG),$(LINUX_CONFIG))"; \
		true; \
	fi
  ...
  redo-links: remove-links setup-links
