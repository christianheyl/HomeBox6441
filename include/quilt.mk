...
define SetupLink/Default
	...
	cp $(1) $(2)
endef
# Terry 20140113, TODO, use copy instead of symbolic link
# ln -sf $(1) $(2)

define RemoveLink/Default
	if [ -L $(1) ];then \
		rm -rf $(1); \
	else \
		echo "Warning: $(1) is not a link. Can not remove $(1)"; \
	fi
endef

define SetupKernelLink/Default
	$(call SetupLink/Default,$(1),$(2))
endef

define RemoveKernelLink/Default
	$(call RemoveLink/Default,$(1))
endef

define Kernel/Files
		CURR_DIR=`cd $(1) && pwd -P`; \
		cd $(1); \
		for file in `find . -follow -type f -printf "%P\n" 2>/dev/null |grep -v svn`; \
		do \
			DIR_NAME=`dirname $$$$file`; \
			FILE_NAME=`basename $$$$file`; \
			if [ $(2) == setup ];then \
				$(call SetupKernelLink/Default,$$$$CURR_DIR/$$$$file,$(LINUX_DIR)/$$$$DIR_NAME/$$$$FILE_NAME); \
			elif [ $(2) == clean ];then \
				$(call RemoveKernelLink/Default,$(LINUX_DIR)/$$$$DIR_NAME/$$$$FILE_NAME); \
			fi; \
		done
endef

define Kernel/SetupLinks/Default
	$(call Kernel/Files,$(FILES_DIR),setup)
endef

define Kernel/RemoveLinks/Default
	$(call Kernel/Files,$(FILES_DIR),clean)
endef
...
generic_files=$(foreach fdir,$(GENERIC_FILES_DIR),$(fdir)/.)
files_dir=$(foreach fdir,$(FILES_DIR),$(fdir)/.)
define Kernel/Patch/Default
	rm -rf $(PKG_BUILD_DIR)/patches; mkdir -p $(PKG_BUILD_DIR)/patches
	$(if $(generic_files),$(CP) $(generic_files) $(LINUX_DIR)/)
	$(if $(files_dir), \
		$(call Kernel/SetupLinks/Default); \
	)
