...
void usage(void)
{
	...
	fprintf(stderr, "     [-f file] [-f file [-f file [-f file (v2 only)]]] [-c trx_magic(hex)]\n");
	exit(EXIT_FAILURE);
}
...
int main(int argc, char **argv)
{
	...
	struct trx_header *p;
	char trx_version = 1;
	unsigned long t_magic = TRX_MAGIC;
	...
	while ((c = getopt(argc, argv, "-:2o:m:a:x:b:f:A:c:")) != -1) {
		switch (c) {
			case '2':
...
#define ROUND 0x1000
			...
			case 'c':
				errno = 0;
				t_magic = strtoul(optarg, &e, 0);
				if (errno || (e == optarg) || *e) {
					fprintf(stderr, "illegal numeric string\n");
					usage();
				}
				p = (struct trx_header *) buf;
				p->magic = STORE32_LE(t_magic);
				break;
			default:
				usage();
		}
